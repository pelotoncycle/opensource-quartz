...
struct image_type_params {
	...
	/*
	 * piggyback size is local to the specific image type to be supported,
	 * mkimage core treats this as number of bytes
	 */
	uint32_t piggyback_size;
	/* Image type piggyback pointer */
	void *piggyback;
	/*
	 * The piggyback contents need to be set as per image type to
	 * be generated using this callback function.
	 * further output file post processing (for ex. checksum calculation,
	 * padding bytes etc..) can also be done in this callback function.
	 */
	void (*set_piggyback) (void *, struct stat *, off_t, int, struct mkimage_params *);
