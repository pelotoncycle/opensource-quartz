...
NXTARG:		;
	...
	if (params.lflag || params.fflag) {
		...
		if (tparams->piggyback_size && tparams->piggyback && tparams->set_piggyback) {
			if ((unsigned)sbuf.st_size < (tparams->header_size + tparams->piggyback_size)) {
				fprintf (stderr,
					"%s: Bad size (piggyback): \"%s\" is not valid image\n",
					params.cmdname, params.imagefile);
				exit (EXIT_FAILURE);
			}
			sbuf.st_size -= (off_t)tparams->piggyback_size;
		}
...
#else
		ptr = mmap(0, st_size, PROT_READ, MAP_SHARED, ifd, 0);
#endif
		if (ptr == MAP_FAILED) {
	...
	if (!params.skipcpy) {
		...
		if (tparams->piggyback_size && tparams->piggyback && tparams->set_piggyback) {
			memset(tparams->piggyback, 0, tparams->piggyback_size);

			if (write(ifd, tparams->piggyback, tparams->piggyback_size)
							!= tparams->piggyback_size) {
				fprintf (stderr, "%s: Write piggyback error on %s: %s\n",
					params.cmdname, params.imagefile, strerror(errno));
				exit (EXIT_FAILURE);
			}
		}
   ...
   !defined(__APPLE__)
...
#else
	ptr = mmap(0, st_size, PROT_READ|PROT_WRITE, MAP_SHARED, ifd, 0);
#endif
	if (ptr == MAP_FAILED) {
	...
	if (tparams->piggyback_size && tparams->piggyback && tparams->set_piggyback) {
		tparams->set_piggyback (ptr, &sbuf, st_size, ifd, &params);
	}
