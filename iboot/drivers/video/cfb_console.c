 ...
 * some Macros
 */
#ifdef CONFIG_VIDEO_LINE_STRIDE
/* use plnSizeX for the line stride */
#define VIDEO_LINE_STRIDE	(pGD->plnSizeX)
#else
#define VIDEO_LINE_STRIDE	(pGD->winSizeX)
#endif
...
#define VIDEO_FLUSH		video_flush()
#else
#define VIDEO_FLUSH
...
#if defined(CONFIG_CMD_BMP) || defined(CONFIG_SPLASH_SCREEN) || defined(CONFIG_VIDEO_BOOTLOGO)
#include <watchdog.h>
...
#else
#ifdef CONFIG_VIDEO_BOOTLOGO_ALIGN
#define BMP_ALIGN_CENTER	0x7FFF
#endif /* CONFIG_VIDEO_BOOTLOGO_ALIGN */
#endif
...
#else
#define VIDEO_SIZE		(VIDEO_ROWS*VIDEO_LINE_STRIDE*VIDEO_PIXEL_SIZE)
#define VIDEO_PIX_BLOCKS	(VIDEO_SIZE >> 2)
#define VIDEO_LINE_LEN		(VIDEO_LINE_STRIDE*VIDEO_PIXEL_SIZE)
#endif
...
void video_printf(const char *fmt, ...)
{
	...
	va_start(args, fmt);
	(void)vscnprintf(buf, sizeof(buf), fmt, args);
	va_end(args);
...
#else
	fbp = (unsigned char *) ((unsigned int) video_fb_address +
				 (((y + yoff) * VIDEO_LINE_STRIDE) + xoff) * bpp);
#endif
...
int video_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	if (!((bmp->header.signature[0] == 'B') &&
...
#else
		dst = (unsigned char*)CONFIG_SYS_VIDEO_LOGO_ADDR;
#endif /* CONFIG_SYS_VIDEO_LOGO_ADDR */
		if (dst == NULL) {
			printf("Error: malloc in gunzip failed!\n");
			return 1;
		}
		debug("Uncompressing bmp.gz...0x%08lx->0x%08x\n", bmp_image, (u32)dst);
		...
		debug("Uncompressing bmp.gz...done\n");
	...
	debug("Display-bmp: %ld x %ld with %d colors\n",
	      width, height, colors);
#else
	debug("Display-bmp: %lu x %lu x %lu bpp with %d colors\n",
...
#if defined(CONFIG_SPLASH_SCREEN_ALIGN) || defined(CONFIG_VIDEO_BOOTLOGO_ALIGN)
...
#endif /* CONFIG_SPLASH_SCREEN_ALIGN || CONFIG_VIDEO_BOOTLOGO_ALIGN */
	...
	debug("Display-bmp: x %d, y %d, w %lu, h %lu\n",
			x, y, width, height);
	...
	debug("Display-bmp: drawing...video_fb_address 0x%08X, fb 0x%08X\n",
			(unsigned int)video_fb_address, (unsigned int)fb);
...
#else
		printf("ERR: %lu bit/pixel bitmaps not supported\n",
			bpp);
#endif
		break;
	}

	debug("Display-bmp: drawing...done\n");
...
#else
	int skip_stride = VIDEO_LINE_STRIDE - VIDEO_VISIBLE_COLS;
	int skip = (skip_stride + width - VIDEO_LOGO_WIDTH) * VIDEO_PIXEL_SIZE;
#endif
...
#else
	unsigned char *dest = (unsigned char *) screen +
		((y * (skip_stride + width) * VIDEO_PIXEL_SIZE) + x * VIDEO_PIXEL_SIZE);
#endif
...
#else
	space = ((VIDEO_COLS*VIDEO_PIXEL_SIZE) / 2 - VIDEO_INFO_X) / VIDEO_FONT_WIDTH;
#endif
	len = strlen(info);
...
static int __board_video_bootlogo(ulong bmp_image, ulong bmp_max_size)
{
	return -1;
}

int board_video_bootlogo(ulong bmp_image, ulong bmp_max_size)
	__attribute__ ((weak, alias("__board_video_bootlogo")));

static void *video_bootlogo(void)
{
	int ret;
	ulong addr = CONFIG_VIDEO_BOOTLOGO_IMAGE_ADDR;

	ret = board_video_bootlogo(addr, CONFIG_VIDEO_BOOTLOGO_MAX_SIZE);
	if (!ret) {
		int x, y;

#ifdef CONFIG_VIDEO_BOOTLOGO_HEIGHT
		video_logo_height = CONFIG_VIDEO_BOOTLOGO_HEIGHT;
#else
		video_logo_height = 0;
#endif
		...
		if (video_display_bitmap(addr, x, y))
			printf("ERR: failed to display bootlogo\n");
	} else {
		printf("ERR: cannot load bootlogo (%d)\n", ret);

#ifdef CONFIG_SYS_WHITE_ON_BLACK
		/* WHITE BG & BLACK FG */
		memset((void*)VIDEO_FB_ADRS, 0xFF, VIDEO_SIZE);
#else
		/* BLACK BG & WHITE FG */
		memset((void*)VIDEO_FB_ADRS, 0x00, VIDEO_SIZE);
...
#endif /* CONFIG_VIDEO_BOOTLOGO */

#ifdef CONFIG_VIDEO_TEST_PATTERN

#define	N_BLK_VERT	2
#define	N_BLK_HOR	4

#define CONSOLE_COLOR565_BLACK	0x0000
#define CONSOLE_COLOR565_RED	0xF800
#define CONSOLE_COLOR565_GREEN	0x07E0
#define CONSOLE_COLOR565_YELLOW	0xFFE0
#define CONSOLE_COLOR565_BLUE	0x001F
#define CONSOLE_COLOR565_MAGENTA	0xF81F
#define CONSOLE_COLOR565_CYAN	0x07FF
#define CONSOLE_COLOR565_GREY	0x7BEF
#define CONSOLE_COLOR565_WHITE	0xffff

#define CONSOLE_COLOR8888_BLACK	0x00000000
#define CONSOLE_COLOR8888_RED	0x00FF0000
#define CONSOLE_COLOR8888_GREEN	0x0000FF00
#define CONSOLE_COLOR8888_YELLOW	0x00FFFF00
#define CONSOLE_COLOR8888_BLUE	0x000000FF
#define CONSOLE_COLOR8888_MAGENTA	0x00FF00FF
#define CONSOLE_COLOR8888_CYAN	0x0000FFFF
#define CONSOLE_COLOR8888_GREY	0x00C0C0C0
#define CONSOLE_COLOR8888_WHITE	0x00FFFFFF	/* Must remain last / highest	*/

static int test_colors[N_BLK_HOR*N_BLK_VERT];

static void *video_test_pattern(void)
{
	uint v_max  = VIDEO_VISIBLE_ROWS;
	uint h_max  = VIDEO_VISIBLE_COLS;
	uint v_step = (v_max + N_BLK_VERT - 1) / N_BLK_VERT;
	uint h_step = (h_max + N_BLK_HOR  - 1) / N_BLK_HOR;
	uint v, h;
	uint line_padding = VIDEO_LINE_STRIDE - VIDEO_VISIBLE_COLS;

	printf ("[VIDEO] Test Pattern 0x%08x: %d x %d [%d x %d]: %u\n",
			(unsigned int)video_fb_address,
			h_max, v_max, h_step, v_step, VIDEO_DATA_FORMAT);

	switch (VIDEO_DATA_FORMAT) {
	case GDF_16BIT_565RGB: {
		unsigned short *pix = (unsigned short *)video_fb_address;

		test_colors[0] = CONSOLE_COLOR565_BLACK;
		test_colors[1] = CONSOLE_COLOR565_RED;
		test_colors[2] = CONSOLE_COLOR565_GREEN;
		test_colors[3] = CONSOLE_COLOR565_YELLOW;
		test_colors[4] = CONSOLE_COLOR565_BLUE;
		test_colors[5] = CONSOLE_COLOR565_MAGENTA;
		test_colors[6] = CONSOLE_COLOR565_CYAN;
		test_colors[7] = CONSOLE_COLOR565_WHITE;
	...
	case GDF_32BIT_X888RGB: {
		unsigned int *pix = (unsigned int *)video_fb_address;

		test_colors[0] = CONSOLE_COLOR8888_BLACK;
		test_colors[1] = CONSOLE_COLOR8888_RED;
		test_colors[2] = CONSOLE_COLOR8888_GREEN;
		test_colors[3] = CONSOLE_COLOR8888_YELLOW;
		test_colors[4] = CONSOLE_COLOR8888_BLUE;
		test_colors[5] = CONSOLE_COLOR8888_MAGENTA;
		test_colors[6] = CONSOLE_COLOR8888_CYAN;
		test_colors[7] = CONSOLE_COLOR8888_WHITE;
	...
	default:
		printf("Error: unsupported data format %u\n", VIDEO_DATA_FORMAT);
		break;
	}

	return video_fb_address;
}
#endif /* CONFIG_VIDEO_TEST_PATTERN */
...
#if defined(CONFIG_VIDEO_TEST_PATTERN)
	debug("Video: Drawing the test pattern ...\n");
	video_console_address = video_test_pattern();
#elif defined(CONFIG_VIDEO_BOOTLOGO)
	debug("Video: Drawing the boot logo...\n");
	video_console_address = video_bootlogo();
#elif defined(CONFIG_VIDEO_LOGO)
...
#else /* CONFIG_VIDEO_NOT_CONSOLE_DEVICE */
int video_fb_init(void)
{
	int r;
