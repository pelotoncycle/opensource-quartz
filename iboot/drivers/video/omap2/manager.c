...
static int num_managers __attribute__ ((section (".data")));
static struct list_head manager_list __attribute__ ((section (".data")));
static struct omap_overlay_manager *mgrs[MAX_DSS_MANAGERS] __attribute__ ((section (".data")));
...
} dss_cache __attribute__ ((section (".data")));
...
static int dss_mgr_wait_for_vsync(struct omap_overlay_manager *mgr)
{
	...
	/* Return immediately if dss device is not active */
...
static int configure_dispc(void)
{
	...
	for (i = 0; i < num_ovls; ++i) {
		...
		if (!oc->dirty) {
			DSSDBG("configure_overlay %d not dirty\n", i);
			continue;
		}
		...
		if (mgr_busy[oc->channel]) {
			busy = true;
			DSSDBG("configure_overlay %d busy\n", i);
			continue;
		}
	...
	for (i = 0; i < num_mgrs; ++i) {
		...
		if (!mc->manual_upd_display){
			if(mc->skip_init)
				...
				DSSDBG("configure_overlay mgr %d go\n", i);
				dispc_go(i);
			}
		}
	}
...
int dss_init_overlay_managers(struct platform_device *pdev)
{
	...
	memset(&dss_cache, 0, sizeof(dss_cache));
...
#else
		mgr = memalign(ARCH_DMA_MINALIGN, sizeof(*mgr));
		if (mgr)
