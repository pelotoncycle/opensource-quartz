...
#endif
} dispc __attribute__ ((section (".data")));
...
static int wait_for_completion_timeout(struct completion *x, ulong timeout_ms);
static void dispc_error_worker(void);

static inline void dispc_write_reg(const u16 idx, u32 val) __attribute__((always_inline));
static inline void dispc_write_reg(const u16 idx, u32 val)
{
	__raw_writel(val, dispc.base + idx);
}

static inline u32 dispc_read_reg(const u16 idx) __attribute__((always_inline));
...
static void dispc_save_context(void)
{
	...
	if (dss_has_feature(FEAT_GLOBAL_ALPHA))
		SR(GLOBAL_ALPHA);
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND
	SR(GLOBAL_BUFFER);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
	...
	DSSVDBG("context saved, ctx_loss_count %d\n", dispc.ctx_loss_cnt);
}
...
static void dispc_restore_context(void)
{
	...
	DSSVDBG("ctx_loss_count: saved %d, current %d\n",
	...
	DSSVDBG("context restored\n");
}
#else
#define dispc_save_context()
#define dispc_restore_context()
#endif
...
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND
#ifdef CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND
	dispc_reg_config.gfx_top_buffer = 0;
	dispc_reg_config.gfx_bottom_buffer = 0;
	dispc_reg_config.wb_top_buffer = 0;
	dispc_reg_config.wb_bottom_buffer = 0;
#else
	dispc_reg_config.gfx_top_buffer = 4;
	dispc_reg_config.gfx_bottom_buffer = 4;
	dispc_reg_config.wb_top_buffer = 0;
	dispc_reg_config.wb_bottom_buffer = 0;
#endif /* CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND */
...
#ifdef CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND
	return max(1024U, sa_calc_wrap(&dispc_reg_config, channel_no));
#else
...
int dispc_runtime_get(void)
{
	...
	if (dispc.runtime_count++ == 0) {
		...
		DSSVDBG("dispc_runtime_get\n");

#if 0
		/* Sets DSS min latency constraint */
...
#else
		/* CD_L3_1 */
		omap_clk_domain(&prcm->cm_l3_1_clkstctrl, CD_CLKCTRL_CLKTRCTRL_NO_SLEEP);
		omap_pwrdm_wait_transition(&prcm->pm_core_pwrstctrl);
		/* CD_L3_2 */
		omap_clk_domain(&prcm->cm_l3_2_clkstctrl, CD_CLKCTRL_CLKTRCTRL_NO_SLEEP);
		omap_pwrdm_wait_transition(&prcm->pm_core_pwrstctrl);
#endif
...
#else
		/* CD_L3_1 */
		omap_clk_domain(&prcm->cm_l3_1_clkstctrl, CD_CLKCTRL_CLKTRCTRL_HW_AUTO);
		omap_pwrdm_wait_transition(&prcm->pm_core_pwrstctrl);
		/* CD_L3_2 */
		omap_clk_domain(&prcm->cm_l3_2_clkstctrl, CD_CLKCTRL_CLKTRCTRL_HW_AUTO);
		omap_pwrdm_wait_transition(&prcm->pm_core_pwrstctrl);
#endif
#endif /* CONFIG_OMAP44XX */
...
static void dispc_read_plane_fifo_sizes(void)
{
	...
	/*if (cpu_is_omap44xx())*/ {
#ifdef CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND
		size = dispc_read_reg(DISPC_GLOBAL_BUFFER);
		size = FLD_MOD(size, 0, 2, 0); /* GFX BUF top to GFX */
		size = FLD_MOD(size, 0, 5, 3); /* GFX BUF bottom to GFX */
		size = FLD_MOD(size, 0, 26, 24); /* WB BUF top to GFX */
		size = FLD_MOD(size, 0, 29, 27); /* WB BUF bottom to GFX */
		dispc_write_reg(DISPC_GLOBAL_BUFFER, size);

		dispc.fifo_size[OMAP_DSS_GFX] +=
			FLD_GET(dispc_read_reg(DISPC_OVL_FIFO_SIZE_STATUS(OMAP_DSS_WB)), start, end);

		DSSINFO("enable DSS FIFO WA2 (0x%08x)\n", size);
#else
		size = dispc_read_reg(DISPC_GLOBAL_BUFFER);
		size = FLD_MOD(size, 4, 2, 0); /* GFX BUF top to WB */
		size = FLD_MOD(size, 4, 5, 3); /* GFX BUF bottom to WB */
		size = FLD_MOD(size, 0, 26, 24); /* WB BUF top to GFX */
		size = FLD_MOD(size, 0, 29, 27); /* WB BUF bottom to GFX */
		dispc_write_reg(DISPC_GLOBAL_BUFFER, size);
		...
		DSSINFO("enable DSS FIFO WA1 (0x%08x)\n", size);
#endif /* CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND */
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG
		DSSINFO("FIFO size: GFX 0x%04x, VID1 0x%04x, VID2 0x%04x, VID3 0x%04x\n",
			dispc.fifo_size[OMAP_DSS_GFX],
			dispc.fifo_size[OMAP_DSS_VIDEO1],
			dispc.fifo_size[OMAP_DSS_VIDEO2],
			dispc.fifo_size[OMAP_DSS_VIDEO3]);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG */
	}
#endif /* CONFIG_OMAP44XX */
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
}
...
{
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=1fb9e3b7bd828e20f82ed94ae5f190610548de3b
 * OMAPDSS: DISPC: Allow -1 row-pixel increment on OMAP4/5
 */
#if 0
...
#else
#if 0
		/* Workaround: don't panic */
		static bool __print_once = false;
		if (!__print_once) {
			__print_once = true;
			DSSWARN("invalid pixinc: pixels %d, ps %d\n", pixels, ps);
			dump_stack();
		}
#else
		DSSWARN("invalid pixinc: pixels %d, ps %d\n", pixels, ps);
		...
		s32 *row_inc, unsigned *offset1, bool ilace)
{
	...
	*row_inc = view->v_inc * y_decim + 1 - width * bpp;
	...
	DSSVDBG(" ps: %d/%d, width: %d/%d, offset1: %d,"
		" height: %d, row_inc:%d\n", view->bpp, bpp,
		...
		s32 *row_inc, s32 *pix_inc)
{
	...
	DSSVDBG("calc_rot(%d): scrw %d, %dx%d\n", rotation, screen_width,
			width, height);
		...
		s32 *row_inc, s32 *pix_inc, int x_decim, int y_decim)
{
	...
	DSSDBG("calc_rot(%d): scrw %d, %dx%d\n", rotation, screen_width,
			width, height);
#else
	DSSDBG("calc_rot(%d,%d): scrw %d, %dx%d, field: mode %d, offset %u, ps %d\n", rotation, mirror, screen_width,
			width, height, fieldmode, field_offset, ps);
#endif
		...
		enum omap_color_mode color_mode)
{
	...
	u64 pclk = dispc_pclk_rate(channel);
	...
	if (height > out_height) {
		...
		if (mgr && mgr->device)
			ppl = mgr->device->panel.timings.x_res;
				...
				u16 *x_decim, u16 *y_decim, bool *five_taps)
{
	 ...
	 */
#if defined(CONFIG_OMAP44XX)
	bool can_decimate_x = !(bpp & 7);
#else
	bool can_decimate_x = false;
#endif /* CONFIG_OMAP44XX */
	...
	*	--------------------+
	...
	y = min_y_decim;
	while (1) {
		DSSVDBG("min_x_decim %d, max_x_decim %d, x %d; min_y_decim %d, max_y_decim %d, y %d\n",
				min_x_decim, max_x_decim, x, min_y_decim, max_y_decim, y);
		...
		*five_taps = true;
		...
		DSSVDBG("in_width %d, out_width %d; in_height %d, out_height %d\n",
		...
		DSSVDBG("%d*%d,%d*%d->%d,%d requires %lu(3T), %lu(5T) Hz\n",
...
#else
#if defined(CONFIG_OMAP34XX)
		if (in_width > (1024 << !*five_taps))
			goto loop;
#endif
#endif

		DSSVDBG("required fclk rate = %lu Hz\n", fclk);
		DSSVDBG("current fclk rate = %lu Hz\n", fclk_max);
...
loop:
		...
		if (x == max_x_decim && y == max_y_decim) {
			DSSDBG("failed to set up scaling %u*%u to %u*%u, "
		...
		enum omap_channel channel, u32 puv_addr)
{
	...
	/*if (cpu_is_omap44xx())*/ {
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND
		u32 fifo_size = dispc_get_plane_fifo_size(plane);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG
		/* for debugging */
		u8 hi_start, hi_end, lo_start, lo_end;
		u32 old_high, old_low;

		dss_feat_get_reg_field(FEAT_REG_FIFOHIGHTHRESHOLD, &hi_start, &hi_end);
		dss_feat_get_reg_field(FEAT_REG_FIFOLOWTHRESHOLD, &lo_start, &lo_end);
		old_high = REG_GET(DISPC_OVL_FIFO_THRESHOLD(plane), hi_start, hi_end);
		old_low = REG_GET(DISPC_OVL_FIFO_THRESHOLD(plane), lo_start, lo_end);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG */

#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND
		if (omap_dss_overlay_bw_requested()) {
			if (plane == OMAP_DSS_GFX) {
				fifo_low = 1504; /* 8-aligned */
				fifo_high = fifo_size - 1;
			} else {
				/* plane != OMAP_DSS_GFX */
				int max_decim = max(x_decim, y_decim);
#ifdef CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND
				if (max_decim > 2) {
#if 0
					...
					fifo_low = 1504; /* 8-aligned */
#endif
					fifo_high = (fifo_size << 1) - 1;
				} else
#endif /* CONFIG_OMAP4_DSS_FIFO_UNDERFLOW_WORKAROUND */
				if (max_decim > 1) {
					fifo_low = fifo_size - 8;
					fifo_high = fifo_size - 1;
				} else {
					fifo_low = 1680; /* 8-aligned */
					fifo_high = fifo_size - 1;
				}
			}
		} else {
...
#else /* !CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
		fifo_low = dispc_calculate_threshold(plane, paddr + offset0,
				   puv_addr + offset0, width, height,
				   row_inc, pix_inc);
...
#ifdef CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG
		/* for debugging */
		if (fifo_high != old_high || fifo_low != old_low) {
			DSSINFO("%c(%d): %d,%d %d*%dx%d*%d -> %dx%d\n",
				omap_dss_overlay_bw_requested() ? '*' : ' ',
				plane, pos_x, pos_y, width, x_decim,
				height, y_decim, out_width, out_height);
			DSSINFO("fifo(%d) low/high old %u/%u, new %u/%u\n",
				plane, old_low, old_high, fifo_low, fifo_high);
		}
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND_DEBUG */
...
static void dispc_enable_lcd_out(enum omap_channel channel, bool enable)
{
	...
	DSSDBG("dispc_enable_lcd_out %d: %d (%d)\n", channel, enable, is_on);
	...
	if (!enable && is_on) {
		...
		if (r)
			DSSDBG("failed to register FRAMEDONE isr\n");
	}
	...
	if (!enable && is_on) {
		...
		if (r)
			DSSDBG("failed to unregister FRAMEDONE isr\n");
	}
}
...
{
	if (channel == OMAP_DSS_CHANNEL_LCD2) {
		REG_FLD_MOD(DISPC_CONFIG2, 0x3, 7, 6);
	} else {
		REG_FLD_MOD(DISPC_CONFIG, 0x3, 7, 6);
	}
}
...
{
	if (channel == OMAP_DSS_CHANNEL_LCD2) {
		REG_FLD_MOD(DISPC_CONTROL2, mode, 31, 30);
...
#else
	DSSINFO("PCLK %uKHz, HSYNC %luHz, VSYNC %luHz\n", timings->pixel_clock, ht, vt);
#endif
}
...
void dispc_dump_clocks(void/*struct seq_file *s*/)
{
	...
	if (dss_has_feature(FEAT_CORE_CLK_DIV)) {
		printf("- DISPC-CORE-CLK -\n");
		l = dispc_read_reg(DISPC_DIVISOR);
		lcd = FLD_GET(l, 23, 16);

		printf("lck\t\t%-16lulck div\t%u\n",
...
#ifdef CONFIG_OMAP2_DSS_DEBUG_SUPPORT
void dispc_dump_regs(void)
{
	int i, o;
#define DUMPREG(r) printf("%-50s %08x\n", #r, dispc_read_reg(r))
		...
		bool ieo, bool ipc, bool ihs, bool ivs, u8 acbi, u8 acb)
{
	...
	DSSVDBG("onoff %d rf %d ieo %d ipc %d ihs %d ivs %d acbi %d acb %d\n",
...
static void dispc_error_worker(void/*struct work_struct *work*/)
{
#if 0
	int i;
#endif
	...
	if (errors & DISPC_IRQ_GFX_FIFO_UNDERFLOW) {
		...
		for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
			...
			if (ovl->id == 0) {
				...
				dss_feat_get_reg_field(FEAT_REG_FIFOHIGHTHRESHOLD, &hi_start, &hi_end);
				dss_feat_get_reg_field(FEAT_REG_FIFOLOWTHRESHOLD, &lo_start, &lo_end);
				DSSINFO("%cfifo(%d) low/high/size %u/%u/%u (UNDERFLOW)\n",
					omap_dss_overlay_bw_requested() ? '*' : ' ',
					OMAP_DSS_GFX,
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_GFX), lo_start, lo_end),
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_GFX), hi_start, hi_end),
					dispc_get_plane_fifo_size(OMAP_DSS_GFX));
				DSSINFO("(%d) pos x/y %u/%u; size w/h %u/%u\n",
					OMAP_DSS_GFX,
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_GFX), 10, 0),
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_GFX), 26, 16),
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_GFX), 10, 0) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_GFX), 26, 16) + 1);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
		...
		for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
			...
			if (ovl->id == 1) {
				...
				DSSINFO("%cfifo(%d) low/high/size %u/%u/%u (UNDERFLOW)\n",
					omap_dss_overlay_bw_requested() ? '*' : ' ',
					OMAP_DSS_VIDEO1,
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO1), lo_start, lo_end),
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO1), hi_start, hi_end),
					dispc_get_plane_fifo_size(OMAP_DSS_VIDEO1));
				DSSINFO("(%d) pos x/y %u/%u; size w/h %u/%u; out w/h %u/%u\n",
					OMAP_DSS_VIDEO1,
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO1), 10, 0),
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO1), 26, 16),
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO1), 10, 0) + 1,
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO1), 26, 16) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO1), 10, 0) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO1), 26, 16) + 1);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
				dispc_enable_plane(ovl->id, 0);
				dispc_go(ovl->manager->id);
				mdelay(50);
		...
		for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
			...
			if (ovl->id == 2) {
				...
				DSSINFO("%cfifo(%d) low/high/size %u/%u/%u (UNDERFLOW)\n",
					omap_dss_overlay_bw_requested() ? '*' : ' ',
					OMAP_DSS_VIDEO2,
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO2), lo_start, lo_end),
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO2), hi_start, hi_end),
					dispc_get_plane_fifo_size(OMAP_DSS_VIDEO2));
				DSSINFO("(%d) pos x/y %u/%u; size w/h %u/%u; out w/h %u/%u\n",
					OMAP_DSS_VIDEO2,
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO2), 10, 0),
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO2), 26, 16),
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO2), 10, 0) + 1,
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO2), 26, 16) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO2), 10, 0) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO2), 26, 16) + 1);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
				dispc_enable_plane(ovl->id, 0);
				dispc_go(ovl->manager->id);
				mdelay(50);
		...
		for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
			...
			if (ovl->id == 3) {
				...
				DSSINFO("%cfifo(%d) low/high/size %u/%u/%u (UNDERFLOW)\n",
					omap_dss_overlay_bw_requested() ? '*' : ' ',
					OMAP_DSS_VIDEO3,
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO3), lo_start, lo_end),
					REG_GET(DISPC_OVL_FIFO_THRESHOLD(OMAP_DSS_VIDEO3), hi_start, hi_end),
					dispc_get_plane_fifo_size(OMAP_DSS_VIDEO3));
				DSSINFO("(%d) pos x/y %u/%u; size w/h %u/%u; out w/h %u/%u\n",
					OMAP_DSS_VIDEO3,
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO3), 10, 0),
					REG_GET(DISPC_OVL_POSITION(OMAP_DSS_VIDEO3), 26, 16),
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO3), 10, 0) + 1,
					REG_GET(DISPC_OVL_PICTURE_SIZE(OMAP_DSS_VIDEO3), 26, 16) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO3), 10, 0) + 1,
					REG_GET(DISPC_OVL_SIZE(OMAP_DSS_VIDEO3), 26, 16) + 1);
#endif /* CONFIG_OMAP4_DSS_FIFO_WORKAROUND */
				dispc_enable_plane(ovl->id, 0);
				dispc_go(ovl->manager->id);
				mdelay(50);
		...
		unsigned long timeout)
{
	...
	return r;
#else
	return omap_dispc_wait_for_irq_timeout(irqmask, timeout);
#endif
}
...
static void _omap_dispc_initial_config(void)
{
	...
	if (dss_has_feature(FEAT_GLOBAL_MFLAG)) {
		...
		if (board_info && board_info->num_devices > 0) {
			struct omap_dss_device *dss_dev = NULL;

			if (board_info->default_device)
				dss_dev = board_info->default_device;
			else if (board_info->devices[0])
				dss_dev = board_info->devices[0];

			if (dss_dev) {
				sizex = dss_dev->panel.timings.x_res;
				sizey = dss_dev->panel.timings.y_res;
				tot_size = sizex * sizey;
			}
		}
#endif

		if (sizex && sizey && tot_size <= (1280 * 720)) {
			/* Disable the Display sub system global MFLAG */
			dispc_write_reg(DISPC_GLOBAL_MFLAG, 0);
			DSSINFO("disable global MFLAG (%dx%d)\n", sizex, sizey);
		} else {
			/* Enable the Display sub system global MFLAG*/
			dispc_write_reg(DISPC_GLOBAL_MFLAG, 2);
			DSSINFO("enable global MFLAG (%dx%d)\n", sizex, sizey);
		}
	}
#endif
}
...
/*#ifdef CONFIG_OMAP2_DSS_COLLECT_IRQ_STATS*/
	...
	dispc.base = (void __iomem *)DISPC_BASE;
#endif
...
{
	return omap_dispchw_probe(NULL);
}

void dispc_uninit_platform_driver(void)
{
	omap_dispchw_remove(NULL);
}
