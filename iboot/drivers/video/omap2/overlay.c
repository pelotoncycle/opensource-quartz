...
#else
static int num_overlays __attribute__ ((section (".data")));
static struct list_head overlay_list __attribute__ ((section (".data")));
#endif
...
int dss_check_overlay(struct omap_overlay *ovl, struct omap_dss_device *dssdev)
{
	...
	DSSVDBG("check_overlay %d: (%d,%d %dx%d -> %dx%d) disp (%dx%d)\n",
			ovl->id,
...
void dss_init_overlays(struct platform_device *pdev)
{
	...
	for (i = 0; i < dss_feat_get_num_ovls(); ++i) {
...
#else
		ovl = memalign(ARCH_DMA_MINALIGN, sizeof(*ovl));
		if (ovl)
		...
		ovl->info.max_x_decim = ovl->info.max_y_decim = 16;
#else
		ovl->info.max_x_decim = ovl->info.max_y_decim = 1;
#endif
#endif
...
void dss_recheck_connections(struct omap_dss_device *dssdev, bool force)
{
	...
	DSSVDBG("recheck_connections: dev %s, channel %d, force %d\n",
			dssdev->name, dssdev->channel, force);
