...
/*#define VERBOSE_IRQ*/
/* James Wu: we still don't support DSI Command mode & TE */
...
#else
static struct dsi_data dsi_data[MAX_NUM_DSI] __attribute__ ((section (".data")));
static struct platform_device dsi_pdev_map[MAX_NUM_DSI] = {
	[0] = { .id = 0, },
	[1] = { .id = 1, },
};
static const void __iomem *dsi_base[MAX_NUM_DSI] = {
	[0] = (void __iomem *)DSI1_BASE,
	[1] = (void __iomem *)DSI2_BASE,
};
#endif
...
#else
static inline struct dsi_data *dsi_get_dsidrv_data(struct platform_device *dsidev) __attribute__((always_inline));
static inline struct dsi_data *dsi_get_dsidrv_data(struct platform_device *dsidev)
{
	return &(dsi_data[dsidev->id]);
}

static inline struct platform_device *dsi_get_dsidev_from_dssdev(struct omap_dss_device *dssdev) __attribute__((always_inline));
static inline struct platform_device *dsi_get_dsidev_from_dssdev(struct omap_dss_device *dssdev)
{
	return &(dsi_pdev_map[dssdev->phy.dsi.module]);
}
...
#else
static inline int dsi_get_dsidev_id(struct platform_device *dsidev) __attribute__((always_inline));
...
static void dsi_perf_show(struct platform_device *dsidev, const char *name)
{
	...
	printf(KERN_INFO "DSI(%s): %u us + %u us = %u us (%uHz), "
...
static inline void print_irq_status(u32 status) {}
static inline void print_irq_status_vc(int channel, u32 status) {}
static inline void print_irq_status_cio(u32 status) {}
 ...
 * Returns 0: timedout
 */
static int wait_for_completion_timeout(struct platform_device *dsidev,
		struct completion *x, ulong timeout_ms)
{
	ulong start;
...
static int dsi_clk_use_count = 0;

static void dsi_enable_clocks(void)
{
	if (dsi_clk_use_count++ == 0) {
...
#else
#error "dsi_enable_clocks() not implemented\n"
#endif /* CONFIG_OMAP44XX */

	}

	DSSVDBG("dsi_clk_use_count++ = %d\n", dsi_clk_use_count);
}

static void dsi_disable_clocks(void)
{
	if (--dsi_clk_use_count == 0) {
		...
		DSSVDBG("%s: CM_DSS_CLKSTCTRL 0x%08x, CM_DSS_DSS_CLKCTRL 0x%08x\n", __func__,
				__raw_readl(&prcm->cm_dss_clkstctrl), __raw_readl(&prcm->cm_dss_dss_clkctrl));
#else
#error "dsi_disable_clocks() not implemented\n"
#endif /* CONFIG_OMAP44XX */
...
static void _dsi_print_reset_status(struct platform_device *dsidev)
{
	...
	printf(KERN_DEBUG "DSI resets: ");

	l = dsi_read_reg(dsidev, DSI_PLL_STATUS);
	printf("PLL (%d) ", FLD_GET(l, 0, 0));

	l = dsi_read_reg(dsidev, DSI_COMPLEXIO_CFG1);
	printf("CIO (%d) ", FLD_GET(l, 29, 29));
...
static unsigned long dsi_get_sys_clk_rate(void)
{
#if defined(CONFIG_OMAP44XX)
	return get_sys_clk_freq();
#else
#error "dsi_get_sys_clk_rate() not implemented"
	return 0;
#endif
}
		...
		struct dsi_clock_info *cinfo)
{
...
#else
		cinfo->clkin = dsi_get_sys_clk_rate();
#endif
	...
	DSSVDBG("clkin %lu, fint %lu\n", cinfo->clkin, cinfo->fint);

	if (cinfo->fint > dsi->fint_max || cinfo->fint < dsi->fint_min)
		return -EINVAL;

	cinfo->clkin4ddr = 2 * cinfo->regm * cinfo->fint;
	DSSVDBG("clkin4ddr %lu\n", cinfo->clkin4ddr);
		...
		struct dispc_clock_info *dispc_cinfo)
{
	...
	if (req_pck == dsi->cache_req_pck &&
		...
		DSSVDBG("DSI clock info found from cache\n");
	...
	if (dispc_cinfo->min_fck_per_pck > 0) {
		min_fck_per_pck = dispc_cinfo->min_fck_per_pck;
		DSSDBG("min_fck_per_pck %d\n", min_fck_per_pck);
	} else
		...
		struct dsi_clock_info *cinfo)
{
	...
	DSSVDBG("clkin (%s) rate %ld, highfreq %d\n",
			cinfo->use_sys_clk ? "dss_sys_clk" : "pclkfree",
			cinfo->clkin,
			cinfo->highfreq);

	/* DSIPHY == CLKIN4DDR */
	DSSVDBG("CLKIN4DDR = 2 * %d / %d * %lu / %d = %lu\n",
			cinfo->regm,
			cinfo->regn,
			cinfo->clkin,
			cinfo->highfreq + 1,
	...
	DSSVDBG("Data rate on 1 DSI lane %ld Mbps\n",
			cinfo->clkin4ddr / 1000 / 1000 / 2);

	DSSVDBG("Clock lane freq %ld Hz\n", cinfo->clkin4ddr / 4);

	DSSVDBG("regm_dispc = %d, %s (%s) = %lu\n", cinfo->regm_dispc,
		dss_get_generic_clk_source_name(OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC),
		dss_feat_get_clk_source_name(OMAP_DSS_CLK_SRC_DSI_PLL_HSDIV_DISPC),
		cinfo->dsi_pll_hsdiv_dispc_clk);
	DSSVDBG("regm_dsi = %d, %s (%s) = %lu\n", cinfo->regm_dsi,
		...
		bool enable_hsdiv)
{
	...
	if (!board_info->dss_platform_enable) {
		DSSERR("can't get dss_platform_enable() to enable VDDS_DSI regulator\n");
		return -EINVAL;
	}
#endif
		...
		struct seq_file *s*/)
{
	...
	printf(	"dsi pll source = %s\n",
			cinfo->use_sys_clk ? "dss_sys_clk" : "pclkfree");

	printf(	"Fint\t\t%-16luregn %u\n", cinfo->fint, cinfo->regn);

	printf(	"CLKIN4DDR\t%-16luregm %u\n",
			cinfo->clkin4ddr, cinfo->regm);

	printf(	"%s (%s)\t%-16luregm_dispc %u\t(%s)\n",
...
void dsi_dump_clocks(void/*struct seq_file *s*/)
{
	...
	for  (i = 0; i < MAX_NUM_DSI; i++) {
		...
		if (dsidev && dsi->pdev == dsidev && dsi->base)
			dsi_dump_dsidev_clocks(dsidev);
		else
			printf("- DSI%d driver is not registered -\n", i+1);
	}
#endif
}
#endif /* CONFIG_OMAP2_DSS_DEBUG_SUPPORT */
...
static void dsi_dump_dsidev_regs(struct platform_device *dsidev/*,
		struct seq_file *s*/)
{
#define DUMPREG(r) printf("%-35s %08x\n", #r, dsi_read_reg(dsidev, r))
...
/*static*/ void dsi1_dump_regs(void/*struct seq_file *s*/)
{
#if 0
	struct platform_device *dsidev = dsi_get_dsidev_from_id(0);
	...
	if (dsidev && dsi->pdev == dsidev && dsi->base)
		...
		DSSERR("DSI1 driver is not registered\n");
#endif
}

/*static*/ void dsi2_dump_regs(void/*struct seq_file *s*/)
{
#if 0
	struct platform_device *dsidev = dsi_get_dsidev_from_id(1);
	...
	if (dsidev && dsi->pdev == dsidev && dsi->base)
		...
		DSSERR("DSI2 driver is not registered\n");
#endif
}
#endif /* CONFIG_OMAP2_DSS_DEBUG_SUPPORT */
...
{
	struct dsi_data *dsi = dsi_get_dsidrv_data(dsidev);
	DSSDBG("ddr_clk = %lu Hz, 1ddr = %u.X ns, 2ddr = %u.X ns\n",
		dsi->current_cinfo.clkin4ddr / 4, ddr2ns(dsidev, 1), ddr2ns(dsidev, 2));
}
#endif
	...
	DSSDBG("DSI_DSIPHY_CFG0: ths_prepare %u; ths_prepare_ths_zero %u; ths_trail %u; ths_exit %u\n",
		FLD_GET(r, 31, 24), FLD_GET(r, 23, 16), FLD_GET(r, 15, 8), FLD_GET(r, 7, 0));
	...
	DSSDBG("DSI_DSIPHY_CFG1: tlpx_half %u; tclk_trail %u; tclk_zero %u\n",
		FLD_GET(r, 22, 16), FLD_GET(r, 15, 8), FLD_GET(r, 7, 0));

	r = dsi_read_reg(dsidev, DSI_DSIPHY_CFG2);
	r = FLD_MOD(r, tclk_prepare, 7, 0);
	dsi_write_reg(dsidev, DSI_DSIPHY_CFG2, r);
	DSSDBG("DSI_DSIPHY_CFG2: tclk_prepare %u\n",
		FLD_GET(r, 7, 0));
}
...
static int dsi_cio_init(struct omap_dss_device *dssdev)
{
	...
	/*if (cpu_is_omap44xx())*/ {
		...
		DSSDBG("DSI_CLK_CTRL: 0x%04x; DDR_CLK_ALWAYS_ON %u; HS_AUTO_STOP_ENABLE %u\n",
			r, FLD_GET(r, 13, 13), FLD_GET(r, 18, 18));
#endif
	}
#endif
...
static int dsi_sync_vc_l4(struct platform_device *dsidev, int channel)
{
	...
	if (REG_GET(dsidev, DSI_VC_CTRL(channel), 5, 5)) {
		if (wait_for_completion_timeout(dsidev, &completion,
				10/*msecs_to_jiffies(10)*/) == 0) {
...
static int dsi_vc_enable(struct platform_device *dsidev, int channel,
		bool enable)
{
	DSSVDBG("dsi_vc_enable channel %d, enable %d\n",
			channel, enable);
...
static void dsi_vc_flush_long_data(struct platform_device *dsidev, int channel)
{
	while (REG_GET(dsidev, DSI_VC_CTRL(channel), 20, 20)) {
		...
		DSSVDBG("\t\tb1 %#02x b2 %#02x b3 %#02x b4 %#02x\n",
				(val >> 0) & 0xff,
				(val >> 8) & 0xff,
				(val >> 16) & 0xff,
				(val >> 24) & 0xff);
		...
		u8 data_type, u8 *data, u16 len, u8 ecc)
{
	...
	DSSVDBG("dsi_vc_send_long, %d bytes\n", len);
	...
	if (i) {
		...
		DSSVDBG("\tsending remainder bytes %d\n", i);
		...
		u8 data_type, u16 data, u8 ecc)
{
	...
	if (dsi->debug_write)
#endif
		DSSVDBG("dsi_vc_send_short(ch%d, dt %#x, b1 %#x, b2 %#x)\n",
				channel,
		...
		u8 *buf, int buflen)
{
	...
	if (dt == DSI_DT_RX_ACK_WITH_ERR) {
		...
		if (dsi->debug_read)
#endif
			DSSVDBG("\tDCS short response, 1 byte: %02x\n", data);
		...
		if (dsi->debug_read)
#endif
			DSSVDBG("\tDCS short response, 2 byte: %04x\n", data);
		...
		if (dsi->debug_read)
#endif
			DSSVDBG("\tDCS long response, len %d\n", len);
...
static int dsi_video_proto_config(struct omap_dss_device *dssdev)
{
...
#ifdef CONFIG_OMAP2_DSS_DEBUG_SUPPORT
	r = dsi_read_reg(dsidev, DSI_CTRL);
	DSSDBG("VP_DATA_BUS_WIDTH %u\n", FLD_GET(r, 7, 6));
	DSSDBG("LINE_BUFFER %u\n", FLD_GET(r, 13, 12));
	DSSDBG("VP_DE_POL %u; VP_HSYNC_POL %u; VP_VSYNC_POL %u\n",
		FLD_GET(r, 9, 9), FLD_GET(r, 10, 10), FLD_GET(r, 11, 11));
	DSSDBG("VP_VSYNC_START %u; VP_VSYNC_END %u; VP_HSYNC_START %u; VP_HSYNC_END %u\n",
		FLD_GET(r, 15, 15), FLD_GET(r, 16, 16), FLD_GET(r, 17, 17), FLD_GET(r, 18, 18));
	DSSDBG("BLANKING_MODE %u; HFP_BLANKING %u; HBP_BLANKING %u; HSA_BLANKING %u\n",
		FLD_GET(r, 20, 20), FLD_GET(r, 21, 21), FLD_GET(r, 22, 22), FLD_GET(r, 23, 23));
#endif
	...
	if (dssdev->dsi_timings) {
		...
		WARN((line * dssdev->ctrl.pixel_size / 8) % lanes != 0,
		     ...
		     "parameters");
#else
		if ((line * dssdev->ctrl.pixel_size / 8) % lanes != 0) {
		     DSSWARN("TL should be an exact "
		     "integer, try changing DISPC horizontal blanking "
		     "parameters\n");
		}
#endif
	...
	DSSDBG("DSI_VM_TIMING1: HBP %u; HFP %u; HSA %u\n",
		FLD_GET(r, 11, 0), FLD_GET(r, 23, 12), FLD_GET(r, 31, 24));
	...
	DSSDBG("DSI_VM_TIMING2: VBP %u; VFP %u; VSA %u; WINDOW_SYNC %u\n",
		FLD_GET(r, 7, 0), FLD_GET(r, 15, 8), FLD_GET(r, 23, 16), FLD_GET(r, 27, 24));

	r = dsi_read_reg(dsidev, DSI_VM_TIMING3);
	r = FLD_MOD(r, t.vact, 14, 0);		/* Active lines */
	r = FLD_MOD(r, t.tl, 31, 16);		/* Total line length */
	dsi_write_reg(dsidev, DSI_VM_TIMING3, r);
	DSSDBG("DSI_VM_TIMING3: VACT %u; TL %u\n",
		FLD_GET(r, 14, 0), FLD_GET(r, 31, 16));
	...
	DSSDBG("DSI_VM_TIMING4: hsa_interleave_hs %u; hfp_interleave_hs %u; hbp_interleave_hs %u\n",
		FLD_GET(r, 23, 16), FLD_GET(r, 15, 8), FLD_GET(r, 7, 0));
	...
	DSSDBG("DSI_VM_TIMING5: hsa_interleave_lp %u; hfp_interleave_lp %u; hbp_interleave_lp %u\n",
		FLD_GET(r, 23, 16), FLD_GET(r, 15, 8), FLD_GET(r, 7, 0));

	r = FLD_VAL(t.bl_hs_int, 31, 16) |	/* BL_HS_INTERLEAVING */
		FLD_VAL(t.bl_lp_int, 15, 0);	/* BL_LP_INTERLEAVING */
	dsi_write_reg(dsidev, DSI_VM_TIMING6, r);
	DSSDBG("DSI_VM_TIMING6: bl_interleave_hs %u; bl_interleave_lp %u\n",
		FLD_GET(r, 31, 16), FLD_GET(r, 15, 0));

	r = FLD_VAL(t.enter_lat, 31, 16) |	/* ENTER_HS_MODE_LATENCY */
		FLD_VAL(t.exit_lat, 15, 0);	/* EXIT_HS_MODE_LATENCY */
	dsi_write_reg(dsidev, DSI_VM_TIMING7, r);
	DSSDBG("DSI_VM_TIMING7: enter_hs_mode_lat %u; exit_hs_mode_lat %u\n",
			FLD_GET(r, 31, 16), FLD_GET(r, 15, 0));
...
static void dsi_proto_timings(struct omap_dss_device *dssdev)
{
	...
	DSSDBG("ddr_clk_pre %u, ddr_clk_post %u (offset_ddr_clk %u)\n",
			ddr_clk_pre,
			ddr_clk_post, offset_ddr_clk);
			...
			int x, int y, int w, int h)
{
	...
	DSSVDBG("dsi_update_screen_l4 (%d,%d %dx%d)\n",
			x, y, w, h);
	...
	DSSVDBG("max_pixels_per_packet %d\n", max_pixels_per_packet);

	pixels_left = w * h;

	DSSVDBG("total pixels %d\n", pixels_left);
		...
		u16 x, u16 y, u16 w, u16 h)
{
	...
	DSSVDBG("dsi_update_screen_dispc(%d,%d %dx%d)\n",
			x, y, w, h);
...
static void dsi_framedone_timeout_work_callback(void/*struct work_struct *work*/)
{
...
int dsi_init_display(struct omap_dss_device *dssdev)
{
	...
	if (dsi_get_num_data_lanes_dssdev(dssdev) > dsi->num_data_lanes) {
		DSSDBG("DSI%d can't support more than %d data lanes\n",
...
#else
	dsi = &(dsi_data[dsi_module]);
	memset(dsi, 0, sizeof(*dsi));

	dsi->pdev = dsidev;

	board_info = omap_dss_get_board_info();
	dsi->dsi_mux_pads = board_info->dsi_mux_pads;
#endif
...
#else
	dsi->base = (void __iomem *)dsi_base[dsi_module];
#endif
...
#else
	return omap_dsi1hw_probe(&(dsi_pdev_map[0]));
#endif
}
...
#else
	omap_dsi1hw_remove(&(dsi_pdev_map[0]));
#endif
}
