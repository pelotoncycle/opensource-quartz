...
#define	PRCM_DEBUG_DUMP_REG_ADDRESS
...
static u32 omap4_prminst_read_inst_reg(u8 part, s16 inst, u16 idx)
{
...
#ifdef PRCM_DEBUG_DUMP_REG_ADDRESS
static u32 omap4_cminst_get_inst_addr(u8 part, s16 inst, u16 idx)
{
	BUG_ON(part >= OMAP4_MAX_PRCM_PARTITIONS ||
	       part == OMAP4430_INVALID_PRCM_PARTITION ||
	       !_cm_bases[part]);
	return OMAP2_L4_IO_ADDRESS(_cm_bases[part] + inst + idx);
}
#endif /* PRCM_DEBUG_DUMP_REG_ADDRESS */

/* Read a register in a CM instance */
static u32 omap4_cminst_read_inst_reg(u8 part, s16 inst, u16 idx)
{
	BUG_ON(part >= OMAP4_MAX_PRCM_PARTITIONS ||
	       part == OMAP4430_INVALID_PRCM_PARTITION ||
	       !_cm_bases[part]);
...
#define d_pr(fmt, args...)				\
	{						\
		printf(fmt , ## args);		\
	}

#define d_pr_ctd(fmt, args...)			\
	{						\
		printf(fmt , ## args);		\
	}

static void prcmdebug_dump_dpll(struct d_dpll_info *dpll,
				int flags)
{
...
static void prcmdebug_dump_intgen(struct d_intgen_info *intgen,
				  int flags)
{
...
static void prcmdebug_dump_mod(struct d_mod_info *mod,
	int flags)
{
...
#ifdef PRCM_DEBUG_DUMP_REG_ADDRESS
	d_pr("         %s(0x%08x=0x%08x)", mod->name, mod->clkctrl, clkctrl);
#else
	d_pr("         %s(0x%08x)", mod->name, clkctrl);
#endif /* PRCM_DEBUG_DUMP_REG_ADDRESS */
	...
	if (optclk)
		d_pr_ctd(" optclk=0x%x", optclk);

	d_pr_ctd("\n");
}

static void prcmdebug_dump_real_cd(struct d_clkd_info *cd,
	int flags)
{
	u32 clktrctrl =
...
#ifdef PRCM_DEBUG_DUMP_REG_ADDRESS
	d_pr("      %s(0x%08x=0x%08x) mode=%s", cd->name,
		omap4_cminst_get_inst_addr(cd->prcm_partition, cd->cm_inst, cd->clkdm_offs + OMAP4_CM_CLKSTCTRL),
		clktrctrl, cmtrctrl_s[mode]);
#else
	d_pr("      %s(0x%08x) mode=%s", cd->name, clktrctrl, cmtrctrl_s[mode]);
#endif /* PRCM_DEBUG_DUMP_REG_ADDRESS */

	d_pr_ctd(" activity=0x%x", activity);
...
#if 0
	if (dynamicdep)
		d_pr_ctd(" dynamicdep=0x%x", dynamicdep);
#endif

	d_pr_ctd("\n");
}

static void prcmdebug_dump_cd(struct d_clkd_info *cd,
	int flags)
{
	...
	if (cd->cm_inst != -1) {
		prcmdebug_dump_real_cd(cd, flags);
	...
	while (*dpll) {
		prcmdebug_dump_dpll(*dpll, flags);
		dpll++;
	}
	...
	while (*intgen) {
		prcmdebug_dump_intgen(*intgen, flags);
		intgen++;
	}
}

static void prcmdebug_dump_pd(struct d_pwrd_info *pd,
	int flags)
{
...
/* prcm */
static int do_dump_prcm_regs(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	prcmdebug_dump(0);
	return 0;
}
...
U_BOOT_CMD(
	prcm, 1, 0,	do_dump_prcm_regs,
	"Dump OMAP PRCM registers",
	""
);
...
/* prcmon */
static int do_dump_prcm_on_regs(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
U_BOOT_CMD(
	prcmon, 1, 0,	do_dump_prcm_on_regs,
	"Dump OMAP PRCM ON registers",
	""
);
