...
void omap_clk_enable(u32 *fclk_addr, u32 *iclk_addr, u32 *idlest_addr, u32 bit)
{
	u32	value;

	/* Enable clocks */
	setbits_le32(fclk_addr, (0x01 << bit));
	value = __raw_readl(fclk_addr); /* OCP barrier */
	setbits_le32(iclk_addr, (0x01 << bit));
	value = __raw_readl(iclk_addr); /* OCP barrier */

	/* wait for ready */
	wait_on_value((0x01 << bit), 0, idlest_addr, LDELAY);
}

void omap_clk_disable(u32 *fclk_addr, u32 *iclk_addr, u32 bit)
{
	/* Disable clocks */
	clrbits_le32(fclk_addr, (0x01 << bit));
	clrbits_le32(iclk_addr, (0x01 << bit));
