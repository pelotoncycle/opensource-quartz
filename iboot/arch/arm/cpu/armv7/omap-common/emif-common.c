...
static int warmreset_mode __attribute__ ((section (".data"))) = 0;

#if 0
void set_lpmode_selfrefresh(u32 base)
#else
static void set_lpmode_selfrefresh(u32 base)
#endif
{
...
void force_emif_self_refresh()
{
	...
	/* Don't enable WARM RESET mode if emif_pwr_mgmt_ctrl is not configured. */
	struct emif_reg_struct *emif1 = (struct emif_reg_struct *)EMIF1_BASE;
	if (readl(&emif1->emif_pwr_mgmt_ctrl))
		warmreset_mode = 1;
#endif
...
#ifdef DEBUG_MIN_2
	printf("g_mr:%u.%u,0x%08x=0x%x\n", emif_num(base), cs, mr_addr, mr);
#else
...
#ifdef DEBUG_MIN_2
	printf("s_mr:%u.%u,0x%08x=0x%x\n", emif_num(base), cs, mr_addr, mr_val);
#else
	debug("set_mr: EMIF%d cs %d mr %08x val 0x%x\n", emif_num(base),
	      cs, mr_addr, mr_val);
#endif
...
#else
static void do_lpddr2_init(u32 base, u32 cs, const struct emif_regs *regs)
#endif
{
#if 0
	u32 mr_addr;
...
#else
		if (regs->freq <= 200000000)
			set_mr(base, cs, LPDDR2_MR1, MR1_BL_8_BT_SEQ_WRAP_EN_NWR_3);
		else if (regs->freq <= 400000000)
			set_mr(base, cs, LPDDR2_MR1, MR1_BL_8_BT_SEQ_WRAP_EN_NWR_6);
		else
#if 0
			set_mr(base, cs, LPDDR2_MR1, MR1_BL_8_BT_SEQ_WRAP_EN_NWR_7);
#else
			/* Values are from TI (Kobo Zeus) */
			set_mr(base, cs, LPDDR2_MR1, MR1_BL_8_BT_SEQ_WRAP_EN_NWR_6);
#endif
#endif
...
#else
	/* Encoding of RL in MR2 is (RL - 2) */
	set_mr(base, cs, LPDDR2_MR2, regs->RL - 2);

	/* Enable refresh along with writing MR16 */
	set_mr(base, cs, LPDDR2_MR16 | EMIF_REG_REFRESH_EN_MASK, MR16_REF_FULL_ARRAY);
#endif
...
#else
	do_lpddr2_init(base, CS0, regs);
	if (regs->sdram_config & EMIF_REG_EBANK_MASK)
		do_lpddr2_init(base, CS1, regs);
#endif
...
#endif
{
	...
	/* Reading back the last written register to ensure all writes are complete */
	(void)readl(&emif->emif_ddr_phy_ctrl_1_shdw);
	...
	/* Reading back the last written register to ensure all writes are complete */
	(void)readl(&emif->emif_l3_config);
}
...
#ifdef DEBUG_MIN_1
#define print_timing_reg(reg) printf(#reg"=0x%08x\n", (regs->reg))
#else
				...
				u8 RL)
{
	...
	/* Values are from TI (X-Loader) */
	/**
	 * REG_DDR2_DDQS:
	 * This bit is only for DDR2 mode, because the device
	 * supports LPDDR2; this bit is don’t care.
	 */
	config_reg |= 1 << EMIF_REG_DDR2_DDQS_SHIFT;
...
#if defined(CONFIG_OMAP44XX)
	/** OMAP4 TRM EMIF Timing Initialization:
	 * For a 400MHz DDR clock and tDQSCKMAX=5.5ns, the recommended setting is 0x2.
	 * For a 466MHz DDR clock and tDQSCKMAX=5.5ns, the recommended setting is 0x3.
	 */
	if (freq > 400000000)
		val = 0x3;
...
#else
static u32 get_ddr_phy_ctrl_1(const struct lpddr2_device_details *cs1_device,
				u32 freq, u8 RL)
{
	/* Refer to X-Loader sdram_elpida.c for the best configurations */
	u32 phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_2;
	u32 val = EMIF_DLL_SLAVE_DLY_CTRL_466_MHZ;
	u32 omap_rev = omap_revision();

	/* OMAP4470 TRM:
	 * For a 400 MHz DDR clock, tDQSCKMAX=5.5ns, and RL = 6,
	 * the recommended setting for REG_READ_LATENCY is 0x9.
	 * For a 466 MHz DDR clock, tDQSCKMAX=5.5ns, and RL =7,
	 * the recommended setting for REG_READ_LATENCY is 0xB.
	 */
	if (RL == 6)
		RL = 0x09;
	else if (RL == 7)
	...
	if (freq <= 100000000) {
		/* Other fields are constant magic values. Hardcode them together */
		phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_2;
		val = EMIF_DLL_SLAVE_DLY_CTRL_100_MHZ_AND_LESS;
	} else if (freq <= 200000000) {
		/* Other fields are constant magic values. Hardcode them together */
		if (omap_rev >= OMAP4470_ES1_0) {
			...
			if (cs1_device)
				phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_0;
			else
				phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_1;
			val = EMIF_DLL_SLAVE_DLY_CTRL_200_MHZ;
		}
		...
		if (omap_rev >= OMAP4470_ES1_0) {
			phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_2;
			/* OMAP4470 TRM:
			 * For the typical SDRAM frequency of 400 or 466 MHz and
			 * DLL reference clock frequency of 100 or 116.5 MHz, this
			 * number must be set to 0x37.
			 */
			...
			if (cs1_device)
				...
				phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_1;
			/* OMAP4430/4460 TRM:
			 * For the typical SDRAM frequency of 400 MHz and DLL
			 * reference clock frequency of 100 MHz, this number must
			 * be set to 0x40 (1/4-th of 0xFF).
			 */
			val = EMIF_DLL_SLAVE_DLY_CTRL_400_MHZ;
		}
	} else {
		...
		phy = EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_2;
		/*
		 * Some instability with OMAP4470 when running the DDR at 466MHz is
		 * discovered. This instability is resolved with updated LPDDR settings:
		 *
		 * - DLL delay should be set to 461ps:
		 * DDR_PHY_CTRL_1_SHDW[11:4] = 0x37
		 *
		 * - Read latency should be set to 0xB:
		 * DDR_PHY_CTRL_1_SHDW[3:0] = 0xB
		 *
		 * - Slew Rate should be set to “FASTEST” and Impedance Control to “Drv12”:
	...
	phy |= RL << EMIF_REG_READ_LATENCY_SHIFT;
	phy |= val << EMIF_REG_DLL_SLAVE_DLY_CTRL_SHIFT;

#ifdef DEBUG_MIN_1
	printf("phy_ctrl_1: f %u, RL %d, 0x%08x\n", freq, RL, phy);
#else
	debug("get_ddr_phy_ctrl_1: freq %u, RL %d, phy 0x%08x\n", freq, RL, phy);
#endif
		...
		u32 freq, struct emif_regs *regs)
{
	...
	emif_assert(freq <= MAX_LPDDR2_FREQ);
	debug("emif_calculate_regs: freq %u\n", freq);
	...
	regs->sdram_config = get_sdram_config_reg(cs0_dev_details,
						...
						addressing, RL_FINAL);
#else
	regs->sdram_config = (regs->sdram_config_init & ~EMIF_REG_CL_MASK) |
			(timings->RL << EMIF_REG_CL_SHIFT);
#endif
...
#else
	regs->sdram_tim3 = get_sdram_tim_3_reg(freq, timings, min_tck, addressing);
#endif
...
#else
	regs->emif_ddr_phy_ctlr_1_init =
			get_ddr_phy_ctrl_1(NULL/*ignored*/, sys_freq / 2, RL_BOOT);

	regs->emif_ddr_phy_ctlr_1 =
			get_ddr_phy_ctrl_1(cs1_dev_details, freq, timings->RL);
#endif
...
#else
	print_timing_reg(sdram_config_init);
	print_timing_reg(sdram_config);
	print_timing_reg(ref_ctrl);
	print_timing_reg(sdram_tim1);
	print_timing_reg(sdram_tim2);
	print_timing_reg(sdram_tim3);
	print_timing_reg(read_idle_ctrl);
	print_timing_reg(temp_alert_config);
	print_timing_reg(zq_config);
	print_timing_reg(emif_ddr_phy_ctlr_1);
	print_timing_reg(emif_ddr_phy_ctlr_1_init);
#endif
}
...
#else
#ifdef DEBUG_MIN_2
		get_mr(base, cs, LPDDR2_MR4);
		get_mr(base, cs, LPDDR2_MR5);
		get_mr(base, cs, LPDDR2_MR6);
		get_mr(base, cs, LPDDR2_MR7);
		get_mr(base, cs, LPDDR2_MR8);
#endif
		goto bad_mr;
#endif
	}
			...
			struct lpddr2_device_details *lpddr2_dev_details)
{
	...
	if (!running_from_sdram()) {
		if (warmreset_mode) {
			/* DO NOT reconfigure EMIF phy for LPDDR2 SDRAM detection in WARM RESET */
			phy = readl(&emif->emif_ddr_phy_ctrl_1);
			if (emif_nr == 1 && !cs)
				printf("emif phy: 0x%x\n", phy);
			if ((phy & EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_0) !=
					EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_0) {
				phy = get_ddr_phy_ctrl_1(NULL/*ignored*/, omap_ddr_clk(), RL_BOOT);
				writel(phy, &emif->emif_ddr_phy_ctrl_1);
			}
		} else {
			phy = get_ddr_phy_ctrl_1(NULL/*ignored*/, get_sys_clk_freq() / 2, RL_BOOT);
			writel(phy, &emif->emif_ddr_phy_ctrl_1);
		}
	}
...
#else
	/* We must reconfigure LPDDR2 in WARM RESET to avoid EMIF states lost */
	if (!(in_sdram /*|| warmreset_mode*/))
		lpddr2_init(base, regs);
#endif
...
void emif_post_init_config(u32 base)
{
	...
	debug(">>emif_post_init_config() %x\n", base);

#if 0
	/* Check for DDR PHY ready for EMIF */
	while ((readl(&emif->emif_status) & EMIF_REG_PHY_DLL_READY_MASK)
			!= EMIF_REG_PHY_DLL_READY_MASK)
		;
#endif
...
void dmm_init(u32 base)
{
	...
	debug("dmm_lisa_map_0 0x%08X\n", lisa_map_regs->dmm_lisa_map_0);
	debug("dmm_lisa_map_1 0x%08X\n", lisa_map_regs->dmm_lisa_map_1);
	debug("dmm_lisa_map_2 0x%08X\n", lisa_map_regs->dmm_lisa_map_2);
	debug("dmm_lisa_map_3 0x%08X\n", lisa_map_regs->dmm_lisa_map_3);
...
void sdram_init(void)
{
...
#else
	if (!in_sdram) {
		/* Errata i728: System May Hang During EMIF Frequency Change */
		struct emif_reg_struct *emif = (struct emif_reg_struct *)EMIF1_BASE;
		/* Force no EMIF IDLE to avoid system hung */
		writel(0x0, &emif->emif_pwr_mgmt_ctrl);
		emif = (struct emif_reg_struct *)EMIF2_BASE;
		writel(0x0, &emif->emif_pwr_mgmt_ctrl);
	}
	...
	if (!in_sdram) {
#if !defined(CONFIG_SYS_EMIF_PRECALCULATED_TIMING_REGS) && defined(CONFIG_OMAP44XX)
		if (!emif_sizes[0] && !emif_sizes[1])
			panic("SDRAM size 0 ERR\n");
#endif
...
#else
		if (size_detect != size_prog) {
#if defined(CONFIG_SYS_SDRAM_EXPECTED_SIZE) && (CONFIG_SYS_SDRAM_EXPECTED_SIZE > 0)
			panic("SDRAM size ERR: 0x%x<->0x%x (0x%x)\n",
				size_detect, size_prog, CONFIG_SYS_SDRAM_EXPECTED_SIZE);
		} else if (size_detect != CONFIG_SYS_SDRAM_EXPECTED_SIZE) {
			panic("SDRAM size ERR: 0x%x<->0x%x\n",
				size_detect, CONFIG_SYS_SDRAM_EXPECTED_SIZE);
#else
