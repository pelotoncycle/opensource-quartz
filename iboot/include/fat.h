...
/* James Wu: it's unnecessary to support VFAT on InnoComm devices */
...
#else
#define CHECK_CLUST(x, fatsize) ((x) <= 1 || \
	(x) >= ((fatsize) != 32 ? ((fatsize) != 16 ? 0x0ff0 : 0xfff0) : 0x0ffffff0))
#define CHECK_EOF(x, fatsize) \
	((x) >= ((fatsize) != 32 ? ((fatsize) != 16 ? 0x0ff8 : 0xfff8) : 0x0ffffff8))
#endif
...
} fsdata __attribute__ ((aligned(4)));
...
typedef struct fat_read_info {
	fsdata		*mydata;
	dir_entry	*dentptr;
	__u32		curr_cluster;		/* current cluster */
	uint32_t	curr_clust_offset;	/* current file offset in bytes at the 'curr_cluster' */
} fat_read_info;
