...
#if !defined(CONFIG_SPL_BUILD) && defined(CONFIG_SYS_MEMORY_POST)
 ...
 */
#if 0
#undef	INJECT_DATABUS_ERRORS
#undef	INJECT_DATA_ERRORS
#undef	INJECT_ADDRESS_ERRORS
#endif

#ifdef INJECT_DATABUS_ERRORS
#warning "Injecting data bus errors for testing purposes"
#endif
...
static memory_post_log_t post_log;

/*-------------------------------------------------------------------------*/

static void __arch_memory_post_prepare(memory_post_log_t log) {}
void arch_memory_post_prepare(memory_post_log_t log)
	__attribute__((weak, alias("__arch_memory_post_prepare")));

static void __arch_memory_post_cleanup(memory_post_log_t log) {}
void arch_memory_post_cleanup(memory_post_log_t log)
	__attribute__((weak, alias("__arch_memory_post_cleanup")));

/*-------------------------------------------------------------------------*/

void memory_post_init(memory_post_log_t log)
{
	post_log = log;

	arch_memory_post_prepare(log);
}

void memory_post_exit(void)
{
	arch_memory_post_cleanup(post_log);
	...
	*/
	for (pattern = 1; pattern != 0; pattern <<= 1) {
		...
		*/
		...
		if (readback != pattern) {
			if (post_log)
				post_log("Memory (date bus) error at %08x, "
					"wrote %08x, read %08x !\n",
					(u32)address, (u32)pattern, (u32)readback);
...
int memory_post_addrline2(unsigned long start, unsigned long size)
{
	...
	for (i = 1; i < num_words; i <<= 1) {
		...
		if (readback != pattern) {
			if (post_log)
				post_log("Memory (addr line 2) bit stuck high @ 0x%.8lx:"
					" expected 0x%.8lx, actual 0x%.8lx\n",
					(ulong)&mem[i], pattern, readback);
			ret = -1;
		}
	}
	mem[j] = pattern;
	 ...
	 */
	for (j = 1; j < num_words; j <<= 1) {
		mem[j] = anti_pattern;
		for (i = 1; i < num_words; i <<= 1) {
			readback = mem[i];
			if ((readback != pattern) && (i != j)) {
				if (post_log)
					post_log("Memory (add line 2) bit stuck low or shorted @"
...
int memory_post_dataline(unsigned long long * pmem)
{
	...
	for ( i = 0; i < num_patterns; i++) {
		...
		if (temp64 != pattern[i]){
			...
			if (post_log)
				post_log("Memory (data line) err @ %08x, "
					"wrote %08x%08x, read %08x%08x !\n",
					(u32)pmem, pathi, patlo, hi, lo);
			ret = -1;
		}
	}
	return ret;
		...
		if((target >= base) && (target < end)) {
			*testaddr = ~*target;
			...
			if(readback == *testaddr) {
				if (post_log)
					post_log("Memory (addr line) err @ %08x<->%08x, "
						"XOR value %08x !\n",
						(u32)testaddr, (u32)target, (u32)xor);
				ret = -1;
			}
		}
	}
...
int memory_post_test1(unsigned long start,
				unsigned long size,
				unsigned long val)
{
	unsigned long i;
	...
	for (i = 0; i < size / sizeof (ulong) && !ret; i++) {
		...
		if (readback != val) {
			...
			/* Read 100 times to check it's not read issue */
			for (j = 0; j < 100; j++) {
				readback = mem[i];
				if (readback != val)
					count++;
			...
			if (post_log)
				post_log("Memory err @ %08x, wrote %08x, read %08x (%u)!\n",
					(u32)(mem + i), (u32)val, (u32)readback, count);
...
int memory_post_test2(unsigned long start, unsigned long size)
{
	...
	for (i = 0; i < size / sizeof (ulong) && !ret; i++) {
		...
		if (readback != (1 << (i % 32))) {
			...
			if (post_log)
				post_log("Memory err @ %08x, wrote %08x, read %08x (%u)!\n",
					(u32)(mem + i), (u32)(1 << (i % 32)), (u32)readback, count);
...
int memory_post_test3(unsigned long start, unsigned long size)
{
	...
	for (i = 0; i < size / sizeof (ulong) && !ret; i++) {
		...
		if (readback != i) {
			...
			if (post_log)
				post_log("Memory err @ %08x, wrote %08x, read %08x (%u)!\n",
					(u32)(mem + i), (u32)i, (u32)readback, count);
...
#endif /* !CONFIG_SPL_BUILD && CONFIG_SYS_MEMORY_POST */
