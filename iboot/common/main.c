...
extern void otg_handle_interrupts(int force);
...
int abortboot(int bootdelay)
{
	...
	if (bootdelay > 0) {
		int maxc = 33;
		/* Flush UART console before the boot delay */
		while ((--maxc > 0) && tstc())
			(void)getc();
	}
...
#ifdef CONFIG_SYS_NO_REPEATABLE_COMMAND
			continue;
#else /* !CONFIG_SYS_NO_REPEATABLE_COMMAND */
			flag |= CMD_FLAG_REPEAT;
#endif /* CONFIG_SYS_NO_REPEATABLE_COMMAND */
#ifdef CONFIG_BOOT_RETRY_TIME
		else if (len == -2) {
