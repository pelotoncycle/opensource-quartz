						...
						struct remote_dev_info *dev)
{
	...
	if (dev->type != ADDR_TYPE_BREDR) {
		...
		dev->legacy = FALSE;
error("adapter_emit_device_found::Name:%s, address:%s",dev->name,paddr);
					...
					uint8_t *data, uint8_t data_len)
{
    ...
    int i;
         error("[%08x]",dev_class);
         if (dev_class == 0 && type ==1){             
             error("Class:%d ,Len:%d ,Type:%d ,wl_scan:%d",dev_class,data_len,type,adapter->wl_scan);
             for (i=0;i<data_len;i++){
                error("[%x]",*(data+i));
             }
             dev_class = 0x00240404;
             type = 0;
         }
#endif		 
...
error("adapter_update_found_devices name:%s ,type:%d ,class:%x",name ,type ,dev_class);
