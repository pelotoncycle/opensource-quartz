...
static void mgmt_user_passkey_notification(int sk, uint16_t index, void *buf, size_t len)
{
	struct mgmt_ev_user_passkey_notification *ev = buf;
	struct controller_info *info;
	char addr[18];
	int err;

	if (len < sizeof(*ev)) {
		error("Too small passkey_notification event");
		return;
	}
	...
	if (index > max_index) {
		error("Unexpected index %u in passkey_notification event", index);
		return;
	}

	info = &controllers[index];

	err = btd_event_user_notify(&info->bdaddr, &ev->addr.bdaddr,ev->value);
	if (err < 0) {
...
static gboolean mgmt_event(GIOChannel *io, GIOCondition cond, gpointer user_data)
{
	...
	case MGMT_EV_USER_PASSKEY_NOTIFICATION:
		mgmt_user_passkey_notification(sk, index, buf + MGMT_HDR_SIZE, len);
		break;
