...
static int send_disable_sleep_command(int fd)
{
	char response[1024] = {0};
	uint8_t data[12] =	{0x0c,0xfd,0x09,0x00,0x00,0x00,0xff,0xff,0xff,0xff,0x00,0x00};
	
	hci_command_hdr *cmd = (hci_command_hdr *) data;
	uint16_t opcode = cmd->opcode,i=0;

	struct hci_request rq;
	
	memset(&rq, 0, sizeof(rq));
	printf("send_disable_sleep_command opcode=%x\n",opcode);
	...
	printf("rq.ogf=%x, rq.ocf=%x, rq.clen=%x\n",rq.ogf,rq.ocf,rq.clen);
...
static int send_scan_enable_command(int fd)
{
	char response[1024] = {0};
	uint8_t data[4] =	{0x1a,0x0c,0x01,0x03};
	...
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_scan_enable_command opcode=%x\n",opcode);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3];
...
static int send_set_event_filter_command(int fd)
{
	char response[1024] = {0};
	uint8_t data[6] =	{0x05,0x0c,0x03,0x02,0x00,0x03};
	...
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_set_event_filter_command opcode=%x\n",opcode);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3];
...
static int send_dut_command(int fd)
{
	char response[1024] = {0};
	uint8_t data[3] =	{0x03,0x18,0x00};
	...
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_dut_command opcode=%x\n",opcode);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3];
...
int brf_send_command(void)
{
	int dev_id, dd, ret = 0;
	printf("brf_send_command\n");
	...
	send_disable_sleep_command(dd);
	sleep(1);
	send_scan_enable_command(dd);
	sleep(1);
	send_set_event_filter_command(dd);
	sleep(1);
	send_dut_command(dd);
...
static int send_bt_hci_rx_start_attach_cmd(int fd ,int channels)
{
	char response[1024] = {0};
	uint8_t data[] =	{0x1d,0x20,0x01,0x00};

	if (channels >=0 & channels<=39)
	    data[3]=channels;

//You need use windows tool "BTS Transform" to gen the data patten.
//Send_HCI_BLE_Receiver_Test 0x00 ->{0x1d,0x20,0x01,0x00};
	...
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_bt_hci_rx_start_attach_cmd opcode=%x (Freq:%d)\n",opcode,2402+data[3]*2);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3]; 
...
static int send_bt_hci_tx_start_attach_cmd(int fd ,int channels)
{
	char response[1024] = {0};
	uint8_t data[] =	{0x1e,0x20,0x03,0x00,0x25,0x02};
 
	if (channels >=0 & channels<=39)
	    data[3]=channels;
//You need use windows tool "BTS Transform" to gen the data patten.
//Send_HCI_BLE_Transmitter_Test 0, 0x25, 2  ->{0x1e,0x20,0x03,0x00,0x25,0x02};
	...
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_bt_hci_tx_start_attach_cmd opcode=%x (Freq:%d)\n",opcode,2402+data[3]*2);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3];
...
static int send_bt_hci_end_attach_cmd(int fd)
{
	char response[1024] = {0};
	uint8_t data[3] =	{0x1f,0x20,0x00};
	
	hci_command_hdr *cmd = (hci_command_hdr *) data;
	uint16_t opcode = cmd->opcode,i=0;

//You need use windows tool "BTS Transform" to gen the data patten.
//Send_HCI_BLE_Test_End  ->	{0x1f,0x20,0x00};
	struct hci_request rq;
	memset(&rq, 0, sizeof(rq));
	printf("send_bt_hci_end_attach_cmd opcode=%x\n",opcode);
	rq.ogf    = cmd_opcode_ogf(opcode);
	rq.ocf    = cmd_opcode_ocf(opcode);
	rq.event  = EVT_CMD_COMPLETE;
	rq.cparam = &data[3];    
...
int brf_tx_ble_dut_command(int channel){
	int dev_id, dd, ret = 0;
	printf("brf_tx_2402_ble_dut_command\n");
	...
	send_scan_enable_command(dd);
	sleep(1);
	send_bt_hci_end_attach_cmd(dd);
	sleep(1);	
      send_bt_hci_tx_start_attach_cmd(dd,channel);
	sleep(1);
...
int brf_rx_ble_dut_command(int channel){
	int dev_id, dd, ret = 0;
	printf("brf_rx_2402_ble_dut_command\n");
	...
	send_scan_enable_command(dd);
	sleep(1);	
      send_bt_hci_rx_start_attach_cmd(dd,channel);
	sleep(2);
...
static void usage(void)
{
	...
	printf("Usage:\n");
	printf("\thciattach [-n] [-p] [-b] [-d] [-e] [-r] [-t timeout] [-s initial_speed] <tty> <type | id> [speed] [flow|noflow] [bdaddr]\n");
	printf("\thciattach -l\n");
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt=getopt(argc, argv, "bnpde:t:s:lr")) != EOF) {
		switch(opt) {
		case 'b':
			send_break = 1;
			break;
		...
		case 'e':
                        if (!strcmp("rx",argv[2])){
                          brf_rx_ble_dut_command(atoi(argv[3]));
                        }else if (!strcmp("tx",argv[2])){
                           brf_tx_ble_dut_command(atoi(argv[3]));
                        }else{
                          printf("Error !!!\n");
                        }
			exit(0);
