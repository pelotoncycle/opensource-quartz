...
static void shutdown_mdl(struct mcap_mdl *mdl)
{
	...
	if (mdl->dc) {
		DBG("Call g_io_channel_shutdown() with mdl->dc");
...
// Shut down the MDL channel as required by PTS MCAP test-TC_MCAP_CM_DIS_BV_04_C
gboolean mcap_mdl_shutdown_for_pts_blueti(struct mcap_mdl *mdl)
{
	DBG("");
	struct mcap_mcl *mcl = mdl->mcl;
	mdl->state= MDL_WAITING;
