...
#include <log.h>
#define DBG(fmt, arg...)  printf("MCAP UTILS: %s: " fmt "\n" , __FUNCTION__ , ## arg)
...
{ DBG("");
	...
	if (ctype != DBUS_TYPE_STRING) {
		g_set_error(err, MCAP_ERROR, MCAP_DIC_ENTRY_PARSE_ERROR,
...
{ DBG("");
	...
	if (ctype != DBUS_TYPE_ARRAY) {
		g_set_error(err, MCAP_ERROR, MCAP_DIC_PARSE_ERROR,
...
static gboolean parse_data_type(DBusMessageIter *iter, gpointer data,
								GError **err)
{ DBG("");
	struct mcap_application *app = data;
...
{ DBG("");
	...
	if (g_ascii_strcasecmp(role, MCAP_SINK_ROLE_AS_STRING) == 0) {
		app->role = MCAP_SINK;
	} else if (g_ascii_strcasecmp(role, MCAP_SOURCE_ROLE_AS_STRING) == 0) {
		app->role = MCAP_SOURCE;
	} else {
		g_set_error(err, MCAP_ERROR, MCAP_UNSPECIFIED_ERROR,
...
{ DBG("");
	...
	if (g_ascii_strcasecmp("Reliable", chan_type) == 0)
		app->chan_type = MCAP_RELIABLE_DC;
	else if (g_ascii_strcasecmp("Streaming", chan_type) == 0)
		app->chan_type = MCAP_STREAMING_DC;
	else {
		g_set_error(err, MCAP_ERROR, MCAP_DIC_ENTRY_PARSE_ERROR,
...
{ DBG("");
	...
	app = g_new0(struct mcap_application, 1);
	app->ref = 1;

	app->chan_type = MCAP_RELIABLE_DC;
	app->chan_type_set = TRUE;
	app->description = strdup("MCAP TEST");
	app->role = MCAP_SINK;
	app->role_set = TRUE;
	app->data_type= 1007;
	app->data_type_set = TRUE;
...
{ DBG("");
	...
	for (l = app_list; l; l = l->next) {
		struct mcap_application *app = l->data;
...
{ DBG("");
	...
	sdp_uuid16_create(&svc_uuid_sink,  0x1402);
	sdp_uuid16_create(&svc_uuid_source, 0x1401);
...
static gboolean register_service_protocols(struct mcap_adapter *adapter,
...
static void free_mcap_list(void *list)
{ DBG("");
	sdp_list_t *mcap_list = list;

	sdp_list_free(mcap_list, (sdp_free_func_t)sdp_data_free);
}
...
{ DBG("");
	...
	if (sdp_set_supp_feat(sdp_record, sup_features) < 0) {
		sdp_list_free(sup_features, free_mcap_list);
		return FALSE;
	}
...
{ DBG("");
	if ((rec_role == MCAP_SINK && app_role == MCAP_SOURCE) ||
			(rec_role == MCAP_SOURCE && app_role == MCAP_SINK))
		return TRUE;
...
static void free_mdep_data(gpointer data)
{
	...
	mcap_application_unref(mdep_data->app);
...
static gboolean mcap_get_add_prot_desc_list(const sdp_record_t *rec,
								guint16 *psm)
{
	DBG("");
		sdp_data_t *pdl, *p0, *p1;
...
static gboolean get_ccpsm(sdp_list_t *recs, uint16_t *ccpsm)
{
	...
	for (l = recs; l; l = l->next) {
		...
		if (mcap_get_prot_desc_list(rec, ccpsm, NULL))
			return TRUE;
	}
...
static gboolean get_dcpsm(sdp_list_t *recs, uint16_t *dcpsm)
{
	...
	for (l = recs; l; l = l->next) {
		...
		if (mcap_get_add_prot_desc_list(rec, dcpsm))
			return TRUE;
	}
 ...
 void con_mcl_data_unref(struct conn_mcl_data *conn_data)
{
	...
	mcap_device_unref(conn_data->dev);
	dbus_message_unref(conn_data->msg);
	dbus_connection_ref(conn_data->conn);
	g_free(conn_data);
}
...
void create_mcl_cb(struct mcap_mcl *mcl, GError *err, gpointer data)
{
	...
	struct mcap_device *device = conn_data->dev;
	GError *gerr = NULL;
...
/*
	...
	func(0x1003, data, NULL);
/*
	bt_string2uuid(&uuid, MCAP_UUID);
...
static void mcap_free_application(struct mcap_application *app)
{ DBG("");
	if (app->dbus_watcher)
...
{ DBG("");
	...
	DBG("mcap_application_ref(%p): ref=%d", app, app->ref);
	return app;
}
...
{ DBG("");
	...
	DBG("mcap_application_unref(%p): ref=%d", app, app->ref);
	if (app->ref > 0)
		return;

	mcap_free_application(app);
}


/*

void free_mcap_channel(struct mcap_channel *chan)
{
DBG("");
