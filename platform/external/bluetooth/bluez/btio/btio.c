...
static int set_addr_type(int sock, uint32_t addr_type)
{
	if (setsockopt(sock, SOL_BLUETOOTH, BT_ADDR_TYPE, &addr_type, sizeof(addr_type)) < 0)
		return -errno;
...
static gboolean l2cap_set(int sock, int sec_level, uint16_t imtu,
				...
				int flushable, uint32_t priority, uint8_t force_active,
				uint8_t addr_type, GError **err)
{
	if (imtu || omtu || mode) {
	...
	if (set_addr_type(sock, addr_type) < 0)
		return FALSE;
						...
						BtIOOption opt1, va_list args)
{
	...
	while (opt != BT_IO_OPT_INVALID) {
		...
		case BT_IO_OPT_PRIORITY:
			...
			break;
		case BT_IO_OPT_DEST_BDADDR_TYPE:
			opts->addr_type = va_arg(args,int);
			break;
		default:
			g_set_error(err, BT_IO_ERROR, BT_IO_ERROR_INVALID_ARGS,
							...
							BtIOOption opt1, ...)
{
	...
	case BT_IO_L2ERTM:
		return l2cap_set(sock, opts.sec_level, opts.imtu, opts.omtu,
				...
				opts.priority, opts.force_active, opts.addr_type , err);
	case BT_IO_RFCOMM:
					...
					struct set_opts *opts, GError **err)
{
	...
	case BT_IO_L2RAW:
		...
		if (l2cap_bind(sock, &opts->src, server ? opts->psm : 0,
							opts->cid, err) < 0)
			goto failed;
		if (!l2cap_set(sock, opts->sec_level, 0, 0, 0, -1, -1, 0, opts->force_active,
				opts->addr_type, err))
			goto failed;
		break;
	case BT_IO_L2CAP:
		...
		if (!l2cap_set(sock, opts->sec_level, opts->imtu, opts->omtu,
				opts->mode, opts->master, opts->flushable,
				opts->priority, opts->force_active, 
				opts->addr_type, err))
			goto failed;
		break;
	case BT_IO_L2ERTM:
