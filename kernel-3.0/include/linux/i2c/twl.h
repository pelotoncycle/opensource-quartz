...
#define JTAGVERNUM 0x87 //for 6032, es1.0, es1.1 ...
...
struct ntc_res_temp_desc{
	int temp;
	int res;
};
struct batt_volt_cap_desc{
	int volt;
	int cap;
};
#define BATT_VOLT_CAP_SIZE 21
struct Loading_Table
{
	int			curr; /*Loading current*/
	struct batt_volt_cap_desc			*batt_volt_cap;				/*Voltage v.s. Capacity table*/
};
struct charge_cv_profile{
	int cap;
	int curr;
};
...
struct twl4030_bci_platform_data {
	...
	unsigned int tblsize_603x;
	
 	struct charge_cv_profile	*battery_cv_profile;
	unsigned int temp_rpu_ohm;
	unsigned int temp_rpd_ohm;
	int			ac_share_usb_connector;
	unsigned int charger_fuel_gauge_type;
...
/*For compatible with 603x bci driver, already removed in 4AI-1.6, please fixed this*/
	/* twl6032 */
...
struct twl_pwm_led {
	...
	/**
	 * TWL6032 default_clock_cycle:
	 *   PWM1/PWM2 clock rate:
	 *     * 128:	256Hz
	 *     * 64:	512Hz
	 */
	int default_clock_cycle;
	int invert_duty_cycle;	/* output duty cycle = 100 - current duty cycle */
	int max_duty_cycle;
	int min_duty_cycle;
	/** platform_init
	 * Returns 0: don't register leds class in TWL pwm driver
	 * Others: register leds class in TWL pwm driver
	 */
...
struct twl4030_platform_data {
	...
	struct twl_pwm_data				*pwm;
	struct twl_rtc_data				*rtc;
