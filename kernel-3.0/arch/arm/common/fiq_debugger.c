...
struct fiq_debugger_state {
	...
	bool console_enable;
	bool console_enable_no_debug;
	int current_cpu;
	atomic_t unhandled_fiq_count;
	bool in_fiq;

#ifdef CONFIG_FIQ_DEBUGGER_CONSOLE
/** https://android.googlesource.com/kernel/common/+/0069420b2100d15ab6ae3088fe39f44c9cda296f
...
#ifdef CONFIG_FIQ_DEBUGGER_CONSOLE_NO_DEBUG_MODE
static bool initial_console_enable_no_debug = true;
#else
static bool initial_console_enable_no_debug;
#endif
			...
			int this_cpu, void *regs, void *svc_sp)
{
	...
	while ((c = debug_getc(state)) != FIQ_DEBUGGER_NO_CHAR) {
		...
		if (!state->debug_enable) {
			...
			if (!state->console_enable_no_debug) {
			...
			if (state->debug_count) {
				if (state->debug_count < DEBUG_MAX)
					state->debug_buf[state->debug_count] = 0;
				else
					state->debug_buf[DEBUG_MAX - 1] = 0;
				state->debug_count = 0;
				signal_helper |=
...
static int fiq_debugger_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_FIQ_DEBUGGER_CONSOLE) && defined(CONFIG_ANDROID_BOOT_REASON)
	if (!initial_console_enable &&
			(android_boot_get_reason() != ANDROID_BOOT_REASON_NORMAL
				|| android_boot_get_mode() != ANDROID_BOOT_MODE_NORMAL)) {
		initial_debug_enable = true;
		initial_console_enable = true;
	}
#endif /* CONFIG_FIQ_DEBUGGER_CONSOLE && CONFIG_ANDROID_BOOT_REASON */
	...
	state->console_enable_no_debug = initial_console_enable_no_debug;
...
/* Force to enable sleep timer during probing */
#if 0
	if (state->no_sleep)
#endif
		handle_wakeup(state);

	/* REMOVED because we initialize tty earlier */
#if 0
#if defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
/** https://android.googlesource.com/kernel/common/+/0069420b2100d15ab6ae3088fe39f44c9cda296f
 * ARM: fiq_debugger: lock between tty and console writes
...
err_register_fiq:
	...
	state->tty_rbuf = NULL;
	put_tty_driver(state->tty_driver);
	unregister_console(&state->console);
	if (pdata->uart_free)
