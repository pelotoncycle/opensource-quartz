...
#else
	/* Avoid "misc dsscomp: tiler slot not big enough for frame ..." error */
	data.tiler1d_slotsz =
		ALIGN(ALIGN(mem->width, 64) * mem->height * 4 * 4 + (PAGE_SIZE * 32), PAGE_SIZE * 32);
#endif
...
#else
	if (sgx_config && sgx_config->tiler_total_size > SZ_64M) {
		u32 size = ALIGN(mem->tiler1d_mem, PAGE_SIZE * 32) + ion->nonsecure_tiler2d_size;
		if ((size + SZ_32M) > sgx_config->tiler_total_size) {
			pr_err("android_display: tiler total size is too small %u (%u)\n",
					sgx_config->tiler_total_size, size);
			ion->tiler2d_size = SZ_128M;
		} else
			ion->tiler2d_size = sgx_config->tiler_total_size;
	} else
		ion->tiler2d_size = SZ_128M;
#endif
