...
static struct delayed_work clk_disable_dss_work;

static void clk_disable_dss_workfunc(struct work_struct *work)
{
	struct clk *clkp;

	list_for_each_entry(clkp, &clocks, node) {
		if (clkp->flags & DSS_ENABLE_ON_INIT)
			clk_disable(clkp);
	}
	pr_info("clk: dss early-init clocks released\n");
}

static int __init clk_disable_dss_clocks(void)
{
	if (videolfb_is_enabled()) {
		struct clockdomain *l3_1_clkdm, *l3_2_clkdm;
		...
		INIT_DELAYED_WORK_DEFERRABLE(&clk_disable_dss_work, clk_disable_dss_workfunc);
		schedule_delayed_work_on(0, &clk_disable_dss_work, msecs_to_jiffies(12000));
	}

	return 0;
}
subsys_initcall(clk_disable_dss_clocks);
