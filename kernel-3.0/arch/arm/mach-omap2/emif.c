				...
				u8 RL)
{
	...
	/* Sync value with X-Loader */
	/**
	 * REG_DDR2_DDQS:
	 * This bit is only for DDR2 mode, because the device
	 * supports LPDDR2; this bit is don’t care.
	 */
	mask_n_set(config_reg, OMAP44XX_REG_DDR2_DDQS_SHIFT,
		   OMAP44XX_REG_DDR2_DDQS_MASK,
		   1);
...
/* OMAP4: EMIF: LPDDR interface configuration update */
/* GIT commit: 44a76c8dfff24b12f25623da16d16e60199efe9d */
#if 0
...
#else
/* Sync value with X-Loader */
static u32 get_ddr_phy_ctrl_1(const struct lpddr2_device_info *cs1_device,
		u32 freq, u8 RL)
{
	u32 phy = 0, val = 0;

	/* OMAP4430/4460/4470 TRM:
	...
	if (RL == 6)
		...
		RL += 2;
	mask_n_set(phy, OMAP44XX_REG_READ_LATENCY_SHIFT,
		   OMAP44XX_REG_READ_LATENCY_MASK, RL);

	if (freq <= 100000000) {
		/* Other fields are constant magic values. Hardcode them together */
		mask_n_set(phy, OMAP44XX_EMIF_DDR_PHY_CTRL_1_BASE_VAL_SHIFT,
			OMAP44XX_EMIF_DDR_PHY_CTRL_1_BASE_VAL_MASK,
			EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_2);
		...
		if (cpu_is_omap447x()) {
			...
			if (cs1_device)
				...
				mask_n_set(phy, OMAP44XX_EMIF_DDR_PHY_CTRL_1_BASE_VAL_SHIFT,
					OMAP44XX_EMIF_DDR_PHY_CTRL_1_BASE_VAL_MASK,
					EMIF_DDR_PHY_CTRL_1_BASE_VAL_DLL_MASTER_SENSITIVITY_1);
			val = EMIF_DLL_SLAVE_DLY_CTRL_200_MHZ;
		}
	} else if (freq <= 233333333) {
		/* Other fields are constant magic values. Hardcode them together */
...
{
	static bool derate_refresh_log = true, derate_refresh_and_timings_log = true;
	...
	if (unlikely(temperature == SDRAM_TEMP_HIGH_DERATE_REFRESH)) {
		if (derate_refresh_log) {
			derate_refresh_log = false;
			derate_refresh_and_timings_log = true;
			pr_emerg("emif%d: SDRAM temperature is high (DERATE_REFRESH)\n", emif_nr + 1);
		}
		...
		if (derate_refresh_and_timings_log) {
			derate_refresh_log = true;
			derate_refresh_and_timings_log = false;
			pr_emerg("emif%d: SDRAM temperature is high (DERATE_REFRESH_AND_TIMINGS)\n", emif_nr + 1);
		}
...
static irqreturn_t emif_threaded_isr(int irq, void *dev_id)
{
	...
	if (emif_temperature_level[emif_nr] >= SDRAM_TEMP_VERY_HIGH_SHUTDOWN) {
		...
		enforce_emergency_sync();
		enforce_emergency_remount();
		kernel_power_off();
	}
...
#else
	/* EMIF 466MHz & 233MHz only */
	if (freq >= 466666666)
		regs->ref_ctrl -= 2;
	else if (freq < 400000000 && freq >= 233333333)
		regs->ref_ctrl -= 1;
#endif
...
#else
	regs->emif_ddr_phy_ctlr_1_init =
	    get_ddr_phy_ctrl_1(NULL/*ignored*/, EMIF_FREQ_19_2_MHZ, RL_19_2_MHZ);

	regs->emif_ddr_phy_ctlr_1_final =
	    get_ddr_phy_ctrl_1(cs1_device, freq, regs->RL_final);
#endif
...
static int emif_driver_suspend(struct platform_device *pdev, pm_message_t state)
{
	emif_clear_irq(pdev->id);
	return 0;
}

static int emif_driver_resume(struct platform_device *pdev)
{
	u32 base = (pdev->id == 0) ? OMAP44XX_EMIF1_VIRT : OMAP44XX_EMIF2_VIRT;
	u32 temp;

	/* Refer to setup_emif_interrupts() */
