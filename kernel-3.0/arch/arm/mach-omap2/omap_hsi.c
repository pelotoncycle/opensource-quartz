...
/*
Because of the sequence in arch_initcall() section is fixed during link time, 
omap_hsi_dev_init() might be called before .init_machine and result in mipi-hsi
driver registration failed later.
Move omap_hsi_dev_init() to arch_initcall_sync section to make sure .init_machine
can change omap_hsi_registration_allowed variable using
omap_hsi_allow_registration() before omap_hsi_dev_init() is called.
*/
//arch_initcall(omap_hsi_dev_init);
arch_initcall_sync(omap_hsi_dev_init);
