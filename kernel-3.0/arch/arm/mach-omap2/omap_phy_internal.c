...
#define USB2PHY_GPIOMODE		BIT(29)
static struct clk *phyclk, *clk48m, *clk32k;
static void __iomem *ctrl_base;
static int usbotghs_control;
struct omap_device_pad_info {
	u16				mode;
	struct omap_mux_partition	*partition;
	struct omap_mux			*mux;
};
struct usb_uart_mux{
	struct omap_device_pad_info usb;
	struct omap_device_pad_info uart;
};
struct usb_uart_mux dp,dm,uart_rx;
...
extern int omap_serial_ext_uart_pause(u8 port_id,int pause);
void omap4430_dpdm_select(int mode, int diable_uart)
{
	u32 usb2phycore;
	...
	case 0: 
		usb2phycore = omap4_ctrl_pad_readl(CONTROL_USB2PHYCORE);
		if(usb2phycore&USB2PHY_GPIOMODE){
#if defined(CONFIG_OMAP_FIQ_DEBUGGER) && defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
#else			
			if(diable_uart)
				//omap_serial_ext_uart_disable(2);
				omap_serial_ext_uart_pause(2,1);
#endif			
			omap_mux_write(uart_rx.usb.partition, uart_rx.usb.mode, uart_rx.usb.mux->reg_offset);
			usb2phycore&=~USB2PHY_GPIOMODE;
			usb2phycore|=USB2PHY_DISCHGDET;
			omap4_ctrl_pad_writel(usb2phycore, CONTROL_USB2PHYCORE);

			omap_mux_write(dp.usb.partition, dp.usb.mode, dp.usb.mux->reg_offset);
			omap_mux_write(dm.usb.partition, dm.usb.mode, dm.usb.mux->reg_offset);
#if defined(CONFIG_OMAP_FIQ_DEBUGGER) && defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
#else			
			if(diable_uart)
				//omap_serial_ext_uart_enable(2);	
				omap_serial_ext_uart_pause(2,0);
#endif			
		}	
	...
	case 1:
		usb2phycore = omap4_ctrl_pad_readl(CONTROL_USB2PHYCORE);
		if(!(usb2phycore&USB2PHY_GPIOMODE)||!diable_uart){
#if defined(CONFIG_OMAP_FIQ_DEBUGGER) && defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
#else
			if(diable_uart)
				//omap_serial_ext_uart_disable(2);
			omap_serial_ext_uart_pause(2,1);
#endif		
			usb2phycore|=(USB2PHY_GPIOMODE|USB2PHY_DISCHGDET);
			omap4_ctrl_pad_writel(usb2phycore, CONTROL_USB2PHYCORE);
			mdelay(2);
			omap_mux_write(dp.uart.partition, dp.uart.mode, dp.uart.mux->reg_offset);
			omap_mux_write(dm.uart.partition, dm.uart.mode, dm.uart.mux->reg_offset);	
			omap_mux_write(uart_rx.uart.partition, uart_rx.uart.mode, uart_rx.uart.mux->reg_offset);
#if defined(CONFIG_OMAP_FIQ_DEBUGGER) && defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
#else			
			if(diable_uart)
				//omap_serial_ext_uart_enable(2);
...
static int __init omap_usb_uart3_init(void)
{

dp.usb.mode = omap_mux_get_by_name("usba0_otg_dp.usba0_otg_dp", 
	 &dp.usb.partition, &dp.usb.mux);
dm.usb.mode = omap_mux_get_by_name("usba0_otg_dm.usba0_otg_dm", 
	 &dm.usb.partition, &dm.usb.mux);
uart_rx.usb.mode = omap_mux_get_by_name("uart3_rx_irrx.uart3_rx_irrx", 
	 &uart_rx.usb.partition, &uart_rx.usb.mux)|OMAP_PIN_INPUT_PULLUP;

dp.uart.mode = omap_mux_get_by_name("usba0_otg_dp.uart3_rx_irrx", 
	 &dp.uart.partition, &dp.uart.mux)|OMAP_PIN_INPUT_PULLUP;
dm.uart.mode = omap_mux_get_by_name("usba0_otg_dm.uart3_tx_irtx", 
	 &dm.uart.partition, &dm.uart.mux);
uart_rx.uart.mode = omap_mux_get_by_name("uart3_rx_irrx.safe_mode", 
	 &uart_rx.uart.partition, &uart_rx.uart.mux)|OMAP_PIN_INPUT_PULLUP;

	return 0;
}

arch_initcall(omap_usb_uart3_init);
