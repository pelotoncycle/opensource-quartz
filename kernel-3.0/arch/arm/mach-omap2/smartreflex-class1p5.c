...
#else
		if (work_data->pm_qos_added) {
			work_data->pm_qos_added = false;
...
#else
{
		unsigned long u_volt_safe_old = u_volt_safe;

		/* Add margin IF we are less than and equal to nominal */
		if ((u_volt_safe + u_volt_margin) <= volt_data->volt_nominal) {
			u_volt_safe += u_volt_margin;
		} else {
			u_volt_safe = max_t(unsigned long, u_volt_safe, volt_data->volt_nominal);
			if (u_volt_safe_old < volt_data->volt_nominal)
				u_volt_margin = (unsigned long)volt_data->volt_nominal - u_volt_safe_old;
			else
				u_volt_margin = 0;
		}
		pr_info("%s: %s new Vsafe %lu->%lu (%lu margin)\n", __func__, voltdm->name,
				u_volt_safe_old, u_volt_safe, u_volt_margin);
}
#endif
	}
...
#else
	if (!work_data->pm_qos_added) {
		pm_qos_add_request(&work_data->qos, PM_QOS_CPU_DMA_LATENCY, 0);
		work_data->pm_qos_added = true;
	}
#endif
