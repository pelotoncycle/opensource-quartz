...
static struct usbhs_wakeup {
	...
	struct clockdomain *l3init_clkdm;
} *usbhs_wake;
...
#if defined(CONFIG_MACH_OMAP4_WINMATE) || defined(CONFIG_MODEM_CINTERION)
		/* WA before 10ms USB remote wakeup modem firmware available */
		.enable = OMAP_PIN_OUTPUT | OMAP_PIN_OFF_OUTPUT_HIGH | OMAP_MUX_MODE4,
#else
...
void usbhs_wakeup()
{
	...
	if (workq) {
		...
		/*
		   The EHCI interrupt should happens after 60Mhz enabled, just wait 200ms so
		   that ehci_omap_bus_resume() can be called and enable 60Mhz as well.
		*/
		...
		if (queued) {
			...
			if (usbhs_wake->l3init_clkdm)
				clkdm_wakeup(usbhs_wake->l3init_clkdm);

			/* Enable USB PHY reference clocks so that EHCI interrupt can happen */
			for (i = 0; i < OMAP3_HS_USB_PORTS; i++) {
				if (ehci_data.transceiver_clk[i])
					clk_enable(ehci_data.transceiver_clk[i]);
			}

			pm_runtime_get(usbhs_wake->dev);

			dev_dbg(usbhs_wake->dev, "USB IO PAD Waked-up\n");
		}
	}
}
...
static void usbhs_resume_work(struct work_struct *work)
{
	...
	/*
	   Disable USB PHY clocks due to they are already enabled by ehci_omap_bus_resume(),
	   therefore the USB PHY clocks can be gated in suspend mode due to usecount=0
	*/
	for (i = 0; i < OMAP3_HS_USB_PORTS; i++) {
		if (ehci_data.transceiver_clk[i])
			clk_disable(ehci_data.transceiver_clk[i]);
	}

	if (usbhs_wake->l3init_clkdm)
		clkdm_allow_idle(usbhs_wake->l3init_clkdm);
}
...
void __init usbhs_init(const struct usbhs_omap_board_data *pdata)
{
	...
	ehci_data.hub_reset = pdata->hub_reset;
	ehci_data.hub_reset_gpio_port = pdata->hub_reset_gpio_port;
	...
	usbhs_wake->l3init_clkdm = clkdm_lookup("l3_init_clkdm");
	if (!usbhs_wake->l3init_clkdm) {
