...
static bool omap4_l3_probe_reboot = false;
...
static irqreturn_t l3_interrupt_handler(int irq, void *_l3)
{
	...
	if (l3->debug_irq == 0) {
		...
		pr_warning("L3: debug_irq triggered immediately after requested!\n");
	} else {
		inttype = irq == l3->debug_irq ? L3_DEBUG_ERROR : L3_APPLICATION_ERROR;
		if ((l3->app_irq == 0) && (inttype == L3_APPLICATION_ERROR))
			pr_warning("L3: app_irq triggered immediately after requested!\n");
	}
...
static int __init omap4_l3_probe(struct platform_device *pdev)
{
	...
	if (omap4_l3_probe_reboot)	{
		pr_err("omap4_l3_probe_reboot is set! Reboot now!\n");
		kernel_restart(NULL);
	}
	return 0;
