...
#else
{
	u32 vtarget = voltdm->pmic->vsel_to_uv(target_vsel);
	u32 vcurr = voltdm->pmic->vsel_to_uv(current_vsel);

	smps_steps = (vtarget > vcurr) ? (vtarget - vcurr) : (vcurr - vtarget);
	/* 2us added as buffer. */
	smps_delay = DIV_ROUND_UP(smps_steps, voltdm->pmic->slew_rate) + 2;
}
#endif
	udelay(smps_delay);
...
static int __init omap_vc_setup_lp_time(struct voltagedomain *voltdm,
					bool is_retention)
{
	/* Backport correct code from K3.4 correct it */
	u32 prescaler;
	u32 cycles;
	u32 time;
	u32 voltage_diff;
	...
	if (is_retention)
		voltage_diff = voltdm->pmic->on_volt - voltdm->pmic->ret_volt;
	else
		voltage_diff = voltdm->pmic->on_volt - voltdm->pmic->off_volt;

	time = DIV_ROUND_UP(voltage_diff, voltdm->pmic->slew_rate);

	time += OMAP_VC_I2C_ACK_DELAY;

	if (!is_retention)
		time += voltdm->pmic->switch_on_time;
	...
	return (prescaler << OMAP4430_RAMP_DOWN_PRESCAL_SHIFT) |
	    (prescaler << OMAP4430_RAMP_UP_PRESCAL_SHIFT) |
	    (cycles << OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
