...
static unsigned long twl6030_vsel_to_uv(const u8 vsel)
{
	...
	if (!is_offset_valid) {
		twl_i2c_read_u8(TWL6030_MODULE_ID0, &smps_offset,
				REG_SMPS_OFFSET);
		twl_i2c_read_u8(TWL_MODULE_USB, &twl6030_sub_id,
				USB_PRODUCT_ID_LSB);
	 ...
	 * OMAP4430.
	 */
	if ((vsel > 0x39) && (twl6030_sub_id == 0x32)) {
		/*
		 ...
		 * switch from DVS to non-DVS ranges on TWL6032.
 		 ...
 		 */
#ifdef CONFIG_OMAP4_TWL6032_NON_DVS_RANGE_VSEL_DEBUG
		pr_warning("%s: non-DVS range VSEL 0x%x\n", __func__, vsel);
#endif /* CONFIG_OMAP4_TWL6032_NON_DVS_RANGE_VSEL_DEBUG */
		switch (vsel) {
		case 0x3A:
			return 1367400;	/* 1367.4mV */
		case 0x3B:
			return 1519300;	/* 1519.3mv */
		case 0x3C:
			return 1823100;	/* 1823.1mv */
		case 0x3D:
			return 1924400;	/* 1924.4mv */
		case 0x3E:
			return 2127000;	/* 2127.0mv */
		default:
			pr_err("%s: OUT OF RANGE! non mapped vsel for 0x%x\n", __func__, vsel);
			return 2127000;
		}
	} else
	if (vsel == 0x3A)
...
static u8 twl6030_uv_to_vsel(unsigned long uv)
{
	 ...
	 * OMAP4430.
	 */
	if (twl6030_sub_id == 0x32) {
		u8 vsel = 0;

		if (smps_offset & 0x8)
			vsel = DIV_ROUND_UP(uv - 709000, 12660) + 1;
		else
			vsel = DIV_ROUND_UP(uv - 607700, 12660) + 1;
		...
		if (vsel > 0x39) {
			 ...
			 * switch from DVS to non-DVS ranges on TWL6032.
 			 ...
 			 */
#ifdef CONFIG_OMAP4_TWL6032_NON_DVS_RANGE_VSEL_DEBUG
			pr_warning("%s: non-DVS range VSEL 0x%x, %lu\n", __func__, vsel, uv);
#endif /* CONFIG_OMAP4_TWL6032_NON_DVS_RANGE_VSEL_DEBUG */
			if (!(smps_offset & 0x8) && uv <= 1367400)		/* 1367.4mV */
				vsel = 0x3A;
			else if ((smps_offset & 0x8) && uv <= 1418000)	/* 1418.0mV */
				vsel = 0x39;
			else if (uv <= 1519300)	/* 1519.3mv */
				vsel = 0x3B;
			else if (uv <= 1823100)	/* 1823.1mv */
				vsel = 0x3C;
			else if (uv <= 1924400)	/* 1924.4mv */
				vsel = 0x3D;
			else if (uv <= 2127000)	/* 2127.0mv */
				vsel = 0x3E;
			else {
				pr_err("%s: OUT OF RANGE! non mapped vsel for %lu Vs max 2127000\n",
					__func__, uv);
				vsel = 0x3E;
			}
		}
...
/* http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=7bd771c325648b756da8d9041217544944eba7f0 */
		/*
		 * Errata ProDB00121179 says voltage overshoot may occur if
		 * switch from DVS to non-DVS ranges on TWL6032.
 		 * - DVS range VSEL codes     0x01-0x39
...
static __initdata struct omap_pmic_map omap_talos_twl_map[] = {
	{
		.name = "mpu",
		.omap_chip = OMAP_CHIP_INIT(CHIP_IS_OMAP443X |
...
int __init omap_twl_init(void)
{
	...
	if(machine_is_omap4_wedjat() || machine_is_omap4_talos7() || machine_is_omap4_talos10() || machine_is_innocomm_oracle() ||
			machine_is_omap4_puzzle() || machine_is_omap4_quartz() || machine_is_omap4_saga() || machine_is_omap4_pb1icom() || machine_is_omap4_winmate())

		return omap_pmic_register_data(omap_talos_twl_map, desc);		
	else
...
int __init omap_twl_pmic_update(char *name, u32 old_chip_id, u32 new_chip_id)
{
	...
	if(machine_is_omap4_wedjat() || machine_is_omap4_talos7() || machine_is_omap4_talos10() || machine_is_innocomm_oracle() ||
		...
		return omap_pmic_update(omap_talos_twl_map, name, old_chip_id, new_chip_id);
	else	
