...
//#include <linux/i2c/twl603x-gpadc.h>
#include <linux/i2c/twl6030-gpadc.h>

//#define CONFIG_SND_OMAP_ENABLE_HS_MIC_DETECT_FUNCTION 
//#define CONFIG_SND_OMAP_ENABLE_HS_MIC_ADC_DETECT_FUNCTION 
...
struct twl6040_hook_data {
	struct snd_soc_jack *hook;
	int state;	
	int enable;
    int amp_power_on;
};
...
/* codec private data */
struct twl6040_data {
	struct twl6040_hook_data hs_hook;
	struct delayed_work hook_enable_delayed_work;
	struct delayed_work check_hook_delayed_work;	
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	if (twl6040_get_icrev(twl6040) <= TWL6041_REV_2_0) {
		...
		case SND_SOC_DAPM_PRE_PMU:
			...
			pr_info("HSDACL reset is done when HSDACR is enabled\n");
		...
		case SND_SOC_DAPM_POST_PMD:
			/* HSDAC settling time */
			pr_info("dac_left settling time..\n");
			usleep_range(80, 200);
			break;
		default:
			break;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	if (twl6040_get_icrev(twl6040) < TWL6040_REV_1_3) {
		...
		case SND_SOC_DAPM_PRE_PMD:
			...
			pr_info("HSDACR reset is done when HSDACL is enabled\n");			
		...
		case SND_SOC_DAPM_POST_PMU:
			/* HSDAC settling time */
			pr_info("dac_right settling time..\n");
			usleep_range(80, 200);
			break;
...
{
	/* HFDAC settling time */
#if defined(CONFIG_MACH_INNOCOMM_ORACLE)
pr_info("HFDAC settling time..\n");
...
#if defined (CONFIG_MACH_OMAP4_QUARTZ)
static int twl6040_auxl_boost_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
		pr_info("twl6040_auxl_boost_event=%d\n",SND_SOC_DAPM_EVENT_ON(event));
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		pr_info("twl6040_auxl_boost_event\n");
		gpio_set_value(GPIO_AUDIO_AMP_EN,true);
	} else {
		gpio_set_value(GPIO_AUDIO_AMP_EN,false);
	}

	return 0;
}
static int twl6040_auxr_boost_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *kcontrol, int event)
{
       return 0;
}
...
static irqreturn_t twl6040_audio_hook_handler(int irq, void *data)
{
	...
	struct twl6040 *twl6040 = codec->control_data;
	struct twl6040_data *priv = snd_soc_codec_get_drvdata(codec);
	struct twl6040_hook_data *hs_hook = &priv->hs_hook;
	u8 intid = 0;
	int state;

	if (hs_hook->enable == 0)return IRQ_HANDLED;
	
	intid = twl6040_reg_read(twl6040, TWL6040_REG_INTID);
	//pr_info(KERN_INFO
    //           "twl6040_audio_hook_handler 0x03=0x%X......\n",intid);
	if (intid & TWL6040_HOOKINT) {
		//mutex_lock(&priv->mutex);

		state =twl6040_read_reg_volatile(codec, TWL6040_REG_STATUS) & 0x01 ; //TWL6040_HKCOMP
		//pr_info("twl6040_hs_hook_report state=%d\n",state);	   
		//schedule_delayed_work(&priv->check_hook_delayed_work, msecs_to_jiffies(1000));
...
static void twl6040_enable_hook_work(struct work_struct *work)
{
	struct twl6040_data *priv = container_of(work,
					struct twl6040_data, hook_enable_delayed_work.work);

	struct snd_soc_codec *codec = priv->codec;	
	struct twl6040_hook_data *hs_hook = &priv->hs_hook;
	int status,readState=0, state = 0;
	struct twl6040 *twl6040 = codec->control_data;

#if defined(CONFIG_SND_OMAP_ENABLE_HS_MIC_ADC_DETECT_FUNCTION)
	if (priv->codec_powered == 0){
		status = twl6040_reg_read(twl6040, TWL6040_REG_LDOCTL);			
		status |= TWL6040_REFENA;
		twl6040_reg_write(twl6040, TWL6040_REG_LDOCTL, status);
	}

	readState = twl6040_reg_read(twl6040, TWL6040_REG_AMICBCTL);
	...
	twl6040_write(codec, TWL6040_REG_AMICBCTL,readState); 	
#else
	/* Sync status */
	//twl6040_write(codec, TWL6040_REG_HKCTL1,0xd9);	
	status = twl6040_read_reg_volatile(codec, TWL6040_REG_STATUS);
	state=status & TWL6040_PLUGCOMP;
	twl6040_write(codec, TWL6040_REG_HKCTL1,0xd9);
#endif	
	schedule_delayed_work(&priv->check_hook_delayed_work, msecs_to_jiffies(1000));
...
static void twl6040_check_hook_work(struct work_struct *work)
{
	struct twl6040_data *priv = container_of(work,
					struct twl6040_data, check_hook_delayed_work.work);
	...
	if (hs_hook->enable == 0)return ;
#if defined (CONFIG_SND_OMAP_ENABLE_HS_MIC_ADC_DETECT_FUNCTION)
        state = twl6030_get_gpadc_conversion(2);
	//schedule_delayed_work(&priv->hook_enable_delayed_work, msecs_to_jiffies(1000));		
	readState = twl6040_reg_read(twl6040, TWL6040_REG_AMICBCTL);
        readState&=(~0x05);	
	twl6040_write(codec, TWL6040_REG_AMICBCTL,readState); 	
	if (priv->codec_powered==0){
		readState = twl6040_reg_read(twl6040, TWL6040_REG_LDOCTL);		
		readState &= (~TWL6040_REFENA);
		twl6040_reg_write(twl6040, TWL6040_REG_LDOCTL, readState);
	}	
	if (state<2500 && state >800 ){ 
		//iphone headset value 2045 ,sumsung 1732
		pr_info("HS Jack has microphone.....(%d)\n",state);
		snd_soc_jack_report(hs_jack->jack, SND_JACK_HEADSET, SND_JACK_HEADSET);
		if (&priv->hs_jack.sdev)
			switch_set_state(&priv->hs_jack.sdev, 0x01);//BIT_HEADSET =0x01
	}else{
		pr_info("HS Jack doesn't have a microphone.....(%d)\n",state);
		snd_soc_jack_report(hs_jack->jack, SND_JACK_HEADPHONE,SND_JACK_HEADSET);
		if (&priv->hs_jack.sdev)
			switch_set_state(&priv->hs_jack.sdev, 0x02);  //BIT_HEADSET_NO_MIC =0x02
	}
#else
	state =twl6040_read_reg_volatile(codec, TWL6040_REG_STATUS) & 0x01 ; //TWL6040_HKCOMP
    if (state == 0x00){
		pr_info("HS Jack has microphone.....\n");
		snd_soc_jack_report(hs_jack->jack, SND_JACK_HEADSET, SND_JACK_HEADSET);
		if (&priv->hs_jack.sdev)
			switch_set_state(&priv->hs_jack.sdev, 0x01);//BIT_HEADSET =0x01
	}else{
		pr_info("HS Jack doesn't have a microphone.....\n");
		snd_soc_jack_report(hs_jack->jack, SND_JACK_HEADPHONE,SND_JACK_HEADSET);
		if (&priv->hs_jack.sdev)
			switch_set_state(&priv->hs_jack.sdev, 0x02);  //BIT_HEADSET_NO_MIC =0x02
	}	
	...
	pr_info("HS Jack Amp power pin number:%d.\n",CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN);
        if (hs_hook->amp_power_on == 1){	
	    pr_info("We need the HS Jack Amp power!\n");
		
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)		   		
            if(hs_ring_mode ==1){
                pr_info("Ring mode plug in the headset mic!!!\n");
                headset_amp_power_event(1);
            }
#endif		  
        }else{
				   ...
				   struct snd_soc_jack *jack, int report)
{
	...
	/* Sync status */
	status = twl6040_read_reg_volatile(codec, TWL6040_REG_STATUS);
	//Laker: Plug-in: PLUGDET / HS_DET is low (state=1) ; Plug-out: PLUGDET / HS_DET is not low (state=0)
	if (status & TWL6040_PLUGCOMP)
		state = report;
	
//Laker: Plug-in: PLUGDET / HS_DET is not low (state=0) ; Plug-out: PLUGDET / HS_DET is low (state=1)
#if defined (CONFIG_MACH_OMAP4_ZEUS) || defined (CONFIG_MACH_OMAP4_SAGA)
	state=(state)?0:SND_JACK_HEADPHONE;
#elif defined(CONFIG_MACH_OMAP4_PB1ICOM)
	state=(state)?0:SND_JACK_HEADSET; //enable headset mic as well
#endif
	
	
#if defined(CONFIG_SND_OMAP_ENABLE_HS_MIC_DETECT_FUNCTION)
{
	//struct twl6040 *twl6040 = codec->control_data;
	struct twl6040_hook_data *hs_hook = &priv->hs_hook;
      ...
      if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
           if (gpio_get_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN)){	
             hs_hook->amp_power_on=true;
           }
	...
	if (state){   
		hs_hook->enable=1;
		schedule_delayed_work(&priv->hook_enable_delayed_work, msecs_to_jiffies(10));
	}else{
		hs_hook->enable=0;
		schedule_delayed_work(&priv->hook_enable_delayed_work, msecs_to_jiffies(1));
	}	
}
#endif
...
#if defined(CONFIG_SND_OMAP_ENABLE_HS_MIC_DETECT_FUNCTION)
    if(state == 0){
		snd_soc_jack_report(jack, state,SND_JACK_HEADSET);
		if (&priv->hs_jack.sdev )
			switch_set_state(&priv->hs_jack.sdev, state?2:0);  //BIT_HEADSET_NO_MIC =0x02
    }
#else
...
static const char *twl6040_headset_amp_power_texts[] = {
	"Off", "On",
};
static const struct soc_enum twl6040_headset_amp_power_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(twl6040_headset_amp_power_texts),
			twl6040_headset_amp_power_texts);
...
static int twl6040_headset_amp_power_get_enum(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
      if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
	    ucontrol->value.enumerated.item[0] = gpio_get_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN);
      	}else{
...
static int twl6040_headset_amp_power_put_enum(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int control = ucontrol->value.enumerated.item[0];
	int ret;

      if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
           ret=gpio_get_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN);
           printk("Headset:old:%d",ret);

	     gpio_set_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, control);	
           printk(", new:%d\n",control);
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)		   
           headset_amp_power_event(control);
           hs_ring_mode=control;
#endif
      }
...
static const struct snd_kcontrol_new twl6040_snd_controls[] = {
	...
	SOC_ENUM_EXT("Headset Amp Power", twl6040_headset_amp_power_enum,
		twl6040_headset_amp_power_get_enum,
		twl6040_headset_amp_power_put_enum)	
...
#if defined (CONFIG_MACH_OMAP4_QUARTZ)
	SND_SOC_DAPM_SUPPLY("AUXL Boost Supply", SND_SOC_NOPM, 0, 0,
			 twl6040_auxl_boost_event,
			 SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),

	SND_SOC_DAPM_SUPPLY("AUXR Boost Supply", SND_SOC_NOPM, 0, 0,
			 twl6040_auxr_boost_event,
			 SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
#endif			 
...
#if defined (CONFIG_MACH_OMAP4_QUARTZ)
	{"AUXL Boost Supply", NULL, "Aux Left Playback"},
	{"AUXR Boost Supply", NULL, "Aux Right Playback"},
	
	{"AUXL", NULL, "AUXL Boost Supply"},
	{"AUXR", NULL, "AUXR Boost Supply"},
#else
...
static int twl6040_probe(struct snd_soc_codec *codec)
{
...
#ifndef CONFIG_MACH_OMAP4_QUARTZ
	if (priv->vddhf_reg) {
		pr_info("%s:vddhf\n", dev_name(codec->dev));
    ...
    if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
	    BUG_ON(gpio_request(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, "headset_mic_power") < 0);
	    gpio_direction_output(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, 0);
    }

	INIT_DELAYED_WORK(&priv->hook_enable_delayed_work, twl6040_enable_hook_work);
	INIT_DELAYED_WORK(&priv->check_hook_delayed_work, twl6040_check_hook_work);
	...
	if (ret) {
		pr_info("Hook PLUG IRQ request failed: %d\n", ret);
		goto hook_irq_err;
	}
...
#if defined (CONFIG_MACH_OMAP4_QUARTZ)
	if (gpio_request(GPIO_AUDIO_AMP_EN, "AUDIO_AMP_EN")) {
		pr_info("[Audio]Could not request GPIO %d\n", GPIO_AUDIO_AMP_EN);
		return 0;
	}
	gpio_direction_output(GPIO_AUDIO_AMP_EN, 0);
#endif	
