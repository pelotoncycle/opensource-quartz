...
#include <mach/gpio.h>
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)
   int snd_omap_hook_key_gpio_enhancement = 1;
#endif
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)
...
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)
#define HEADSET_HOOK_DECT_GPIO 51
struct delayed_work	twl4030_headset_gpio_hook;
struct wake_lock         wake_lock_hook_irq;
#endif
struct delayed_work	twl4030_headset_hook_polling;
int headsetFirstTime=0;
int headsetBiasTotal=0;
int heasetBiasEnabled=0;
int release_key=0;
int headset_ring_mode=0;
static struct input_dev *hookbutton_dev;
#endif
				...
				struct snd_pcm_hw_params *params, int flag)
{
	...
	int channels;
#if 0
	int ch=2,pcm_len=16,rate=16000; // ch*pcm_len*rate < 19200K/div
	int div = 18; // 19200K/div(19200k/37.5=512k)//0004461: [FR] shodow modem pcm function (not ready)
#endif
...
#else
            /*Relationship file : omap-mcbsp.c ,mcbsp.c*/
/* For PCM interface ,but it has uplink voice issue			
			ret = snd_soc_dai_set_fmt(modem_rtd->cpu_dai,
					  SND_SOC_DAIFMT_DSP_B |
					  SND_SOC_DAIFMT_NB_NF |
					  SND_SOC_DAIFMT_CBS_CFS);
*/		
	        /*I2S interface ,4430 is master mode*/
			ret = snd_soc_dai_set_fmt(modem_rtd->cpu_dai,
					  SND_SOC_DAIFMT_I2S |
					  SND_SOC_DAIFMT_NB_NF |
					  SND_SOC_DAIFMT_CBS_CFS);		
			...
			ret = snd_soc_dai_set_sysclk(modem_rtd->cpu_dai, OMAP_MCBSP_SYSCLK_CLKS_EXT,
							 ch*pcm_len*rate*div, SND_SOC_CLOCK_OUT); //FCLK =16k
			if (ret < 0) {
				printk(KERN_ERR "can't set DMIC cpu system clock\n");
				return ret;
			}
			
			ret = snd_soc_dai_set_clkdiv(modem_rtd->cpu_dai, OMAP_MCBSP_CLKGDV, div);//CLK			
			if (ret < 0) {
...
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)
int get_gpadc_conversion(int channel_no)
{
...
static irqreturn_t hookbutton_irq(int irq, void *dev_id)
{
	wake_lock(&wake_lock_hook_irq);	   	
	schedule_delayed_work(&twl4030_headset_gpio_hook, msecs_to_jiffies(1));
	wake_unlock(&wake_lock_hook_irq);
	return IRQ_HANDLED;
}


static void twl4030_headset_gpio_hook_func(struct work_struct *work)
{
	int temp=0;
	int hookkey=0;	

      temp=get_gpadc_conversion(2);
      hookkey=gpio_get_value(HEADSET_HOOK_DECT_GPIO);
	  
	if (!headsetFirstTime && hookkey ==1) {
		headsetFirstTime++;
  	      pr_info("Error irq hook key..\n");
		return;		
	}
#if 0	  
	
/*Send hook key*/
      if (temp <80  && hookkey == 0){
           pr_info("key down...\n");	  	
           input_report_key(hookbutton_dev, KEY_KP1,0);	
      	}
      if (temp >1000 && hookkey == 1){
           pr_info("key up\n");	  		  	
           input_report_key(hookbutton_dev, KEY_KP1,1);		
      	}
      input_sync(hookbutton_dev);	
#endif
      pr_info("[sdp4430] Headset hook key first:%d adc:%d gpio:%d\n",headsetFirstTime,temp,hookkey);		
  	
	if (hookkey == 0)	  
	    schedule_delayed_work(&twl4030_headset_hook_polling, msecs_to_jiffies(1));	
	else{
          pr_info("cancel\n");
          heasetBiasEnabled=0;
          headsetBiasTotal =0;
          if (release_key == 1)
          {
               pr_info("Send Hook key up \n");
               input_report_key(hookbutton_dev, KEY_KP1,0);
               input_sync(hookbutton_dev);			
		  release_key =0;
	    }		  
          cancel_delayed_work_sync(&twl4030_headset_hook_polling);		  
	}
}	

#endif
static int headset_hook_button_init(void)
{
	int err = 0;
	
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)		
    if(snd_omap_hook_key_gpio_enhancement){
	    /* PWRBTN == PWRON */
	    if (gpio_request(HEADSET_HOOK_DECT_GPIO, "headset-hook") < 0){
		    printk(KERN_ERR "Unable to allocate GPIO for power button\n");
		    err = -EBUSY;
		    goto out;
	    }
	
           gpio_direction_input(HEADSET_HOOK_DECT_GPIO);	
           gpio_set_debounce(HEADSET_HOOK_DECT_GPIO,GPIO_DEBOUNCE_TIME);
	  
	     if (request_irq(OMAP_GPIO_IRQ(HEADSET_HOOK_DECT_GPIO), hookbutton_irq, IRQF_TRIGGER_FALLING |IRQF_TRIGGER_RISING ,
			"headsetHookButton", NULL) < 0) {
		    printk(KERN_ERR "Unable to allocate IRQ for headset hook button\n");
		    err = -EBUSY;
		    goto free_irq_and_out;
	     }
            disable_irq(OMAP_GPIO_IRQ(HEADSET_HOOK_DECT_GPIO));
    }
#endif	
	hookbutton_dev = input_allocate_device();
	if (!hookbutton_dev) {
		printk(KERN_ERR
			"Unable to allocate input device for headset hook button\n");
		err = -ENOMEM;
		goto free_input_dev;
	}

	hookbutton_dev->evbit[0] = BIT_MASK(EV_SYN) |BIT_MASK(EV_KEY);
	hookbutton_dev->keybit[BIT_WORD(KEY_KP1)] = BIT_MASK(KEY_KP1);
	hookbutton_dev->name = "headset-hook-button";
	err = input_register_device(hookbutton_dev);
	if (err) {
		input_free_device(hookbutton_dev);
		goto free_input_dev;
	}
	
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)	
    if(snd_omap_hook_key_gpio_enhancement){
        wake_lock_init(&wake_lock_hook_irq, WAKE_LOCK_SUSPEND,"hook-irq");
    } 
#endif
	return 0;
free_input_dev:
	input_unregister_device(hookbutton_dev);
	
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)	
free_irq_and_out:
    if(snd_omap_hook_key_gpio_enhancement){
        free_irq(OMAP_GPIO_IRQ(HEADSET_HOOK_DECT_GPIO), NULL);
    }
out:
#endif
	return err;
}
static void twl4030_polling_headset_hook_func(struct work_struct *work)
{
	int temp=0;
	temp=get_gpadc_conversion(2);
	
	if (headsetFirstTime ==0 && temp <100) {
  	      pr_info("Error polling hook key..\n");
             schedule_delayed_work(&twl4030_headset_hook_polling, msecs_to_jiffies(100));		  
		return;		
	}else{
		headsetFirstTime =1;
	}
	
	headsetBiasTotal += temp;
	if (temp < 100 ){
		heasetBiasEnabled++;
		if (heasetBiasEnabled >1 ){
			if ( (headsetBiasTotal/heasetBiasEnabled) <100){
				headsetBiasTotal =0;
				heasetBiasEnabled =0;
				if (release_key ==0){
				  pr_info("Send Hook key down \n");
				  input_report_key(hookbutton_dev, KEY_KP1,1);	
                                  input_sync(hookbutton_dev);
				  release_key =1;
				}else{
				  pr_info("waitting release hook key....");                            
				}
			}			
		}
	}
	...
	//pr_info("Headset hook key vol:%d total:%d cont:%d \n",temp,headsetBiasTotal,heasetBiasEnabled);	 
	return;
}
#endif
...
{
	int ret=0;
	if(!machine_is_omap4_talos7() && !machine_is_omap4_talos10() && !machine_is_omap4_puzzle() &&
		!machine_is_omap4_zeus() && !machine_is_omap4_olympus() && !machine_is_innocomm_oracle() && 
        !machine_is_omap4_quartz() && !machine_is_omap4_saga() && !machine_is_omap4_pb1icom() && !machine_is_omap4_winmate())
{
...
{
	if (SND_SOC_DAPM_EVENT_ON(event) ){
        if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
		
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)		
             if (headset_ring_mode){
                printk("ring mode\n");
                headsetFirstTime=0;				
                return 0;
             }	
#endif			 
		printk(KERN_INFO "Headset mic power on!! \n");	 
		gpio_set_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, true);
...
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)				
        if (snd_omap_hook_key_gpio_enhancement){					
            enable_irq(OMAP_GPIO_IRQ(HEADSET_HOOK_DECT_GPIO));
        }else{
            schedule_delayed_work(&twl4030_headset_hook_polling, msecs_to_jiffies(100));	
        }
#else
        ...
        if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
		printk(KERN_ERR "Headset mic power off!! \n");
		
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)						
        if(snd_omap_hook_key_gpio_enhancement){
            cancel_delayed_work_sync(&twl4030_headset_gpio_hook);
            disable_irq(OMAP_GPIO_IRQ(HEADSET_HOOK_DECT_GPIO));
        }
#endif	
        ...
        headset_ring_mode =0;
		if (release_key ==1){
             pr_info("Send Hook key down -finish\n");
             input_report_key(hookbutton_dev, KEY_KP1,0);	
             input_sync(hookbutton_dev);	
	         release_key=0;
		}
...
#endif		
		gpio_set_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, false);
...
extern  int headset_amp_power_event( int event)
{
	if (event){
        if (CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN >0){
		printk(KERN_INFO "Ring mode::Headset mic power on\n");	 
		gpio_set_value(CONFIG_SND_OMAP_HEADSET_MIC_AMP_POWER_PIN, true);
        ...
        if (snd_omap_hook_key_gpio_enhancement){					
            ...
            schedule_delayed_work(&twl4030_headset_hook_polling, msecs_to_jiffies(1000));	
        }  
#else
        schedule_delayed_work(&twl4030_headset_hook_polling, msecs_to_jiffies(1000));	
#endif		
...
#endif		
		printk(KERN_ERR "Ring mode ::Headset mic power off\n");
...
#if defined (CONFIG_SND_OMAP_HEADSET_HOOK_KEY)
	headset_hook_button_init();
	INIT_DELAYED_WORK(&twl4030_headset_hook_polling,twl4030_polling_headset_hook_func);	
	
#if defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)									
    if (snd_omap_hook_key_gpio_enhancement){					
        INIT_DELAYED_WORK(&twl4030_headset_gpio_hook,twl4030_headset_gpio_hook_func);		
    }
#endif	
#endif
	if (machine_is_omap_4430sdp() || machine_is_omap_tabletblaze()
		|| machine_is_omap4_panda() || machine_is_omap4_talos7()
		|| machine_is_omap4_talos10() || machine_is_omap4_zeus()
		|| machine_is_omap4_olympus() ||machine_is_innocomm_oracle()
		|| machine_is_omap4_puzzle() || machine_is_omap4_quartz()
        || machine_is_omap4_saga() || machine_is_omap4_pb1icom() || machine_is_omap4_winmate())
...
static int __init sdp4430_soc_init(void)
{
	...
	if (!machine_is_omap_4430sdp() && !machine_is_omap4_panda() &&
		!machine_is_omap_tabletblaze() && !machine_is_omap4_talos7() && 
		!machine_is_omap4_talos10() && !machine_is_omap4_zeus() && 
		!machine_is_omap4_olympus() && !machine_is_innocomm_oracle() && 
		!machine_is_omap4_puzzle() && !machine_is_omap4_quartz() && 
        !machine_is_omap4_saga() && !machine_is_omap4_pb1icom() && !machine_is_omap4_winmate()) {
		
		pr_debug("Not SDP4430, BlazeTablet ,Talos, Puzzle, PandaBoard or PB1icom , winmate !\n");
		return -ENODEV;
	}
	...
	else if (machine_is_omap4_talos7())
		snd_soc_sdp4430.name = "Talos7";
	else if (machine_is_omap4_talos10())
		snd_soc_sdp4430.name = "Talos10";
	else if (machine_is_omap4_puzzle())
		snd_soc_sdp4430.name = "Puzzle";
	else if (machine_is_omap4_quartz())
		snd_soc_sdp4430.name = "Quartz";
	else if (machine_is_omap4_saga())
		snd_soc_sdp4430.name = "Saga";
	else if (machine_is_omap4_zeus())
		snd_soc_sdp4430.name = "Zeus";
	else if (machine_is_omap4_olympus())
		snd_soc_sdp4430.name = "Olympus";
	else if (machine_is_innocomm_oracle())	
		snd_soc_sdp4430.name = "Oracle";
	else if (machine_is_omap4_pb1icom())
		snd_soc_sdp4430.name = "PB1icom";
	else if (machine_is_omap4_winmate())
		snd_soc_sdp4430.name = "Winmate";
	...
	if(!machine_is_omap4_talos7() && !machine_is_omap4_talos10() &&
		!machine_is_omap4_zeus() && !machine_is_omap4_olympus()
		&&!machine_is_innocomm_oracle() && !machine_is_omap4_puzzle()
		&&!machine_is_omap4_quartz() && !machine_is_omap4_saga()
...
#if defined (CONFIG_MACH_OMAP4_SAGA) && defined (CONFIG_SND_OMAP_HOOK_KEY_GPIO_ENHANCEMENT)
    if (system_rev<3)
        snd_omap_hook_key_gpio_enhancement=0;
    else   	 
        snd_omap_hook_key_gpio_enhancement=1;
#endif
	return ret;
