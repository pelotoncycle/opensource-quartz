...
static unsigned long suspend_sys_sync_waiting;
...
static void suspend_sys_sync_handler(unsigned long arg)
{
	if (suspend_sys_sync_count == 0) {
		...
		suspend_sys_sync_waiting += SUSPEND_SYS_SYNC_TIMEOUT;
		if (suspend_sys_sync_waiting > (5 * HZ - 1)) {
			if ((suspend_sys_sync_waiting % HZ) == 0)
				pr_info("suspend_sys_sync: %lu secs\n", suspend_sys_sync_waiting / HZ);
		}
...
#endif
	proc_create("wakelocks_active", S_IRUGO, NULL, &active_wakelock_stats_fops);
