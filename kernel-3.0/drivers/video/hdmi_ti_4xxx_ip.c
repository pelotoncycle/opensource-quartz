...
int hdmi_ti_4xxx_phy_init(struct hdmi_ip_data *ip_data, int phy)
{
	...
	if(machine_is_omap4_wedjat() || machine_is_omap4_talos7() || machine_is_omap4_talos10() || machine_is_omap4_puzzle() ||
		machine_is_omap4_zeus() || machine_is_omap4_olympus()|| machine_is_innocomm_oracle() || machine_is_omap4_quartz() || 
        machine_is_omap4_saga() || machine_is_omap4_pb1icom() || machine_is_omap4_winmate())
		REG_FLD_MOD(hdmi_phy_base(ip_data), HDMI_TXPHY_PAD_CFG_CTRL, 0xF, 30, 27);
	else
	
		REG_FLD_MOD(hdmi_phy_base(ip_data), HDMI_TXPHY_PAD_CFG_CTRL, 0x1, 27, 27);
