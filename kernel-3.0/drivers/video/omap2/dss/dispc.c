...
static struct {
	...
	struct delayed_work	runtime_put_work;
...
{
	if (channel == OMAP_DSS_CHANNEL_LCD2) {
		REG_FLD_MOD(DISPC_CONTROL2, 0x3, 7, 6);
	} else {
		REG_FLD_MOD(DISPC_CONTROL, 0x3, 7, 6);
	}
}
...
#else
	DSSDBG("PCLK %uKHz, HSYNC %luHz, VSYNC %luHz\n", timings->pixel_clock, ht, vt);
#endif
}
...
#ifdef CONFIG_FB_OMAP_BOOTLOADER_INIT
static void dispc_runtime_put_workfunc(struct work_struct *work)
{
	dispc_save_context();
	dispc_runtime_put();
	DSSINFO("delayed runtime is released\n");
}
#endif /* CONFIG_FB_OMAP_BOOTLOADER_INIT */
...
static int omap_dispchw_probe(struct platform_device *pdev)
{
	...
	if (videolfb_is_enabled()) {
		INIT_DELAYED_WORK_DEFERRABLE(&dispc.runtime_put_work, dispc_runtime_put_workfunc);
		schedule_delayed_work_on(0, &dispc.runtime_put_work, msecs_to_jiffies(12500));
	} else
		dispc_runtime_put();
#else
	dispc_runtime_put();
