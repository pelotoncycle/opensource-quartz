			...
			u32 y_nuv, struct sa_struct *sa_info)
{
	...
	if (i == 0) {
		 ...
		 */
#if defined(CONFIG_OMAP2_FIFO_LOW_THRESHOLD) && (CONFIG_OMAP2_FIFO_LOW_THRESHOLD > 0)
		sa_info->min_sa = CONFIG_OMAP2_FIFO_LOW_THRESHOLD * pict_16word + C2 * (Tot_mem -
						pict_16word_ceil - 8);
#else
		sa_info->min_sa = 2 * pict_16word + C2 * (Tot_mem -
						pict_16word_ceil - 8);
#endif /* CONFIG_OMAP2_FIFO_LOW_THRESHOLD */
	} else {
		/* All other cases */
