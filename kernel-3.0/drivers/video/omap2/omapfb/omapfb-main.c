...
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=2d32240422582ee6b83119c81fb189baa8bc3075
 * OMAP4: OMAPFB: Always set a x_decim and y_decim in overlays
 * When using the framebuffer API to setup overlay, there's
 * currently no way to pass a x_decim and y_decim value in
 * so set some safe non-zero defaults.  Otherwise, when
 * the overlay is enabled, zero values will cause divide
 * by zero error in dispc.
 */
	/*
...
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=e67f0202e3ec0fc8cb3338ab935c0d6ead224ea9
