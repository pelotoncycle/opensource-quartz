...
static int tc358765_write_init_config(struct omap_dss_device *dssdev)
{
	...
	if (board_data->board_init) {
		...
		if (r) {
			dev_err(&dssdev->dev, "%pf returns err %d\n", board_data->board_init, r);
			return r;
		}
	}
...
static void tc358765_disable(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->state == OMAP_DSS_DISPLAY_ACTIVE) {
		...
		tc358765_power_off(dssdev);
		dsi_bus_unlock(dssdev);
		dev_info(&dssdev->dev, "%s: disabled\n", __func__);
	} else if (dssdev->state == OMAP_DSS_DISPLAY_SUSPENDED) {
		dssdev->state = OMAP_DSS_DISPLAY_DISABLED;
		dev_info(&dssdev->dev, "%s: was suspended\n", __func__);
	} else if (dssdev->state == OMAP_DSS_DISPLAY_DISABLED) {
		if (dssdev->skip_init) {
			dev_info(&dssdev->dev, "%s: skip init\n", __func__);
			dsi_bus_lock(dssdev);
			if (tc358765_power_on(dssdev) == 0)
				tc358765_power_off(dssdev);
			dsi_bus_unlock(dssdev);
		} else
			dev_info(&dssdev->dev, "%s: was disabled\n", __func__);
	}
...
static int tc358765_enable(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->state == OMAP_DSS_DISPLAY_ACTIVE) {
		dev_info(&dssdev->dev, "%s: was enabled\n", __func__);
	} else {
		dsi_bus_lock(dssdev);
		r = tc358765_power_on(dssdev);
		dsi_bus_unlock(dssdev);
		if (r) {
			dssdev->state = OMAP_DSS_DISPLAY_DISABLED;
			dev_info(&dssdev->dev, "%s: enable failed %d\n", __func__, r);
		} else {
			dssdev->state = OMAP_DSS_DISPLAY_ACTIVE;
			dev_info(&dssdev->dev, "%s: enabled\n", __func__);
		}
	}
...
static int tc358765_suspend(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->state == OMAP_DSS_DISPLAY_ACTIVE) {
		...
		tc358765_power_off(dssdev);
		dsi_bus_unlock(dssdev);
		dev_info(&dssdev->dev, "%s: suspended\n", __func__);
	} else if (dssdev->state == OMAP_DSS_DISPLAY_SUSPENDED) {
		dev_info(&dssdev->dev, "%s: was suspended\n", __func__);
	} else if (dssdev->state == OMAP_DSS_DISPLAY_DISABLED) {
		dev_info(&dssdev->dev, "%s: was disabled\n", __func__);
...
static int tc358765_resume(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->state == OMAP_DSS_DISPLAY_SUSPENDED) {
		...
		if (r) {
			dssdev->state = OMAP_DSS_DISPLAY_SUSPENDED;
			dev_info(&dssdev->dev, "%s: resume failed %d\n", __func__, r);
		} else {
			dssdev->state = OMAP_DSS_DISPLAY_ACTIVE;
			dev_info(&dssdev->dev, "%s: resumed\n", __func__);
		}
