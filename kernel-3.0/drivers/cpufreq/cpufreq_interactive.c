...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE
static unsigned int android_screen_off;
#endif /* CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE */
...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE
	if (android_screen_off)
		new_tune_value = SCREENOFF_TUNE;
	else
#endif /* CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE */
...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE
	if (android_screen_off) {
		if (cur_tune_value != SCREENOFF_TUNE) {
#if (CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_MIN_SAMPLE_TIME % DEFAULT_MIN_SAMPLE_TIME != 0)
#error "Invalid CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_MIN_SAMPLE_TIME"
#endif
			go_hispeed_load = CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_GO_HISPEED_LOAD;
			min_sample_time = CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_MIN_SAMPLE_TIME;
			cpufreq_frequency_table_target(pcpu->policy,
				pcpu->freq_table, pcpu->policy->min,
				CPUFREQ_RELATION_H, &index);
#if (CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_HISPEED_INDEX <= 0)
#error "Invalid CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_HISPEED_INDEX"
#endif
			hispeed_freq =
				pcpu->freq_table[index+CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE_HISPEED_INDEX].frequency;
			cur_tune_value = SCREENOFF_TUNE;
		}
	} else {
#endif /* CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE */
		...
		if ((max_total_avg_load > hi_perf_threshold)
				...
				cur_tune_value = HIGH_PERF_TUNE;
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_HIGHPERF_TUNE
#if (CONFIG_CPU_FREQ_GOV_INTERACTIVE_HIGHPERF_TUNE_MIN_SAMPLE_TIME % DEFAULT_MIN_SAMPLE_TIME != 0)
#error "Invalid CONFIG_CPU_FREQ_GOV_INTERACTIVE_HIGHPERF_TUNE_MIN_SAMPLE_TIME"
#endif
				go_hispeed_load = CONFIG_CPU_FREQ_GOV_INTERACTIVE_HIGHPERF_TUNE_GO_HISPEED_LOAD;
				min_sample_time = CONFIG_CPU_FREQ_GOV_INTERACTIVE_HIGHPERF_TUNE_MIN_SAMPLE_TIME;
#else
...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE
#if (CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_MIN_SAMPLE_TIME% DEFAULT_MIN_SAMPLE_TIME != 0)
#error "Invalid CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_MIN_SAMPLE_TIME"
#endif
				go_hispeed_load = CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_GO_HISPEED_LOAD;
				min_sample_time = CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_MIN_SAMPLE_TIME;
				cpufreq_frequency_table_target(pcpu->policy,
					pcpu->freq_table, pcpu->policy->min,
					CPUFREQ_RELATION_H, &index);
#if (CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_HISPEED_INDEX <= 0)
#error "Invalid CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_HISPEED_INDEX"
#endif
				hispeed_freq =
					pcpu->freq_table[index+CONFIG_CPU_FREQ_GOV_INTERACTIVE_LOWPOWER_TUNE_HISPEED_INDEX].frequency;
				cur_tune_value = LOW_POWER_TUNE;
#else
...
static void cpufreq_interactive_earlysuspend(struct early_suspend *h)
{
	unsigned long flags;

	spin_lock_irqsave(&tune_cpumask_lock, flags);
	android_screen_off = 1;
	cpumask_set_cpu(cpumask_first(cpu_online_mask), &tune_cpumask);
	spin_unlock_irqrestore(&tune_cpumask_lock, flags);
	queue_work(tune_wq, &tune_work);
	pr_info("cpufreq_interactive: screen off\n");
}

static void cpufreq_interactive_lateresume(struct early_suspend *h)
{
	unsigned long flags;

	spin_lock_irqsave(&tune_cpumask_lock, flags);
	android_screen_off = 0;
	cpumask_set_cpu(cpumask_first(cpu_online_mask), &tune_cpumask);
	spin_unlock_irqrestore(&tune_cpumask_lock, flags);
	queue_work(tune_wq, &tune_work);
	pr_info("cpufreq_interactive: screen on\n");
}

static struct early_suspend cpufreq_interactive_earlysuspend_desc = {
	.level		= EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
	.suspend	= cpufreq_interactive_earlysuspend,
	.resume		= cpufreq_interactive_lateresume,
};
...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE
	register_early_suspend(&cpufreq_interactive_earlysuspend_desc);
#endif /* CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE */
...
#ifdef CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE
	unregister_early_suspend(&cpufreq_interactive_earlysuspend_desc);
#endif /* CONFIG_CPU_FREQ_GOV_INTERACTIVE_SCREENOFF_TUNE */
