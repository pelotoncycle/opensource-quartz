...
int asciitohex(int start, int end, u8 *value)
{
    int index = 0;
    bool next = false;
    ...
    while (start <= end)
    {
        switch (diffdate[start])
        {
            case '0': case '1': case '2': case '3': case '4': 
            case '5': case '6': case '7': case '8': case '9': 
                value[index] = (value[index] << 4) | (diffdate[start] & 0xf);
                next = true;
                break;
            case 'a': case 'b': case 'c': case 'd': case 'e': case 'f': 
            case 'A': case 'B': case 'C': case 'D': case 'E': case 'F': 
                value[index] = (value[index] << 4) | ( 9 + (diffdate[start] & 0xf) );
                next = true;
                break;
            default:
                if (next)
...
int asciitodec(int start, int end)
{
    int value=0;
    ...
    while (start<=end)
    {
        switch (diffdate[start])
        {
            case '0': case '1': case '2': case '3': case '4': 
            case '5': case '6': case '7': case '8': case '9': 
                value = value*10 + (diffdate[start] & 0xf);
                canreturn = true;
                break;
            default:
                if (canreturn)
...
unsigned char readdffsversion(struct bq27520_device_info *di)
{
    ...
    memset(RxData,0,sizeof(RxData));
    bq27520_read_block(di, RxData, 0x58, 1, false);
...
//    msleep(1000);
    ...
    msleep(1000);
//    printk("size: %d\n", sizeof(diffdate));
    while ((parsepos < sizeof(diffdate)) && (timeout > 0))
    {
        bool comparefail = false;
        u8 dffsdata[98];
        int lineend = parsepos;
        while(lineend < sizeof(diffdate))
        {
            if (diffdate[lineend] == 0x0d)
            {
                lineend+=2;
                break;
            }
...
//        printk("start:%d    end:%d\n", parsepos, lineend);
        ...
        {
            int x,y;
            switch(diffdate[parsepos]) {
                case 0x57:
                    y = asciitohex(parsepos+1, lineend, dffsdata);
                    if (y > 1)
                    {
#if 1
                        printk("W:");
                        for (x=0;x<y;x++) {
                           printk(" %02x", dffsdata[x]);
                        }
                        printk("\n");
#endif
                        bq27520_write_block(di, dffsdata+1,  dffsdata[1], y-2, true);
                    }
                    break;
                case 0x52:
                    y =asciitohex(parsepos+1, lineend, dffsdata);
                ...
                case 0x43:
...
#endif
                        memset(RxData,0,sizeof(RxData));
                        bq27520_read_block(di, RxData, dffsdata[1],  y-2, true);
#if 1
                        printk("           read:");
                        for (x=0;x<(y-2);x++) {
                            printk(" %02x", RxData[x]);
                    ...
                    break;
                case 0x58:
                    printk("X: %d\n", asciitodec(parsepos+1, lineend));
                    msleep(asciitodec(parsepos+1, lineend));
                    break;
                default:
                    printk("error happen\n");
                    parsepos++;
				...
				void *_data)
{
	...
	switch(cmd){
		case BCI_CONFIG:
		{
			struct bci_config_fuel_gauge *f=_data;	
			
			ret = writeBQ27520TermVolt(di, f->eco_current);
		...
		{
			struct bci_fuel_gauge_status_request *fg_req=_data;
			int cap;
			bq_dataram dr;

			if (!mNotupdatedata)
			{         
				readBQ27520Block(di, &dr);
				fg_req->data_valid=1;
			}			
			cap=dr.StateOfChg;
                                ...
                                struct device_attribute *attr, char *buf)
{
	...
	readBQ27520Version(di, &bqver);

    return sprintf(buf, "bq27520 firmware version[%04x]   Data flash version[%04x]\n", bqver.Fw_version, bqver.Df_version);
}
...
static struct attribute *bq27520_attr[] = {
	&dev_attr_bq_fw.attr,
	NULL
};
				 ...
				 const struct i2c_device_id *id)
{
	...
	twl6030_bci_function_register(BQ27520,&di->nb,
...
#endif
    ret = sysfs_create_group(&client->dev.kobj, &bq27520_attr_group);
    if (ret)
        dev_dbg(&client->dev, "could not create sysfs files\n");
...
#ifdef CONFIG_TWL6032_BCI_BATTERY
	twl6030_bci_function_unregister(BQ27520);
#else
