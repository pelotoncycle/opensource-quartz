...
static void bq2416x_set_eco_current(struct bq2416x_device_info *di,int curr)
{
	u8 curr_reg,value=0;
	bq2416x_read_byte(di, &curr_reg, REG_BATTERY_CURRENT);
	if(curr<50)curr=50;
...
#if 0 
	if (current_mA >1500)Voreg|=I_LIMIT_SEL_2A5;
	else Voreg&=~I_LIMIT_SEL_2A5;
#else //always set as 1.5A to protect adaptor
	Voreg&=~I_LIMIT_SEL_2A5;
#endif
...
static int bq2416x_charger_debug(struct bq2416x_device_info *di)	{
	...
	for(i=0;i<8;i++){
		...
		case REG_CONTROL_REGISTER:
			printk(KERN_INFO"%s, %s STAT output, %s EOC, %s Charge, %s HiZ\n",
		...
		case REG_BATTERY_VOLTAGE:
				printk(KERN_INFO"Regulation Voltage %dmV, In Curr limit: %s\n",
		...
		case REG_BATTERY_CURRENT:
				printk(KERN_INFO"Charge Curr: %dmA, Eoc Curr:%dmA\n",
					(read_reg[i]>>CHARGE_CURRENT_SHIFT)*75+550,
					(read_reg[i]&0x07)*50+50);
			break;
		case REG_DPM_VOLTAGE_AND_STATUS:
				printk(KERN_INFO"MINSYS_STATUS: %s , DPM_STATUS:%s, USB DPM %dmV, IN DPM %dmV\n",
		...
		case REG_SAFETY_LIMIT:
				printk(KERN_INFO"TMR2X: %s, BATGD:%s, Temp Sense: %s, safe timer: %s, %s\n",
				...
				void *_data)
{
	...
	switch(event){
		case BCI_SET_START_CHARGING:
			if(di->debug>0)
			dev_info(di->dev, "BCI_SET_START_CHARGING\n");
		...
		break;
		case BCI_SET_STOP_CHARGING:
			if(di->debug>0)
			dev_info(di->dev, "BCI_SET_STOP_CHARGING\n");
			if  (di->active == 1) {
		...
		case BCI_SET_CHARGE_CURRENT:
			{
				...
				if(di->debug>0)
				dev_info(di->dev, "BCI_SET_CHARGE_CURRENT %dmA\n",*current_mA);
		...
		case BCI_SET_EOC_CURRENT:
			{
				unsigned int *current_mA=_data;
				if(di->debug>0)
					dev_info(di->dev, "BCI_SET_EOC_CURRENT %dmA\n",*current_mA);
				bq2416x_set_eco_current(di,*current_mA);
		...
		case BCI_SET_INPUT_CURRENT_LIMIT:
			{
				unsigned int *current_mA=_data;
				if(di->debug>0)
					dev_info(di->dev, "BCI_SET_INPUT_CURRENT_LIMIT %dmA\n",*current_mA);
				
				di->max_current_mA=*current_mA;
				if(di->max_current_mA<=500){
				/*select usb path*/
					bq2416x_set_usb_current_limit(di, *current_mA);
					bq2416x_path_select(di,0);
				}else{
				/*select in path*/
					bq2416x_set_in_current_limit(di,*current_mA);
		...
		break;
		case BCI_SET_REGULATION_VOLTAGE:
			{
				unsigned int *voltage_mV=_data;
				
				if(di->debug>0)
					dev_info(di->dev, "BCI_SET_REGULATION_VOLTAGE %dmV\n",*voltage_mV);
				di->reg_voltage_mV=*voltage_mV;
				bq2416x_set_regulation_voltage(di,di->reg_voltage_mV);

			}
		break;
		case BCI_SET_INPUT_VOLTAGE_LIMIT:
			{
				unsigned int *voltage_mV=_data;
				if(di->debug>0)
					dev_info(di->dev, "BCI_SET_INPUT_VOLTAGE_LIMIT %dmV\n",*voltage_mV);
				
				di->max_voltage_mV=*voltage_mV;
			}
		break;
		case BCI_SET_EOC_ENABLE:
			if(di->debug>0)
			dev_info(di->dev, "BCI_SET_EOC_ENABLE\n");
			bq2416x_set_eoc_enable(di,1);
		break;
		case BCI_SET_EOC_DISABLE:
			if(di->debug>0)
			dev_info(di->dev, "BCI_SET_EOC_DISABLE\n");
		...
		case BCI_READ_FAULT_REASON:
			//if(di->debug>0)
			//    dev_info(di->dev, "BCI_READ_FAULT_REASON\n");
			{
				struct bci_read *status=_data;
				u8 state;
				bq2416x_read_block(di, &read_reg[0], 0, 8);
				...
				switch(state){
					...
					default:
						...
						if(di->debug>0)
  							printk(KERN_INFO"reason %x, hw %02x, safety %02x\n",status->reasons,read_reg[1],read_reg[7]);
					break;
				}
	
			}
		break;
		case BCI_SET_EXT_DEBUG_EN:
		    di->debug = (di->debug)?0:1;
			break;
			
		case BCI_DEBUG_MONITOR:
		if(_data){
			unsigned int *dbg=_data;
			di->debug=*dbg;
		}
		if(di->debug>0)
...
#if 0 //original
	ret = bq2416x_read_byte(di, &read_reg, REG_STATUS_CONTROL);
	//twl6030_register_notifier(&di->nb, read_reg&0x07);
	twl6030_register_bci_notifier(&di->nb, read_reg&0x07);
#else //test for register mask
#ifdef CONFIG_TWL603x_BCI_BATTERY
	twl6030_register_bci_notifier(&di->nb, 0x2FFF, NOTIFIER_REG_EXT_CHARGER);
#else
...
#ifdef CONFIG_TWL603x_BCI_BATTERY
	twl6030_unregister_bci_notifier(&di->nb, 1, NOTIFIER_REG_EXT_CHARGER);
#else
	twl6030_unregister_notifier(&di->nb, 1);
#endif
	kfree(di);
