...
/* New debounce time implementations */
#if 0
#define OMAP4_DEF_CTRL_PTV		(1 << 2)
#else
#define OMAP4_CTRL_PTV_SHIFT	2
#endif
...
/* The maximum debounce time for PTV 0x6 is 256 ms */
#define OMAP4_VAL_DEBOUNCINGTIME	64 /* ms */
#define OMAP4_VAL_PVT			0x6
#endif
...
#else
	__raw_writel(OMAP4_DEF_CTRL_NOSOFTMODE |
			(OMAP4_VAL_PVT << OMAP4_CTRL_PTV_SHIFT),
			keypad_data->base + OMAP4_KBD_CTRL);

	__raw_writel(KEY_TIME_VALUE(OMAP4_VAL_DEBOUNCINGTIME * 1000, OMAP4_VAL_PVT),
			keypad_data->base + OMAP4_KBD_DEBOUNCINGTIME);
#endif
...
static int __devinit omap4_keypad_probe(struct platform_device *pdev)
{
	...
	/* We CANNOT enable repeat event for Android platform */
	if (pdata->rep)
