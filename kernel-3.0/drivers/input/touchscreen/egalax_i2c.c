...
#else
#define EGALAX_DBG(level, fmt, args...)  do{ printk( KERN_DEBUG "[egalax_i2c]: " fmt, ## args); } while(0)
#endif
...
static void egalax_i2c_early_suspend(struct early_suspend *handler)
{
	...
	pdata = p_egalax_i2c_dev->client->dev.platform_data;
	...
	if (pdata->suspend_platform_hw) {
		ret = pdata->suspend_platform_hw(pdata);
		if (ret < 0) {
			EGALAX_DBG(DBG_SUSP, "%s suspend power failed\n", __func__);
		}
	}
}
...
static void egalax_i2c_early_resume(struct early_suspend *handler)
{
	...
	if (pdata->resume_platform_hw) {
		ret = pdata->resume_platform_hw(pdata);
		if (ret < 0) {
			EGALAX_DBG(DBG_SUSP, "%s resume power failed\n", __func__);
		}
	}
...
static void sendLoopback(struct i2c_client *client)
{
	...
	if(p_egalax_i2c_dev->TPType == TYPE_NONE) {
		ret = i2c_master_send(client, cmdbuf, MAX_I2C_LEN);
		if (ret > 0) {
			EGALAX_DBG(DBG_MODULE, "buf[%x][%x][%x][%x][%x][%x][%x][%x][%x][%x]\n", 
				cmdbuf[0], cmdbuf[1], cmdbuf[2], cmdbuf[3], cmdbuf[4],
				cmdbuf[5], cmdbuf[6], cmdbuf[7], cmdbuf[8], cmdbuf[9]);		
		}
	}
	else {
		ret = i2c_master_send(client, cmdbuf2, MAX_I2C_LEN);
		if (ret > 0) {
			EGALAX_DBG(DBG_MODULE, "buf[%x][%x][%x][%x][%x][%x][%x][%x][%x][%x]\n", 
				cmdbuf2[0], cmdbuf2[1], cmdbuf2[2], cmdbuf2[3], cmdbuf2[4],
				cmdbuf2[5], cmdbuf2[6], cmdbuf2[7], cmdbuf2[8], cmdbuf2[9]);		
		}
	}

	if (ret < 0) {
		EGALAX_DBG(DBG_MODULE, " %s error(%d)\n", __func__, ret);
	}
}

static int egalax_i2c_check(struct i2c_client *client)
{
	u8 cmdbuf[MAX_I2C_LEN]={0x03, 0x03, 0x0A, 0x01, 0x46, 0, 0, 0, 0, 0};
	int ret = -1;
	int retry = 0;
...
static int __devinit egalax_i2c_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	if (pdata->tp_probed) {
		EGALAX_DBG(DBG_MODULE, "tp already probed!\n");
		return -EPERM;
	}
	...
	if (pdata->init_platform_hw) {
		ret = pdata->init_platform_hw(pdata);
		if (ret < 0) {
			EGALAX_DBG(DBG_MODULE, "Power On Failed!!\n");
			goto fail1;
		}
	}

	ret = egalax_i2c_check(client);
	if (ret < 0) {
		EGALAX_DBG(DBG_MODULE, "tp not found, ret(%d)\n", ret);
		goto fail2;
	}
