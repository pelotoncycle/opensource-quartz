...
static void ft5x0x_reset(void)
{
	...
	gpio_set_value(ZEUS_TP_RESET, 0);
	mdelay(5);
	gpio_set_value(ZEUS_TP_RESET, 1);
	...
	for (id = 0, index = 0; id < MAX_FINGER_NUM; id++) {
		...
		if (current_track_id || previous_track_id) {
			if (just_lift) {
				...
				position = 3 + 6*index;			
				input_x = (unsigned int) (point_data[position+1]) + (unsigned int)((point_data[position] & 0x0f) << 8);
				input_y = (unsigned int)(point_data[position+3]) + (unsigned int) ((point_data[position+2] & 0x0f) << 8);
				input_w = 100;
				id_xy = point_data[position+2] >> 4;

				if (ts->just_resume) {
					if (input_x > ZEUS_TP_ABS_X_MAX || input_y > ZEUS_TP_ABS_Y_MAX) {
						dev_info(&this_client->dev, "irq data ignored!!!\n");
						memset((void *)track_id, -1, sizeof(track_id));
						memset((void *)track_id_last, -1, sizeof(track_id_last));
						goto I2C_READ_ERROR;
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ft5x0x_ts->irq) {
		ret = request_threaded_irq(ft5x0x_ts->irq, NULL, ft5x0x_irq_thread_fn, IRQF_TRIGGER_FALLING, ZEUS_TOUCH_NAME, ft5x0x_ts);
		if (ret == 0) {
	...
	input_dev->name	= ZEUS_TOUCH_NAME;
