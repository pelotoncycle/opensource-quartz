/* drivers/i2c/chips/elan_epl6804.c - light and proxmity sensors driver
...
static void elan_epl6804_register_dump(void);
#endif
...
static void elan_epl6804_sensor_chooser(int light_enable, int prox_enable);
static void elan_epl6804_lsensor_enable(struct elan_epl_data *epld);
static void elan_epl6804_psensor_enable(struct elan_epl_data *epld);
static void elan_epl6804_sensor_disable(struct elan_epl_data *epld);
...
static ssize_t elan_ls_adcresolution_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	elan_epl6804_I2C_Write(client,REG_1,R_SINGLE_BYTE,0x01,0x00);
	elan_epl6804_I2C_Read(client);
	regdata = gRawData.raw_bytes[0];
	regdata &= 0xFC; 
	regdata |= epld->epl_adc_item;

	elan_epl6804_I2C_Write(client,REG_1,W_SINGLE_BYTE,0X02,regdata);
...
static ssize_t elan_ls_channel01_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = elan_epl6804_I2C_Write(client, REG_1, R_SINGLE_BYTE, 0x01, 0x00);
	if (ret > 0) {
		ret = elan_epl6804_I2C_Read(client);
		if (ret > 0) {
			reg_1 = gRawData.raw_bytes[0];
		}
	}

	ret = elan_epl6804_I2C_Write(client, REG_13, R_SINGLE_BYTE, 0x01, 0x00);
	if (ret > 0) {
		ret = elan_epl6804_I2C_Read(client);
		if (ret > 0) {
	...
	ret = elan_epl6804_I2C_Write(client, REG_14, R_TWO_BYTE, 0x01, 0x00);
	if (ret > 0) {
		ret = elan_epl6804_I2C_Read(client);
		if (ret > 0) {
	...
	ret = elan_epl6804_I2C_Write(client, REG_16, R_TWO_BYTE, 0x01, 0x00);
	if (ret > 0) {
		ret = elan_epl6804_I2C_Read(client);
		if (ret > 0) {
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= 0x14; i++) {
		...
		ret = elan_epl6804_I2C_Write(client, i, R_SINGLE_BYTE, 0x01, 0x00);
		if (ret > 0) {
			ret = elan_epl6804_I2C_Read(client);
			if (ret > 0) {
...
static ssize_t elan_ls_enable_light_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (epld->light_flag) {
		elan_epl6804_sensor_chooser(1,-1); // light: true		
	} else {
		elan_epl6804_sensor_chooser(0,-1); // light: false
	} 
...
static ssize_t elan_ls_enable_prox_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (epld->prox_flag) {
		elan_epl6804_sensor_chooser(-1,1); // prox: true
	} else {
		elan_epl6804_sensor_chooser(-1,0); // prox: false
	} 
...
static int elan_epl6804_I2C_Write(struct i2c_client *client, uint8_t regaddr, uint8_t bytecount, uint8_t txbyte, uint8_t data)
{
	...
	{
		printk(KERN_ERR "[ELAN epl6804.c error] %s i2c write retry over %d\n",__func__, I2C_RETRY_COUNT);
		ret = -EINVAL; 
	}
...
static int elan_epl6804_I2C_Read(struct i2c_client *client)
{
	...
	if (retry >= I2C_RETRY_COUNT) {
		printk(KERN_ERR "[ELAN epl6804 error] %s retry over %d\n", __func__, I2C_RETRY_COUNT);
		ret = -EINVAL; 
		goto i2c_error;
	}
...
/* Through elan_epl6804_sensor_chooser to choose light or proximity sensor.
...
static void elan_epl6804_sensor_chooser(int light_enable, int prox_enable)
{
	...
	if (light_flag && !prox_flag) {
		elan_epl6804_lsensor_enable(epld);
		printk("epl6804: light sensor mode\n");
	} else if (!light_flag && prox_flag) {
		elan_epl6804_psensor_enable(epld);
		printk("epl6804: prox sensor mode\n");
	} else if (!light_flag && !prox_flag) {
		elan_epl6804_sensor_disable(epld);
		printk("epl6804: shut down power\n");
	} else {
		printk("epl6804: wait for updating status\n");
		msleep(2);
	}
}
...
static void elan_epl6804_psensor_enable(struct elan_epl_data *epld)
{
	...
	// reg1:
	elan_epl6804_I2C_Write(client,REG_1,W_SINGLE_BYTE,0x02,0x52);    
...
static int set_psensor_intr_threshold(uint16_t low_thd, uint16_t high_thd)
{
	...
	elan_epl6804_I2C_Write(client,REG_2,W_SINGLE_BYTE,0x02,high_lsb);
	elan_epl6804_I2C_Write(client,REG_3,W_SINGLE_BYTE,0x02,high_msb);
	elan_epl6804_I2C_Write(client,REG_4,W_SINGLE_BYTE,0x02,low_lsb);
	elan_epl6804_I2C_Write(client,REG_5,W_SINGLE_BYTE,0x02,low_msb);
...
static void polling_do_work(struct work_struct *work)
{
	...
	// Channel 0
	retw = elan_epl6804_I2C_Write(client, REG_14, R_TWO_BYTE, 0x01, 0x00);
	retr = elan_epl6804_I2C_Read(client);
	if (retr > 0 && retw > 0) {
		ch0 = ((gRawData.raw_bytes[1]<<8) | gRawData.raw_bytes[0]) ;
	}
	// Channel 1
	retw = elan_epl6804_I2C_Write(client, REG_16, R_TWO_BYTE, 0x01, 0x00);
	retr = elan_epl6804_I2C_Read(client);
	if (retr > 0 && retw > 0) {
		ch1 = ((gRawData.raw_bytes[1]<<8) | gRawData.raw_bytes[0]);
	}
...
static int initial_epl6804(struct elan_epl_data *epld)
{
	int ret = 0;
	struct i2c_client *client = epld->client;
	...
	ret = elan_epl6804_I2C_Write(client,REG_0,W_SINGLE_BYTE,0x02,0x62);
	if(ret < 0)
		return -EINVAL;
	ret = elan_epl6804_I2C_Write(client,REG_1,W_SINGLE_BYTE,0x02,0x32);    
	// Disable interrupt mode, set Reg_9 (INTTY[1:0]) to 0x02
	ret = elan_epl6804_I2C_Write(client,REG_9,W_SINGLE_BYTE,0x02,0x02);  	
...
#ifdef DEBUG_REG_DUMP
        elan_epl6804_register_dump();
#endif

	return ret;
}

static int epl6804_setup(struct elan_epl_data *epld)
{
	int err = 0;
	...
	if (!epld->epl_input_dev) {
		pr_err("[ELAN epl6804 error]%s: could not allocate ls input device\n",__func__);
		return -ENOMEM;
	}
	...
	if (err < 0) {
		pr_err("[ELAN epl6804 error]%s: can not register ls input device\n",__func__);
		goto err_free_epl_input_device;
	}

	err = misc_register(&elan_epl_device);
	if (err < 0) {
		pr_err("[ELAN epl6804 error]%s: can not register ls misc device\n",__func__);
		goto err_unregister_epl_input_device;
	}

	err = initial_epl6804(epld);
	if (err < 0) {
		pr_err("[ELAN epl6804 error]%s: fail to initial epl6804 (%d)\n",__func__, err);
		goto initial_fail;
	}
...
static int elan_epl6804_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	struct elan_epl_data *epld = NULL;
	struct sensors_pm_platform_data *pdata = NULL; /*elan_epl_platform_data*/ 
	int err = 0;
	
	printk("elan-epl6804 probe start\n");

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[ELAN epl6804 error] i2c func error\n");
		return -ENOTSUPP;
	}

	pdata = client->dev.platform_data;
	if (!pdata) {
		printk("[ELAN epl6804 error] platform data null\n");
		return -1;
	}
	...
	err = epld->init(pdata);
	if(err < 0) {
		printk("[ELAN epl6804 error] epl6804 initilization failure\n");
		return err;
	}

	epld->epl_wq = create_singlethread_workqueue("epl6804_wq");
	if (!epld->epl_wq) {
		printk("[ELAN epl6804 error] can't create workqueue\n");
		err = -ENOMEM;
		goto err_create_singlethread_workqueue;
	}

	err = epl6804_setup(epld);
	if (err < 0) {
		printk("[ELAN epl6804 error] epl6804_setup error\n");
		goto err_epl6804_setup;
	}

	err = queue_delayed_work(epld->epl_wq, &polling_work,msecs_to_jiffies(POLLING_RATE));
	if(err<0)
	{
		printk("[ELAN epl6804 error] queue_delayed_work fail\n");
	...
	if (err !=0) {
		printk("[ELAN epl6804 error] create sysfs group error\n");
		goto err_fail;		
	}

 
	if (sysfs_create_link(&devices_kset->kobj, &epld->epl_input_dev->dev.kobj, "light")) {
		printk("[ELAN epl6804 error] create link to device failed\n");
	}
 

	printk("elan-epl6804 probe end\n");
	return err;
...
static int elan_epl6804_remove(struct i2c_client *client)
{
...
static int elan_epl6804_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct elan_epl_data *epld = i2c_get_clientdata(client);
	struct sensors_pm_platform_data *pdata = client->dev.platform_data;
	int ret = 0;
	...
	if (epld->suspend){
		...
		if (ret < 0) {
			dev_err(&client->dev, "epl6804_suspend suspend light power failed\n");
			return ret;
		} else {
			printk("[epl6804] suspend\n");
		}	
	}
...
static int elan_epl6804_resume(struct i2c_client *client)
{
	struct elan_epl_data *epld = i2c_get_clientdata(client);
	struct sensors_pm_platform_data *pdata = client->dev.platform_data;
	int ret = 0;
	...
	if (epld->resume) {
		...
		if (ret < 0){
			dev_err(&client->dev, "epl6804_resume light power on failed\n");
			return ret;
		} else {
			printk("[epl6804] resume\n");
		}			
	}

	/* After system suspend, the registers are cleared.
	   So we should re-initialize the registers. */
	initial_epl6804(epld);

	return 0;
}

static void elan_epl6804_shutdown(struct i2c_client *client)
{
	struct elan_epl_data *epld = i2c_get_clientdata(client);
	struct sensors_pm_platform_data *pdata = client->dev.platform_data;
	...
	printk("[epl6804] shutdown\n");
}

static const struct i2c_device_id elan_epl6804_id[] = {
	{ LIGHT_DRIVER_NAME, 0 },
	{},
};

MODULE_DEVICE_TABLE(i2c, elan_epl6804_id);

static struct i2c_driver elan_epl6804_driver = {
	.probe = elan_epl6804_probe,
	.remove = elan_epl6804_remove,
	.suspend = elan_epl6804_suspend,
	.resume = elan_epl6804_resume,
	.shutdown = elan_epl6804_shutdown,
	.id_table = elan_epl6804_id,
	.driver = {
...
static int __init elan_epl6804_init(void)
{
	return i2c_add_driver(&elan_epl6804_driver);
}

static void __exit elan_epl6804_exit(void)
{
	i2c_del_driver(&elan_epl6804_driver);
}

module_init(elan_epl6804_init);
module_exit(elan_epl6804_exit);

MODULE_DESCRIPTION("Innocomm ELAN epl6804 driver");
MODULE_LICENSE("GPL");
