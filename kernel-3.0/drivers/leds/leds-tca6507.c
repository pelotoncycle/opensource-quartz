...
static void tca6507_work(struct work_struct *work)
{
	...
	//Reset all registers before setting new LED pattern, for  synchronize PWM timming
	for (r = 0; r < TCA6507_REG_CNT-1; r++)
		i2c_smbus_write_byte_data(cl, r, 0);
				   ...
				   enum led_brightness brightness)
{
	...
	led->offtime = 0;
	led_assign(led);
	pr_info("tca6507_brightness_set, led->num:%d, brightness:%d\n",led->num,brightness);
}
			     ...
			     unsigned long *delay_off)
{
	...
	*delay_off = led->offtime;
	pr_info("tca6507_blink_set, led->num:%d, brightness:%d\n",led->num,led->led_cdev.brightness);
	return 0;
}
				   ...
				   const struct i2c_device_id *id)
{
	...
	pr_info("tca6507_probe, pdata->leds.num_leds:%x\n",pdata->leds->num_leds);
	gpio_request(pdata->gpio_enable, "LED_DRIVER_EN");
	...
	schedule_work(&tca->work);
#else
	for (i = 0; i < TCA6507_REG_CNT-1; i++)
		tca->reg_file[i] = i2c_smbus_read_byte_data(client, i);
#endif
	pr_info("tca6507_probe, ready to work!\n");
	return 0;
exit:
	while (i--) {
...
static int __devexit tca6507_remove(struct i2c_client *client)
{
	...
	kfree(tca);
	/* Set tca6507_en_pin as output low*/
...
#if 0
static int tca6507_suspend(struct i2c_client *client, pm_message_t state)
{
#else
static int tca6507_suspend(struct device *dev)
{
	...
	if((tca->reg_file[0]==0)&&(tca->reg_file[1]==0)&&(tca->reg_file[2]==0))
		gpio_direction_output(pdata->gpio_enable,0);
...
#if defined(CONFIG_MACH_OMAP4_WINMATE) || defined(CONFIG_MACH_OMAP4_PB1ICOM)
	if((tca->reg_file[0]==0)&&(tca->reg_file[1]==0)&&(tca->reg_file[2]==0))
#else
	if(gpio_get_value(pdata->gpio_enable)==0)
#endif
		gpio_direction_output(pdata->gpio_enable,1);
...
static struct dev_pm_ops tca6507_pm = {
