...
static int rtc_twl_msecure_gpio;
...
static int twl_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (rtc_twl_msecure_gpio)
		gpio_set_value(rtc_twl_msecure_gpio, 1);
...
out:
	if (rtc_twl_msecure_gpio)
		gpio_set_value(rtc_twl_msecure_gpio, 0);
	return ret;
}
...
static int __devinit twl_rtc_probe(struct platform_device *pdev)
{
	...
	u8 rd_reg;
	struct twl_rtc_data *pdata = pdev->dev.platform_data;

	rtc_twl_msecure_gpio = 0;
	if (pdata) {
		if (pdata->msecure_gpio > 0) {
			ret = gpio_request(pdata->msecure_gpio, "twl-msecure");
			if (ret < 0) {
				dev_err(&pdev->dev, "can't request gpio %d\n",
					pdata->msecure_gpio);
				goto out0;
			}
			rtc_twl_msecure_gpio = pdata->msecure_gpio;
			gpio_direction_output(rtc_twl_msecure_gpio, 1);
		}
		if (pdata->default_time.tm_mday <= 0)
			dev_warn(&pdev->dev, "invalid tm_mday %d\n",
				pdata->default_time.tm_mday);
		if (pdata->default_time.tm_mon > 11)
			dev_warn(&pdev->dev, "invalid tm_mon %d\n",
				pdata->default_time.tm_mon);
		if (pdata->default_time.tm_year <= 100)
			dev_warn(&pdev->dev, "invalid tm_year %d\n",
				pdata->default_time.tm_year);
	}
	...
	if (!(rd_reg & BIT_RTC_CTRL_REG_STOP_RTC_M)) {
		if (pdata && pdata->default_time.tm_year > 100) {
			unsigned char rtc_data[ALL_TIME_REGS + 1];

			rtc_data[1] = bin2bcd(pdata->default_time.tm_sec);
			rtc_data[2] = bin2bcd(pdata->default_time.tm_min);
			rtc_data[3] = bin2bcd(pdata->default_time.tm_hour);
			rtc_data[4] = bin2bcd(pdata->default_time.tm_mday);
			rtc_data[5] = bin2bcd(pdata->default_time.tm_mon + 1);
			rtc_data[6] = bin2bcd(pdata->default_time.tm_year - 100);

			/* update all the time registers in one shot */
			ret = twl_i2c_write(TWL_MODULE_RTC, rtc_data,
				(rtc_reg_map[REG_SECONDS_REG]), ALL_TIME_REGS);
			if (ret < 0) {
				dev_err(&pdev->dev, "init rtc time error %d\n", ret);
				goto out1;
			}
			dev_info(&pdev->dev, "init time: %04d/%02d/%02d %02d:%02d:%02d\n",
					pdata->default_time.tm_year + 1900,
					pdata->default_time.tm_mon + 1,
					pdata->default_time.tm_mday,
					pdata->default_time.tm_hour,
					pdata->default_time.tm_min,
					pdata->default_time.tm_sec);
		}
...
out1:
	if (rtc_twl_msecure_gpio) {
		gpio_set_value(rtc_twl_msecure_gpio, 0);
		gpio_free(rtc_twl_msecure_gpio);
		rtc_twl_msecure_gpio = 0;
	}
out0:
	return ret;
