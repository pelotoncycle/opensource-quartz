...
static const char* duty_state_str[] = {
	"NORMAL",
	"HEATING",
	"COOLING_0",
	"COOLING_1",
...
/* state struct */
static unsigned int nitro_interval = 20000; /* ms */
...
#ifdef CONFIG_OMAP_THERMAL_DEBUG_VERBOSE
	pr_info("%s enter at (%u) state: NORMAL<-%s\n", __func__, policy->cur, duty_state_str[state]);
#endif
...
#ifdef CONFIG_OMAP_THERMAL_DEBUG_VERBOSE
	pr_info("%s enter at (%u); state: %s<-%s; %u ms to exit cool\n", __func__, policy->cur,
		duty_state_str[next_state], duty_state_str[state],
...
{
#ifdef CONFIG_OMAP_THERMAL_DEBUG_VERBOSE
	pr_info("%s enter; state: HEATING<-%s; %u ms to exit heating\n",
		__func__, duty_state_str[state], duty_desc.heating_budget);
#endif
...
{
	static unsigned int prev_old_freq = UINT_MAX, prev_new_freq = UINT_MAX;
	...
	/* Workaround to avoid 2 CPUFREQ_POSTCHANGE notify events on OMAP4 Kernel */
	if (prev_new_freq == freqs->new && prev_old_freq == freqs->old)
		goto done;

	prev_new_freq = freqs->new;
	prev_old_freq = freqs->old;

#ifdef CONFIG_OMAP_THERMAL_DEBUG_VERBOSE
	pr_info("%s: freqs %u->%u state: %s\n", __func__, freqs->old,
			freqs->new, duty_state_str[state]);
#endif
...
/* Remember to modify the condition in init_duty_governor()/omap4_duty_governor_pcb_sections for OMAP4_DUTY_HEATING state */
#if !defined(CONFIG_MACH_OMAP4_PB1ICOM) && !defined(CONFIG_MACH_OMAP4_QUARTZ)
		if (freqs->new < cooling_rate)
#else
		if (freqs->new < nitro_rate)
#endif
		{
...
static int omap4_duty_resume(struct device *dev)
{
	int ret = 0;
#ifdef CONFIG_OMAP_THERMAL_DEBUG_VERBOSE
	pr_info("%s enter at ()\n", __func__);
	...
	ret = register_hotcpu_notifier(&omap4_duty_cycle_cpu_notifier);
	if (ret) {
		pr_err("%s: failed to register cpu hotplug notifier\n",
		       __func__);
		goto exit;
	}

	ret = cpufreq_register_notifier(&omap4_duty_nb,
					CPUFREQ_TRANSITION_NOTIFIER);
	if (ret) {
		pr_err("%s: failed to setup cpufreq_notifier\n",
		       __func__);
...
static const struct dev_pm_ops omap4_duty_dev_pm_ops = {
	.suspend = omap4_duty_suspend,
	.resume = omap4_duty_resume,
};
