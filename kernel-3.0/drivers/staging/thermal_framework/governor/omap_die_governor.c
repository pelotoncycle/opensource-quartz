...
#ifdef CONFIG_MACH_OMAP4_PUZZLE
#define OMAP_PANIC_TEMP 90000
#define OMAP_ALERT_TEMP 80000
#define OMAP_MONITOR_TEMP 65000
#else
#if 0
...
#else
#define OMAP_PANIC_TEMP	90000
#define OMAP_ALERT_TEMP	75000
#define OMAP_MONITOR_TEMP	60000

#define HYSTERESIS_VALUE	8000
#define DECREASE_MPU_FREQ_PERIOD	10000
#endif
#endif
...
#ifndef HYSTERESIS_VALUE
#define HYSTERESIS_VALUE 2000
#endif
#ifndef NORMAL_TEMP_MONITORING_RATE
#define NORMAL_TEMP_MONITORING_RATE 1000
#endif
#ifndef FAST_TEMP_MONITORING_RATE
#define FAST_TEMP_MONITORING_RATE 250
#endif
#ifndef DECREASE_MPU_FREQ_PERIOD
...
static signed int convert_omap_sensor_temp_to_hotspot_temp(int sensor_temp)
{
	...
	pr_info("cpu hot spot: sensor %d avg sensor %d pcb %d, delta %d hot spot %d\n",
			sensor_temp, omap_gov->avg_cpu_sensor_temp,
				...
				struct list_head *cooling_list, int cpu_temp)
{
	...
	if (set_cooling_level) {
		if (zone->cooling_increment) {
			...
			if (omap_gov->cooling_level < 0) /* workaround for the overflow */
				omap_gov->cooling_level = 0;
		} else
			omap_gov->cooling_level = 0;
		thermal_device_call_all(cooling_list, cool_device,
						omap_gov->cooling_level);
	} else if (!zone->cooling_increment && omap_gov->cooling_level > 0) {
		omap_gov->cooling_level = 0;
		thermal_device_call_all(cooling_list, cool_device, 0);
	}
...
static int omap_cpu_thermal_manager(struct list_head *cooling_list, int temp)
{
	...
	if (cpu_temp >= OMAP_FATAL_TEMP) {
		...
		return FATAL_ZONE;
			...
			(omap_gov->prev_zone == PANIC_ZONE &&
				cpu_temp >= (OMAP_PANIC_TEMP - HYSTERESIS_VALUE))) {
		int temp_upper;
	...
	if (zone != NO_ACTION) {
		...
		if ((omap_gov->prev_zone != zone) || (zone == PANIC_ZONE)) {
			pr_info("%s: sensor %d avg sensor %d pcb %d, delta %d hot spot %d\n",
				 ...
				 omap_gov->pcb_temp, omap_gov->absolute_delta,
				 cpu_temp);
			pr_info("%s: hot spot %d - going into %s zone (%d-%d;%d)\n",
				__func__, cpu_temp, therm_zone->name,
				(zone != SAFE_ZONE) ? (therm_zone->temp_lower + HYSTERESIS_VALUE) : therm_zone->temp_lower,
				therm_zone->temp_upper, HYSTERESIS_VALUE);
		}
#endif
...
static ssize_t cpu_hotspot_temp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", omap_gov->hotspot_temp);
}

static DEVICE_ATTR(hotspot_temp, 0444, cpu_hotspot_temp_show, NULL);

static struct kobject *thermal_debug_kobj;

static void omap_die_governor_sysfs_init(void)
{
	int ret;

	thermal_debug_kobj = kobject_create_and_add("peloton", NULL) ;
	if (thermal_debug_kobj == NULL) {
		pr_err("%s: kobject_create_and_add failed\n", __func__);
		return;
	}

	ret = sysfs_create_file(thermal_debug_kobj, &dev_attr_hotspot_temp.attr);
	if (ret) {
		pr_info("%s: sysfs_create_file 'hotspot_temp' err %d\n", __func__, ret);
		return;
	}
}
