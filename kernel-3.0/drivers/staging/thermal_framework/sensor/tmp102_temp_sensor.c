...
static inline int tmp102_reg_to_mC(int config,s16 val)
	
{
	
	return(config&TMP102_CONF_EM)? ((val & ~0x01) * 1000) / 128:((val & ~0x01) * 1000) / 256;
...
static inline u16 tmp102_mC_to_reg(int config,int val)
	
{
	
	return (config&TMP102_CONF_EM)?(val * 128) / 1000:(val * 256) / 1000;
...
static int tmp102_read_current_temp(struct device *dev)
{
	...
	if (time_after(jiffies, tmp102->last_update + HZ / 3)) {
		...
		//	tmp102->temp[index] = tmp102_reg_to_mC(status);
			tmp102->temp[index] = tmp102_reg_to_mC(tmp102->config,status);
...
#define TMP105_CONFIG  (TMP102_CONF_TM | TMP102_CONF_R1|TMP102_CONF_R0)
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	switch(id->driver_data){
		...
		break;
		case 1: /*tmp105 config only 8bits*/
		default:	
			tmp102->config_orig = ret&0xFF00;
			ret = tmp102_write_reg(client, TMP102_CONF_REG, TMP105_CONFIG);
			if (ret < 0) {
				dev_err(&client->dev, "error writing config register\n");
				goto restore_config_err;
			}
			...
			tmp102->config= ret&0xFF00;
			if (tmp102->config != TMP105_CONFIG) {
				dev_err(&client->dev, "config settings did not stick\n");
				ret = -ENODEV;
				goto restore_config_err;
			}
...
static const struct i2c_device_id tmp102_id[] = {
	...
	{ "tmp105_temp_sensor", 1 },
	
	{ }
};
MODULE_DEVICE_TABLE(i2c, tmp102_id);

static struct dev_pm_ops tmp102_pm = {
