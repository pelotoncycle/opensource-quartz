...
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=0290b8c23a376ea4317a477533a3f1380dad27a3
...
static inline irqreturn_t serial_omap_irq(int irq, void *dev_id)
{
	...
	if (iir & UART_IIR_NO_INT) {
		serial_omap_port_disable(up);
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=ed4b67e8d15c38154221c9c70eec84eb7958d8a9
 * OMAP: UART: Fix IRQ handling return value
 * Ensure the Interrupt handling routine return IRQ_HANDLED vs IRQ_NONE.
 */
		return IRQ_HANDLED;
	}
...
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=f8bf7e79c3c47bfb19a6e4575fde95ab85c689ae
 * serial: omap: Exclude checking for empty TX FIFO in ISR
 */
#if 0
			...
			struct ktermios *old)
{
...
#if defined (CONFIG_MACH_OMAP4_SAGA)
	if (up->pdev->id == 3)
		up->fcr = UART_FCR_R_TRIG_10 | UART_FCR_T_TRIG_01 |
	...
	/* up->fcr_trigger is initialized in probe() function */
	up->fcr = up->fcr_trigger | UART_FCR_ENABLE_FIFO;
...
static int serial_omap_suspend(struct device *dev)
{
	...
	if (up) {
		...
		pm_runtime_get_sync(&up->pdev->dev);
		uart_suspend_port(&serial_omap_reg, &up->port);
		pm_runtime_dont_use_autosuspend(up->port.dev);
		pm_runtime_put_sync_suspend(&up->pdev->dev);
	}
	return 0;
}
...
static int serial_omap_resume(struct device *dev)
{
	...
	if (up) {
		...
		pm_runtime_get_sync(&up->pdev->dev);
		pm_runtime_mark_last_busy(&up->pdev->dev);
		pm_runtime_use_autosuspend(up->port.dev);
		uart_resume_port(&serial_omap_reg, &up->port);
				...
				unsigned long status, void *data)
{
	...
	case CLK_PRE_RATE_CHANGE:
		for (portid = 0 ; portid < OMAP_MAX_HSUART_PORTS ; portid++) {
			...
			if (!up)
				continue;
/* From omap.git (p-android-omap-3.0-dev), OMAP4: DPLL cascading: Removed deadlock during DPLL cascading transition */
#if 0
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	if (omap_up_info->fcr_trigger) {
		up->fcr_trigger = omap_up_info->fcr_trigger & 0xF0;
		dev_info(&pdev->dev, "FCR trigger 0x%02x\n", up->fcr_trigger);
	} else {
		up->fcr_trigger = UART_FCR_R_TRIG_01 | UART_FCR_T_TRIG_01;
	}

if (omap_up_info->no_iflag)
  up->no_iflag = omap_up_info->no_iflag;
...
#endif
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=43fa82e7f31f0ddb61471770189ebf5126d77999
...
int omap_serial_ext_uart_pause(u8 port_id,int pause)
{
	struct uart_omap_port *up;
	int err = 0;
