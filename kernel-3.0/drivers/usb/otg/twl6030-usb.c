...
struct twl6030_usb {
	...
	int irq_protection;
	struct delayed_work	delay_irq;
	struct mutex mutexlock;
	struct delayed_work	delay_mux;
	int uart3_via_usb;
	struct wake_lock        charger_det_lock;
};
extern void omap4430_dpdm_select(int mode,int x);
/*< end:#0005257: [FR]- Uart3 log via USB>*/
...
static int twl6030_usb_ldo_init(struct twl6030_usb *twl)
{
	...
	twl->otgoe = regulator_get(twl->dev, "otg_oe");
...
static irqreturn_t twl6030_usb_irq(int irq, void *_twl)
{
	...
	int status;
	u8 vbus_state, usbid_state, hw_state, misc2_data;
	unsigned charger_type;

	mutex_lock(&twl->mutexlock);
	if(twl->irq_protection>0){				
		printk("usb irq<5s, val = %d \n", twl->irq_protection);
		twl->irq_protection=VALUE_RESET_IN_IRQ;
		mutex_unlock(&twl->mutexlock);		
		return IRQ_HANDLED;
	}

	hw_state = twl6030_readb(twl, TWL6030_MODULE_ID0, STS_HW_CONDITIONS);
	usbid_state = hw_state & STS_USB_ID;
	...
	printk("twl6030_usb_irq  vbus_state[%02x]  prev_vbus[%02x]  usbid_state[%02x] prev_usbid[%02x]\n", vbus_state, twl->prev_vbus, usbid_state, twl->prev_usbid);

	if ((vbus_state) && !(usbid_state)) {
		...
		if(twl->uart3_via_usb)
		omap4430_dpdm_select(0,1);
		...
		if ((charger_type == POWER_SUPPLY_TYPE_USB_CDP)
			...
			regulator_disable(twl->usb3v3);
			wake_lock_timeout(&twl->charger_det_lock, msecs_to_jiffies(500));
			goto vbus_notify;
		}
	...
	if (!vbus_state && !usbid_state) {
		...
		if(twl->uart3_via_usb){
			schedule_delayed_work(&twl->delay_mux,	msecs_to_jiffies(500));
		}
...
static irqreturn_t twl6030_usbotg_irq(int irq, void *_twl)
{
	...
	u8 vbus_state, hw_state, usbid_state, misc2_data;

	mutex_lock(&twl->mutexlock);

	vbus_state = twl6030_readb(twl, TWL_MODULE_MAIN_CHARGE, CONTROLLER_STAT1);
	vbus_state = vbus_state & VBUS_DET;

	hw_state = twl6030_readb(twl, TWL6030_MODULE_ID0, STS_HW_CONDITIONS);
	usbid_state = hw_state & STS_USB_ID;

	if (usbid_state == twl->prev_usbid) {
		twl6030_writeb(twl, TWL_MODULE_USB, 0x1F, USB_ID_INT_LATCH_CLR);
		mutex_unlock(&twl->mutexlock);
		return IRQ_HANDLED;
	}

	printk("twl6030_usbotg_irq  vbus_state[%02x]  prev_vbus[%02x]  usbid_state[%02x] prev_usbid[%02x]\n", vbus_state, twl->prev_vbus, usbid_state, twl->prev_usbid);
	...
	if (usbid_state) {
		...
		if(twl->uart3_via_usb)
			...
			twl->linkstat = status;
			twl->otg.last_event = status;
			atomic_notifier_call_chain(&twl->otg.notifier, status, twl->otg.gadget);
		...
		if (twl->features & TWL6032_SUBCLASS) {
			twl->vbus_enable = 0;
			twl6030_writeb(twl, TWL_MODULE_MAIN_CHARGE , 0x00, CHARGERUSB_CTRL1);
			sysfs_notify(&twl->dev->kobj, NULL, "vbus");
			twl->prev_vbus = 0;
		}
...
static void twl6030_usb_mux_delay_work(struct work_struct *work)
{
	struct twl6030_usb *twl = container_of(work,
		struct twl6030_usb, delay_mux.work);
	u8 vbus_state = twl6030_readb(twl, TWL_MODULE_MAIN_CHARGE,
						CONTROLLER_STAT1) & VBUS_DET;
						
	if(!vbus_state)	
		omap4430_dpdm_select(1,1);

	
}
static void twl6030_usb_irq_delay_work(struct work_struct *work)
{
	struct twl6030_usb *twl = container_of(work,
		struct twl6030_usb, delay_irq.work);

	printk("clear usb irq protection \n");	

	mutex_lock(&twl->mutexlock);	
	if(twl->irq_protection==VALUE_RESET_IN_IRQ){
		twl->irq_protection = 0;		
		mutex_unlock(&twl->mutexlock);
		twl6030_usb_irq(0, twl);
	}else{
		twl->irq_protection = 0;	
		mutex_unlock(&twl->mutexlock);		
	}
...
static int __devinit twl6030_usb_probe(struct platform_device *pdev)
{
	...
	twl->otg.state		= OTG_STATE_UNDEFINED;
	twl->irq_protection = VALUE_SET_IN_PROBE;

	mutex_init(&twl->mutexlock);
	...
	INIT_DELAYED_WORK_DEFERRABLE(&twl->delay_irq,	twl6030_usb_irq_delay_work);
	...
	pdata->phy_init(dev);
	twl6030_phy_suspend(&twl->otg, 0);
	INIT_DELAYED_WORK_DEFERRABLE(&twl->delay_mux,twl6030_usb_mux_delay_work);
	twl->uart3_via_usb=pdata->uart3_via_usb;
	if(twl->uart3_via_usb){
		u8 vbus_state = twl6030_readb(twl, TWL_MODULE_MAIN_CHARGE,
						CONTROLLER_STAT1);
		regulator_enable(twl->usb3v3);
		if (!(vbus_state& VBUS_DET) ){
			omap4430_dpdm_select(1,0);
		}
	...
	schedule_delayed_work(&twl->delay_irq,	msecs_to_jiffies(5000));
...
static int __exit twl6030_usb_remove(struct platform_device *pdev)
{
	...
	if (!IS_ERR(twl->otgoe)) {
		regulator_put(twl->otgoe);
	}
	pdata->phy_exit(twl->dev);
	device_remove_file(twl->dev, &dev_attr_vbus);
	cancel_work_sync(&twl->set_vbus_work);
	cancel_delayed_work_sync(&twl->delay_mux);
