...
static u8 gpadc_save_ctrl_reg;/*mantis: 4459, porting - TWL6030: GPADC: Add suspend/resume hooks*/
static u8 gpadc_save_ctrl2_reg;/*mantis: 4459, porting - TWL6030: GPADC: Add suspend/resume hooks*/
...
static int __devinit twl6030_gpadc_probe(struct platform_device *pdev)
{
	...
	/*Enable Vsys and set prescaler divided by 5*/
	reg=twl6030_gpadc_read(gpadc, TWL6030_GPADC_CTRL);
	reg|=TWL6030_GPADC_CTRL_SCALER_EN;
		
	reg&=~TWL6030_GPADC_CTRL_SCALER_DIV4;	
		
	twl6030_gpadc_write(gpadc,TWL6030_GPADC_CTRL,reg);
	
	/*enable Channel 18 and set prescaler divided by 5*/
	reg=twl6030_gpadc_read(gpadc, TWL6030_GPADC_CTRL2);
	reg|=TWL6030_GPADC_CTRL2_SCALER_EN_CH18|TWL6030_GPADC_CTRL2_REMSENSE_0;
	reg&=~TWL6030_GPADC_CTRL2_VBAT_SCALER_DIV4;
		
	twl6030_gpadc_write(gpadc,TWL6030_GPADC_CTRL2,reg);
...
static int twl6030_gpadc_suspend(struct device *pdev)
{
	...
	struct twl6030_gpadc_data *gpadc= dev_get_drvdata(pdev);
	   
       ret = twl_i2c_read_u8(TWL_MODULE_MADC, 
	   	&gpadc_save_ctrl_reg, TWL6030_GPADC_CTRL);
	   
       if (!ret) {
               ret = twl_i2c_write_u8(TWL_MODULE_MADC, 0, TWL6030_GPADC_CTRL);
       } else {
               dev_err(gpadc->dev, "unable to disable GPADC Control !\n");
       }
	 if (gpadc->features & TWL6032_SUBCLASS) {
		
		ret = twl_i2c_read_u8(TWL_MODULE_MADC, 
			&gpadc_save_ctrl2_reg, TWL6030_GPADC_CTRL2);
		
	       if (!ret) {
	               ret = twl_i2c_write_u8(TWL_MODULE_MADC, 0, TWL6030_GPADC_CTRL2);
	       } else {
	               dev_err(gpadc->dev, "unable to disable GPADC Control2 !\n");
	       }
	 }
...
static int twl6030_gpadc_resume(struct device *pdev)
{
	...
	if (ret)
		...
		ret = twl_i2c_write_u8(TWL_MODULE_MADC, gpadc_save_ctrl_reg,
                                       TWL6030_GPADC_CTRL);
        if (ret)
           dev_err(gpadc->dev, "unable to enable GPADC Control !\n");		   
	  if (gpadc->features & TWL6032_SUBCLASS) {
	        ret = twl_i2c_write_u8(TWL_MODULE_MADC, 
				gpadc_save_ctrl2_reg, TWL6030_GPADC_CTRL2);
	        if (ret)
	           dev_err(gpadc->dev, "unable to enable GPADC Control2 !\n");	
	  }
