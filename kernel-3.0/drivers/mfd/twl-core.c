...
#define twl_has_pwm()	false
	...
	defined(CONFIG_TWL6032_BCI_BATTERY) || \
	defined(CONFIG_TWL603x_BCI_BATTERY) || \
...
_retry:
	...
	*value = twl_map[mod_no].base + reg;
	...
	if (ret != 1) {
		if (--retries > 0) {
			pr_warn("%s: [%d] i2c_write failed to transfer all messages "
				"(addr 0x%04x, reg %d, len %d)\n",
				DRIVER_NAME, retries, twl->address, twl_map[mod_no].base + reg, msg->len);
			msleep(10);
			goto _retry;
		}
	}
	...
	if (ret != 1) {
		pr_err("%s: i2c_write failed to transfer all messages "
			"(addr 0x%04x, reg %d, len %d)\n",
			DRIVER_NAME, twl->address, twl_map[mod_no].base + reg, msg->len);
		if (ret < 0)
			return ret;
		else
			return -EIO;
...
_retry:
	...
	if (ret != 2) {
		if (--retries > 0) {
			pr_warn("%s: [%d] i2c_read failed to transfer all messages "
				"(addr 0x%04x, reg %d, len %d)\n",
				DRIVER_NAME, retries, twl->address, twl_map[mod_no].base + reg, msg->len);
			msleep(10);
			goto _retry;
...
#if 0
		if(machine_is_omap4_wedjat() || machine_is_omap4_talos7()||machine_is_omap4_talos10()||machine_is_omap4_zeus()||
...
{		
	...
	if (twl_has_pwm() && twl_class_is_6030() && pdata->pwm
			&& (features & TWL6032_SUBCLASS)) {
		sub_chip_id = twl_map[TWL_MODULE_PWM].sid;
		child = add_child(sub_chip_id, "twl6032_pwm",
				pdata->pwm, sizeof(*pdata->pwm),
				true, 0, 0);
		if (IS_ERR(child))
			return PTR_ERR(child);
	}
...
static void _suspend_twl6030_settings(void)
{
	...
	/* TOGGLE1 */
	twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x01, 0x90);
	/* MISC1 */
	 ...
	 */
//	twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x6a, 0xE6);
	/* CFG_INPUT_PUPD2 */
	...
	/* CHARGERUSB_CTRL3 */
	twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x01, 0xEA);
#endif
	/*
...
static void _resume_twl6030_settings(void)
{

	/* TOGGLE1 */
	twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x02, 0x90);
#ifndef CONFIG_USB_MUSB_PERIPHERAL
	twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x14, 0x06);
#endif
}
...
static int twl_suspend(struct device *dev)
{
	...
	if(twl_class_is_6030()){
		u8 pwr1,pwr2;
		_suspend_twl6030_settings();
		/*Debug Usage*/
		twl_i2c_read_u8(TWL6030_MODULE_ID1, &pwr1,	0x93);
		twl_i2c_read_u8(TWL6030_MODULE_ID1, &pwr2,	0x94);
...
static struct dev_pm_ops twl_pm = {
	.suspend = twl_suspend,
	.resume  = twl_resume,
};
