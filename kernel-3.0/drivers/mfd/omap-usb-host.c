...
static void omap_usbhs_init(struct device *dev)
{
	...
	if (pdata->ehci_data->hub_reset) {
		if (gpio_is_valid(pdata->ehci_data->hub_reset_gpio_port)) {
			gpio_request(pdata->ehci_data->hub_reset_gpio_port, "USB hub reset");
			gpio_direction_output(pdata->ehci_data->hub_reset_gpio_port, 0);
		}
		udelay(10);
		if (gpio_is_valid(pdata->ehci_data->hub_reset_gpio_port)) {
			gpio_set_value(pdata->ehci_data->hub_reset_gpio_port, 0);
			mdelay(2);
			gpio_set_value(pdata->ehci_data->hub_reset_gpio_port, 1);
			mdelay(2);
			gpio_set_value(pdata->ehci_data->hub_reset_gpio_port, 0);
		}
	}
