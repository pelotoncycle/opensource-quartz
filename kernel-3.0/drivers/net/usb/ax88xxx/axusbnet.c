...
#if 0
	flush_scheduled_work ();//kernel 2.6
#else
	cancel_work_sync(&dev->kevent);//kernel 3.0
#endif
...
/*-------------------------------------------------------------------------*/ 
// precondition: never called in_interrupt 
static void axusbnet_terminate_urbs(struct usbnet *dev)
{
	DECLARE_WAIT_QUEUE_HEAD_ONSTACK(unlink_wakeup);
	DECLARE_WAITQUEUE(wait, current);
	int temp;
...
#endif
{
	...
	if (!dev->suspend_count++) {
		...
		netif_device_detach(dev->net);
#if 0
		(void) unlink_urbs (dev, &dev->rxq);//kernel 2.6
		(void) unlink_urbs (dev, &dev->txq);//kernel 2.6
#else
		axusbnet_terminate_urbs(dev);//kernel 3.0
#endif 
		usb_kill_urb(dev->interrupt);
		/*
