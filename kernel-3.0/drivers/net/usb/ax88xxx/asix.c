...
#ifdef CONFIG_MACH_OMAP4_QUARTZ
#include <linux/gpio.h>
#include <linux/regulator/consumer.h>

#include "../../../arch/arm/mach-omap2/mux.h"
#endif	// CONFIG_MACH_OMAP4_QUARTZ
...
static struct wake_lock ax88xxx_wakelock;
...
#ifdef CONFIG_MACH_OMAP4_QUARTZ
static struct regulator *vnet_d3v3	= NULL;
static bool is_ETHERNET_EN_requestd	= false;
static bool is_ETHERNET_RST_requestd	= false;
#endif	// CONFIG_MACH_OMAP4_QUARTZ
...
static int ax8817x_set_mac_addr(struct net_device *net, void *p)
{
	...
	//Inhibit modify MAC address by other way like "# ip link set eth0 address XX:XX:XX:XX:XX:XX" IF MAC address derives from Bootloader.
	if (is_MAC_derives_from_bootloader()) {
		printk(KERN_WARNING "WARNING: MAC address assigned by Boot Loader, Cannot assign requested address.\n");	
...
static void ax88772b_status(struct usbnet *dev, struct urb *urb)
{
	...
	//printk("[%s]: event->link=0x%x ; link=%d ; carrier_ok=%d \n", __FUNCTION__, event->link, link, netif_carrier_ok(dev->net));

	if (netif_carrier_ok(dev->net) != link) {

		//Laker: to make sure that linkstate will be updated to ConnectivityService
		wake_lock_timeout(&ax88xxx_wakelock, 10*HZ);

		if (link) {
			netif_carrier_on(dev->net);
			ax772b_data->Event = AX_SET_RX_CFG;

			printk("[%s]: carrier_on ; AX_SET_RX_CFG \n", __FUNCTION__);
		} else {
			netif_carrier_off(dev->net);
			ax772b_data->time_to_chk = jiffies;

			printk("[%s]: carrier_off ; update time_to_chk \n", __FUNCTION__);
		}
	...
	if (!link) {
		...
		//printk("[%s]: no_cable=%d ; pw_enabled=%d \n", __FUNCTION__, no_cable, ax772b_data->pw_enabled);
		...
		if (no_cable) {
			...
			//Laker: if we don't have ethernet cable, update the time_to_chk to prevent from
			//             doing PHY_POWER_UP upon connecting cable to save some time
			ax772b_data->time_to_chk = jiffies;
		} else {
	...
	if (!link) {
		...
		if (no_cable) {
			...
			if (ax772b_data->pw_enabled ||
				...
				ax772b_data->Event = PHY_POWER_UP;
				ax772b_data->time_to_chk = jiffies;
				printk("[%s]: has_cable ; PHY_POWER_UP \n", __FUNCTION__);
			}
		}
	}
...
static int ax88772b_resume(struct usb_interface *intf)
{
	...
	//Laker: USB_DPLL power domain issue (if no ethernet cable is connected).
	wake_lock_timeout(&ax88xxx_wakelock, 5*HZ);
	...
	/* Reset the MAC address of dev->net->dev_addr */

	if ((ret = ax8817x_write_cmd (dev, AX88772_CMD_WRITE_NODE_ID, 0, 0,
				      ETH_ALEN, dev->net->dev_addr)) < 0) {
		deverr(dev, "set MAC address failed: %d", ret);
	}


//Laker: let the ax88772b_status() set netif_carrier_off() and update time_to_chk
#if 0
	netif_carrier_off(dev->net);
	...
	if (ax772b_data->OperationMode == OPERATION_PHY_MODE)
		netif_carrier_on(dev->net);
#else
	//Laker: AX88772B already entered power saving state, set pw_enable=1 to send PHY_POWER_UP in ax88772b_status()
	ax772b_data->pw_enabled = 1;
#endif
...
#ifdef ETHERNET_PHY_PS_CONFIG
	*tmp16 = ETHERNET_PHY_PS_CONFIG;
#endif /* ETHERNET_PHY_PS_CONFIG */
	le16_to_cpus(tmp16);
	...
	data->resume = ax88772b_resume;
        ...
        wake_lock_init(&ax88xxx_wakelock, WAKE_LOCK_SUSPEND, "ax88xxx");
...
{
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->priv;
        wake_lock_destroy(&ax88xxx_wakelock);
	if (ax772b_data) {
...
#ifdef ETHERNET_PHY_PS_CONFIG
			tmp16 = ETHERNET_PHY_PS_CONFIG;//nya
#endif /* ETHERNET_PHY_PS_CONFIG */
...
/*
	...
	1.(All of) GPIO default: Input, Pull Low (pre-set at "static struct omap_pad_state_description" in board-44xx-XXXX.c).
 	2.ETHERNET_EN(USB transceiver) control same time and level with VNET_D3V3.
	  Reference: AX88772B specification (Document No: AX88772B/V1.04/08/10/11)
	    1. V_BUS pin define @ 2.1 AX88772B 64-pin Pinout Description, page 12.
	    2. Note 4 @ A.1 Procedure to Enable Default WOL Ready Mode, page 92.
*/
int asix_init_set_power(bool on) {
	int ret = -EIO;

	printk(KERN_INFO "%s() start, Set power %s.\n", __FUNCTION__, true == on ? "On" : "Off");

	do {
		/* Note: Register LDO at "static struct regulator_consumer_supply twl_XXX_supply[] by REGULATOR_SUPPLY () in board-XXXX-twl.c */
		/* Request VNET_D3V3 LDO */
		if (!vnet_d3v3) {
			vnet_d3v3 = regulator_get(NULL, REGULATOR_ETHERNET_3V3);
			if (IS_ERR(vnet_d3v3)) {
				pr_err("%s(): Ethernet 3V3 regulator missing (%ld) ??\n", __FUNCTION__, PTR_ERR(vnet_d3v3));
				vnet_d3v3 = NULL;
				break;
			}
		}

		if (false == is_ETHERNET_EN_requestd) {
			if ((ret = gpio_request(GPIO_ETHERNET_EN, "ethernet_en")) != 0) {
				pr_err("%s(): Can't reserve GPIO: %d.\n", __FUNCTION__, GPIO_ETHERNET_RST);
				break;
			}

			is_ETHERNET_EN_requestd = true;
		}

		if (false == is_ETHERNET_RST_requestd) {
			if ((ret = gpio_request(GPIO_ETHERNET_RST, "ethernet_reset")) != 0) {
				pr_err("%s(): Can't reserve GPIO: %d.\n", __FUNCTION__, GPIO_ETHERNET_RST);
				break;
			}

			is_ETHERNET_RST_requestd = true;
		}

		if (true == on) {
			if ((ret = regulator_enable(vnet_d3v3)) != 0) {
				pr_err("%s(): Failed to turn on Ethernet 3V3.\n", __FUNCTION__);
				break;
			}

			// Attach USB transceiver. 
			if ((ret = gpio_direction_output(GPIO_ETHERNET_EN, 0)) != 0) {
				gpio_free(GPIO_ETHERNET_EN);
				is_ETHERNET_EN_requestd = false;
				pr_err("%s(): Can't set ETHERNET_EN GPIO: %d to 0.\n", __FUNCTION__, GPIO_ETHERNET_EN);
				break;
			}

			msleep(ETHERNET_RESET_RELEASE_TIME);

			if ((ret = gpio_direction_output(GPIO_ETHERNET_RST, 1)) != 0) {
				gpio_free(GPIO_ETHERNET_RST);
				is_ETHERNET_RST_requestd = false;
				pr_err("%s(): Can't set ETHERNET_RST GPIO: %d to 1.\n", __FUNCTION__, GPIO_ETHERNET_RST);
				break;
			}
		} else {
			if ((ret = gpio_direction_output(GPIO_ETHERNET_RST, 0)) != 0) {
				gpio_free(GPIO_ETHERNET_RST);
				is_ETHERNET_RST_requestd = false;
				pr_err("%s(): Can't set ETHERNET_RST GPIO: %d to 0.\n", __FUNCTION__, GPIO_ETHERNET_RST);
				break;
			}

			msleep(ETHERNET_RESET_RELEASE_TIME);

			// Detach USB transceiver. 
			if ((ret = gpio_direction_output(GPIO_ETHERNET_EN, 1)) != 0) {
				gpio_free(GPIO_ETHERNET_EN);
				is_ETHERNET_EN_requestd = false;
				pr_err("%s(): Can't set ETHERNET_EN GPIO: %d to 1.\n", __FUNCTION__, GPIO_ETHERNET_EN);
				break;
			}

			if ((ret = regulator_disable(vnet_d3v3)) != 0) {
				pr_err("%s(): Failed to turn off Ethernet 3V3.\n", __FUNCTION__);
				break;
			}
		}

		ret = 0;
		printk(KERN_INFO "%s() finish successful.\n", __FUNCTION__);
	} while(0);

#if 0
	if (vnet_d3v3)
		regulator_put(vnet_d3v3);
#endif

	return ret;
}
EXPORT_SYMBOL_GPL(asix_init_set_power);
#endif	// CONFIG_MACH_OMAP4_QUARTZ
...
static int __init asix_init(void)
{
		...
		if ((ret = omap_mux_init_gpio(GPIO_ETHERNET_EN, OMAP_PIN_OUTPUT | OMAP_MUX_MODE3)) != 0) {
			pr_err("%s(): Can't initial GPIO: %d.\n", __FUNCTION__, GPIO_ETHERNET_EN);
		}

		if ((ret = omap_mux_init_gpio(GPIO_ETHERNET_RST, OMAP_PIN_OUTPUT | OMAP_MUX_MODE3)) != 0) {
			pr_err("%s(): Can't initial GPIO: %d.\n", __FUNCTION__, GPIO_ETHERNET_RST);
		}

        if ((ret = asix_init_set_power(true)) != 0) {
                pr_err("%s(): Power ON fail! rc=%d, Quit.\n", __FUNCTION__, ret);
                return ret;
        }
#endif	// CONFIG_MACH_OMAP4_QUARTZ
...
#ifdef CONFIG_MACH_OMAP4_QUARTZ
        if ((ret = asix_init_set_power(false)) != 0)
                pr_err("%s(): Power OFF fail! rc=%d.\n", __FUNCTION__, ret);
#endif	// CONFIG_MACH_OMAP4_QUARTZ
}
...
static struct wake_lock quartz_eth_reset_wakelock;
static struct kobject *quartz_eth_sysfs_kobj;

static ssize_t quartz_eth_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/* Hold a 30-sec wakelock to avoid the immediate Linux PM suspend */
	wake_lock_timeout(&quartz_eth_reset_wakelock, 30 * HZ);

	asix_init_set_power(false);
	msleep(350);
	asix_init_set_power(true);

	return sprintf(buf, "quartz usb ethernet reset\n");
}

static DEVICE_ATTR(reset, S_IRUGO, quartz_eth_reset_show, NULL);

static int __init quartz_eth_sysfs_init(void)
{
	wake_lock_init(&quartz_eth_reset_wakelock, WAKE_LOCK_SUSPEND, "quartz_eth_reset");
	quartz_eth_sysfs_kobj = kobject_create_and_add("quartz_eth", NULL) ;
	if (quartz_eth_sysfs_kobj == NULL) {
		pr_err("%s: kobject err\n", __func__);
	} else {
		int ret;

		ret = sysfs_create_file(quartz_eth_sysfs_kobj, &dev_attr_reset.attr);
		if (ret)
			pr_err("%s: reset sysfs err %d\n", __func__, ret);
	}
	return 0;
}
late_initcall(quartz_eth_sysfs_init);

static void __exit quartz_eth_sysfs_exit(void)
{
	wake_lock_destroy(&quartz_eth_reset_wakelock);
	if (quartz_eth_sysfs_kobj) {
		sysfs_remove_file(quartz_eth_sysfs_kobj, &dev_attr_reset.attr);
		kobject_del(quartz_eth_sysfs_kobj);
	}
}
module_exit(quartz_eth_sysfs_exit);
