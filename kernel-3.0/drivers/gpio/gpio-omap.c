...
{
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=90bc9ac779465751ddb509d8e9cfb6d0354b4fb9
...
static void gpio_mask_irq(struct irq_data *d)
{
	...
	spin_lock_irqsave(&bank->lock, flags);
	_set_gpio_irqenable(bank, gpio, 0);
#ifndef CONFIG_GPIO_OMAP_IRQ_SUPPORT_DISABLE_FUNCTION
	_set_gpio_triggering(bank, GPIO_INDEX(bank, gpio), IRQ_TYPE_NONE);
#endif
	spin_unlock_irqrestore(&bank->lock, flags);
}
...
/** http://git.omapzoom.org/?p=kernel/omap.git;a=commit;h=290cdf9401be0f6be1b6488a01149374564b07d5
 * GPIO: OMAP: Fix the sequence to clear the IRQ status
 * The recommended way to clear the GPIO IRQ status is to:
 * - Enable the IRQ
 * - clear the status
 * If this above sequence is not followed, i.e., if the status
 * is cleared after disabling the IRQ then sWAKEUP will not be
 * cleared and gates the module transition.
 */
#if 0
...
#ifdef CONFIG_GPIO_OMAP_IRQ_SUPPORT_DISABLE_FUNCTION
/**
...
#ifdef CONFIG_GPIO_OMAP_IRQ_SUPPORT_DISABLE_FUNCTION_DEBUG
	dev_info(bank->dev, "gpio %u irq %u disabled\n", gpio, d->irq);
#endif
}
...
#ifdef CONFIG_GPIO_OMAP_IRQ_SUPPORT_DISABLE_FUNCTION_DEBUG
	dev_info(bank->dev, "gpio %u irq %u enabled\n", gpio, d->irq);
#endif
}
#endif
