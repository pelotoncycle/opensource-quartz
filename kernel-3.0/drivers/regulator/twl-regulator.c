...
static int twl6030ldo_suspend_enable(struct regulator_dev *rdev)
{
	...
	struct twlreg_info	*info = rdev_get_drvdata(rdev);
	int ret = twl6030reg_set_trans_state(rdev, TWL6030_CFG_TRANS_SLEEP_SHIFT,
					TWL6030_CFG_TRANS_STATE_AUTO);
	if (ret)
		pr_err("%s: <%s> err %d\n", __func__, info->desc.name, ret);
	else
		pr_info("%s: <%s> SLEEP AUTO\n", __func__, info->desc.name);
	return ret;
#else
	return twl6030reg_set_trans_state(rdev, TWL6030_CFG_TRANS_SLEEP_SHIFT,
					TWL6030_CFG_TRANS_STATE_AUTO);
#endif /* CONFIG_REGULATOR_VERBOSE_DEBUG */
}
		...
		if (min_uV <= uV && uV <= max_uV) {
			...
			*selector = vsel;
			ret = twlreg_write(info, TWL_MODULE_PM_RECEIVER,
							VREG_VOLTAGE, vsel);
			if (!ret) {
				pr_info("%s: <%s> %d mV\n", __func__, info->desc.name, mV);
			} else {
				pr_err("%s: <%s> %d mV err %d\n", __func__, info->desc.name, mV, ret);
			}
			return ret;
#else
			*selector = vsel;
...
#ifdef DEBUG
	pr_info("%s: <%s> (%dmV,%dmV) err\n",
			__func__, info->desc.name, min_uV/1000, max_uV/1000);
#endif /* DEBUG */
	return -EDOM;
}
...
#ifdef DEBUG
		pr_info("%s: <%s> invalid voltage (%d,%d)\n",
				__func__, info->desc.name, min_uV/1000, max_uV/1000);
#endif /* DEBUG */
		return -EDOM;
	}
...
#ifdef DEBUG
	{
		int ret = twlreg_write(info, TWL_MODULE_PM_RECEIVER, VREG_VOLTAGE, vsel);
		if (!ret) {
			pr_info("%s: <%s> %d mV\n", __func__, info->desc.name, min_uV/1000);
		} else {
			pr_err("%s: <%s> %d mV err %d\n", __func__, info->desc.name, min_uV/1000, ret);
		}
		return ret;
	}
#else
...
#ifdef DEBUG
			pr_info("%s: <%s> invalid voltage (%d,%d)\n",
					__func__, info->desc.name, min_uV, max_uV);
#endif /* DEBUG */
			return -EINVAL;
		}
		break;
...
#ifdef DEBUG
	{
		int ret = twlreg_write(info, TWL_MODULE_PM_RECEIVER, VREG_VOLTAGE_SMPS,
							vsel);
		if (!ret) {
			pr_info("%s: <%s> %d mV\n", __func__, info->desc.name, min_uV/1000);
		} else {
