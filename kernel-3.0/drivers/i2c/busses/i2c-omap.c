...
#define OMAP_I2C_BUS_BUSY_SHORT_TIMEOUT (msecs_to_jiffies(350))
...
#define OMAP_I2C_SYSTEST_SCL_I_FUNC	(1 << 8)	/* SCL line input value (functional mode) */
#define OMAP_I2C_SYSTEST_SCL_O_FUNC	(1 << 7)	/* SCL line output value (functional mode) */
#define OMAP_I2C_SYSTEST_SDA_I_FUNC	(1 << 6)	/* SDA line input value (functional mode) */
#define OMAP_I2C_SYSTEST_SDA_O_FUNC	(1 << 5)	/* SDA line output value (functional mode) */
...
static int omap_i2c_wait_for_bb_short(struct omap_i2c_dev *dev)
{
	unsigned long timeout;

	timeout = jiffies + OMAP_I2C_BUS_BUSY_SHORT_TIMEOUT;
	while (omap_i2c_read_reg(dev, OMAP_I2C_STAT_REG) & OMAP_I2C_STAT_BB) {
		if (time_after(jiffies, timeout)) {
			dev_warn(dev->dev, "timeout waiting for bus ready!\n");
			return -ETIMEDOUT;
...
static int omap_i2c_bus_clear(struct omap_i2c_dev *dev)
{
	...
	/* SCL low; SDA high */
	omap_i2c_write_reg(dev, OMAP_I2C_SYSTEST_REG,
		OMAP_I2C_SYSTEST_ST_EN | OMAP_I2C_SYSTEST_SDA_O);
	udelay(50);
	/* SCL high; SDA high */
	omap_i2c_write_reg(dev, OMAP_I2C_SYSTEST_REG,
		OMAP_I2C_SYSTEST_ST_EN | OMAP_I2C_SYSTEST_SCL_O | OMAP_I2C_SYSTEST_SDA_O);
	udelay(50);
	/* SCL low; SDA high */
	omap_i2c_write_reg(dev, OMAP_I2C_SYSTEST_REG,
		OMAP_I2C_SYSTEST_ST_EN | OMAP_I2C_SYSTEST_SDA_O);
	...
	/* SCL high; SDA low */
	omap_i2c_write_reg(dev, OMAP_I2C_SYSTEST_REG,
		OMAP_I2C_SYSTEST_ST_EN | OMAP_I2C_SYSTEST_SCL_O);
	udelay(50);
	/* SCL high; SDA high */
	omap_i2c_write_reg(dev, OMAP_I2C_SYSTEST_REG,
		OMAP_I2C_SYSTEST_ST_EN | OMAP_I2C_SYSTEST_SCL_O | OMAP_I2C_SYSTEST_SDA_O);
...

{
	...
	while (((w = omap_i2c_read_reg(dev, OMAP_I2C_SYSTEST_REG)) & (OMAP_I2C_SYSTEST_SCL_I_FUNC | OMAP_I2C_SYSTEST_SDA_I_FUNC))
			!= (OMAP_I2C_SYSTEST_SCL_I_FUNC | OMAP_I2C_SYSTEST_SDA_I_FUNC)) {
		if (--timeout == 0) {
			dev_warn(dev->dev, "bus clear failed: SDA=%d SCL=%d (0x%04x)!\n",
				(w & OMAP_I2C_SYSTEST_SDA_I_FUNC) >> 6, (w & OMAP_I2C_SYSTEST_SCL_I_FUNC) >> 8, w);
			break;
		}
		msleep(5);
	}
...
omap_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	r = omap_i2c_wait_for_bb_short(dev);
	if (r < 0)
	...
	omap_i2c_wait_for_bb_short(dev);
out:
#if 0
	omap_i2c_idle(dev);
